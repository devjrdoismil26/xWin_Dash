...
import { fetchProducts, fetchProductById, createProduct, updateProduct, deleteProduct, bulkUpdateProducts, bulkDeleteProducts, updateProductStatus, duplicateProduct, updateProductPrice, updateProductInventory, validateProductData, formatProductPrice, generateProductSKU } from '../services/productsCoreService';
...
interface UseProductsCoreReturn {
  ...
  changeProductInventory: (id: string, inventory: unknown) => Promise<ProductResponse>;
  // Utilitários
  validateData: (data: ProductFormData) => { isValid: boolean;
  errors: string[];
};
  ...
  clearCurrentProduct??: (e: any) => void;
}
...
export const useProductsCore = (): UseProductsCoreReturn => {
  ...
  const loadProduct = useCallback(async (id: string) => {
    ...
    try {
      ...
      setError(getErrorMessage(err) || 'Erro inesperado ao carregar produto');
  ...
  const createNewProduct = useCallback(async (data: ProductFormData): Promise<ProductResponse> => {
    ...
    try {
      ...
      const errorResponse = {
        success: false,
        error: getErrorMessage(err) || 'Erro inesperado ao criar produto'};
  ...
  const updateExistingProduct = useCallback(async (id: string, data: Partial<ProductFormData>): Promise<ProductResponse> => {
    ...
    try {
      ...
      const errorResponse = {
        success: false,
        error: getErrorMessage(err) || 'Erro inesperado ao atualizar produto'};
  ...
  const deleteExistingProduct = useCallback(async (id: string): Promise<ProductResponse> => {
    ...
    try {
      ...
      const errorResponse = {
        success: false,
        error: getErrorMessage(err) || 'Erro inesperado ao deletar produto'};
  ...
  const bulkUpdate = useCallback(async (ids: string[], updates: Partial<ProductFormData>): Promise<ProductResponse> => {
    ...
    try {
      ...
      const errorResponse = {
        success: false,
        error: getErrorMessage(err) || 'Erro inesperado ao atualizar produtos em lote'};
  ...
  const bulkDelete = useCallback(async (ids: string[]): Promise<ProductResponse> => {
    ...
    try {
      ...
      const errorResponse = {
        success: false,
        error: getErrorMessage(err) || 'Erro inesperado ao deletar produtos em lote'};
  ...
  const changeProductInventory = useCallback(async (id: string, inventory: unknown): Promise<ProductResponse> => {
    setLoading(true);
