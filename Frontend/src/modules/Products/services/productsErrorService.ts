...
const categorizeError = (error: unknown): { category: ErrorLog['category']; severity: ErrorLog['severity'] } => {
  ...
  // Erros de rede
  if (err.code === 'NETWORK_ERROR' || err.message?.includes('network') || err.message?.includes('timeout')) {
    return { category: 'network', severity: 'high'};

  }

  // Erros de negócio
  if (err.code === 'BUSINESS_ERROR' || err.message?.includes('business') || err.message?.includes('rule')) {
    return { category: 'business', severity: 'medium'};

  }

  // Erros críticos do sistema
  if (err.code === 'SYSTEM_ERROR' || err.message?.includes('system') || err.message?.includes('internal')) {
...
const getErrorSuggestions = (error: unknown, operation: string): string[] => {
...
export const handleProductsError = (error: unknown, operation: string, context?: Partial<ErrorContext>): ErrorResponse => {
  ...
  return {
    ...
    code: (error as any).code || errorLog.id,
    details: process.env.NODE_ENV === 'development' ? (error as any) : undefined,
    suggestions};
};

export const logProductsError = (error: unknown, context: ErrorContext): ErrorLog => {
