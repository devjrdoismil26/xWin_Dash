...
import { fetchProductInventory, fetchAllInventory, updateProductInventory, addStock, removeStock, adjustStock, fetchInventoryHistory, fetchInventoryMovements, fetchInventoryAlerts, createInventoryAlert, updateInventoryAlert, deleteInventoryAlert, fetchStockLevels, updateStockLevels, generateInventoryReport, exportInventoryData, validateInventoryData, calculateStockValue, getStockStatus } from '../services/productsInventoryService';
...
interface UseProductInventoryReturn {
  ...
  loadAllInventory: (filters?: string) => Promise<void>;
  ...
  loadHistory: (productId: string, filters?: string) => Promise<void>;
  loadMovements: (filters?: string) => Promise<void>;
  // Operações de alertas
  loadAlerts: (filters?: string) => Promise<void>;
  ...
  loadStockLevels: (filters?: string) => Promise<void>;
  updateStockLevels: (productId: string, levels: StockLevel) => Promise<ProductResponse>;
  // Operações de relatórios
  generateReport: (filters?: string) => Promise<ProductResponse>;
  exportData: (format: 'csv' | 'xlsx' | 'pdf', filters?: string) => Promise<ProductResponse>;
  // Utilitários
  validateData: (inventory: Partial<ProductInventory>) => { isValid: boolean;
  errors: string[];
};
...
export const useProductInventory = (): UseProductInventoryReturn => {
  ...
  const loadInventory = useCallback(async (productId: string) => {
    ...
    try {
      ...
      setError(getErrorMessage(err) || 'Erro inesperado ao carregar estoque');
  ...
  const loadAllInventory = useCallback(async (filters?: string) => {
    setLoading(true);
    ...
    try {
      ...
      setError(getErrorMessage(err) || 'Erro inesperado ao carregar estoque geral');
    ...
    try {
      ...
      if (response.success) {
        ...
        setAllInventory(prev => prev.map(inv => 
          inv.product_id === productId ? (response as any).data : inv
        ));
      ...
      const errorResponse = {
        success: false,
        error: getErrorMessage(err) + ' ao atualizar estoque'};
  ...
  const addStock = useCallback(async (productId: string, quantity: number, reason?: string): Promise<ProductResponse> => {
    ...
    try {
      ...
      const errorResponse = {
        success: false,
        error: getErrorMessage(err) + ' ao adicionar estoque'};
  ...
  const removeStock = useCallback(async (productId: string, quantity: number, reason?: string): Promise<ProductResponse> => {
    ...
    try {
      ...
      const errorResponse = {
        success: false,
        error: getErrorMessage(err) + ' ao remover estoque'};
  ...
  const adjustStock = useCallback(async (productId: string, newQuantity: number, reason?: string): Promise<ProductResponse> => {
    ...
    try {
      ...
      const errorResponse = {
        success: false,
        error: getErrorMessage(err) + ' ao ajustar estoque'};
  ...
  const loadHistory = useCallback(async (productId: string, filters?: string) => {
    setLoading(true);
    ...
    try {
      ...
      setError(getErrorMessage(err) || 'Erro inesperado ao carregar histórico');
  ...
  const loadMovements = useCallback(async (filters?: string) => {
    setLoading(true);
    ...
    try {
      ...
      setError(getErrorMessage(err) || 'Erro inesperado ao carregar movimentações');
  ...
  const loadAlerts = useCallback(async (filters?: string) => {
    setLoading(true);
    ...
    try {
      ...
      setError(getErrorMessage(err) || 'Erro inesperado ao carregar alertas');
  ...
  const createAlert = useCallback(async (productId: string, alert: InventoryAlert): Promise<ProductResponse> => {
    ...
    try {
      ...
      const errorResponse = {
        success: false,
        error: getErrorMessage(err) + ' ao criar alerta'};
  ...
  const updateAlert = useCallback(async (alertId: string, alert: Partial<InventoryAlert>): Promise<ProductResponse> => {
    ...
    try {
      ...
      const errorResponse = {
        success: false,
        error: getErrorMessage(err) + ' ao atualizar alerta'};
  ...
  const deleteAlert = useCallback(async (alertId: string): Promise<ProductResponse> => {
    ...
    try {
      ...
      const errorResponse = {
        success: false,
        error: getErrorMessage(err) + ' ao deletar alerta'};
  ...
  const loadStockLevels = useCallback(async (filters?: string) => {
    setLoading(true);
    ...
    try {
      ...
      setError(getErrorMessage(err) || 'Erro inesperado ao carregar níveis de estoque');
  ...
  const updateStockLevels = useCallback(async (productId: string, levels: StockLevel): Promise<ProductResponse> => {
    ...
    try {
      ...
      const errorResponse = {
        success: false,
        error: getErrorMessage(err) + ' ao atualizar níveis de estoque'};
  ...
  const generateReport = useCallback(async (filters?: string): Promise<ProductResponse> => {
    setLoading(true);
  ...
  const exportData = useCallback(async (format: 'csv' | 'xlsx' | 'pdf', filters?: string): Promise<ProductResponse> => {
    setLoading(true);
    ...
    try {
      ...
      const errorResponse = {
        success: false,
        error: getErrorMessage(err) + ' ao exportar dados'};
