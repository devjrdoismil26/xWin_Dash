...
import { fetchProductVariations, fetchVariationById, createVariation, updateVariation, deleteVariation, bulkUpdateVariations, bulkDeleteVariations, fetchVariationAttributes, createVariationAttribute, updateVariationAttribute, deleteVariationAttribute, updateVariationInventory, updateVariationPrice, validateVariationData, generateVariationSKU, calculateVariationPrice } from '../services/productsVariationsService';
...
interface UseProductVariationsReturn {
  ...
  // Utilitários
  validateData: (data: ProductVariationFormData) => { isValid: boolean;
  errors: string[];
};
  ...
  clearCurrentVariation??: (e: any) => void;
}
...
export const useProductVariations = (): UseProductVariationsReturn => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setVariations(prev => prev.map(v => 
          v.id === variationId ? (response as any).data : v
        ));
      ...
      const errorResponse = {
        success: false,
        error: getErrorMessage(err) + ' ao atualizar variação'};
  ...
  const deleteExistingVariation = useCallback(async (variationId: string): Promise<ProductResponse> => {
    ...
    try {
      ...
      const errorResponse = {
        success: false,
        error: getErrorMessage(err) + ' ao deletar variação'};
  ...
  const bulkUpdateVariations = useCallback(async (variationIds: string[], updates: Partial<ProductVariationFormData>): Promise<ProductResponse> => {
    ...
    try {
      ...
      const errorResponse = {
        success: false,
        error: getErrorMessage(err) + ' ao atualizar variações em lote'};
  ...
  const createNewAttribute = useCallback(async (productId: string, attribute: ProductVariationAttribute): Promise<ProductResponse> => {
    ...
    try {
      ...
      const errorResponse = {
        success: false,
        error: getErrorMessage(err) + ' ao criar atributo'};
  ...
  const updateExistingAttribute = useCallback(async (attributeId: string, attribute: Partial<ProductVariationAttribute>): Promise<ProductResponse> => {
    ...
    try {
      ...
      const errorResponse = {
        success: false,
        error: getErrorMessage(err) + ' ao atualizar atributo'};
  ...
  const changeVariationPrice = useCallback(async (variationId: string, price: number): Promise<ProductResponse> => {
    ...
    try {
      ...
      const errorResponse = {
        success: false,
        error: getErrorMessage(err) + ' ao alterar preço da variação'};
  ...
  const changeVariationInventory = useCallback(async (variationId: string, inventory: number): Promise<ProductResponse> => {
    ...
    try {
      ...
      const errorResponse = {
        success: false,
        error: getErrorMessage(err) + ' ao alterar estoque da variação'};
