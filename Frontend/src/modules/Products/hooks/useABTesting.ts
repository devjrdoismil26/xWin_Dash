...
import { ABTest, ABTestVariant, ABTestResults, ABTestType, ABTestStatus, CreateABTestData, UpdateABTestData } from '../types/products';
...
interface UseABTestingReturn {
  ...
  getDetailedResults: (id: string) => Promise<Record<string, any>>;
  getConversionData: (id: string) => Promise<Record<string, any>>;
  ...
  // Recommendations
  getRecommendations: (id: string) => Promise<Record<string, any>>;
  getOptimizationSuggestions: (id: string) => Promise<Record<string, any>>;
  getNextTestIdeas: (targetId: string) => Promise<Record<string, any>>; }
...
export const useABTesting = (options: UseABTestingOptions = {}): UseABTestingReturn => {
  ...
  const fetchTests = useCallback(async () => {
    try {
      ...
      const data = await apiClient.get<ABTest[]>('/ab-tests');
  ...
  const createTest = useCallback(async (data: CreateABTestData): Promise<ABTest> => {
    try {
      ...
      const newTest = await apiClient.post<ABTest>('/ab-tests', data);
  ...
  const updateTest = useCallback(async (id: string, data: UpdateABTestData): Promise<ABTest> => {
    try {
      ...
      const updatedTest = await apiClient.put<ABTest>(`/ab-tests/${id}`, data);
  ...
  const duplicateTest = useCallback(async (id: string): Promise<ABTest> => {
    try {
      ...
      const duplicatedTest = await apiClient.post<ABTest>(`/ab-tests/${id}/duplicate`);
  ...
  const startTest = useCallback(async (id: string): Promise<ABTest> => {
    try {
      ...
      const startedTest = await apiClient.post<ABTest>(`/ab-tests/${id}/start`);
  ...
  const pauseTest = useCallback(async (id: string): Promise<ABTest> => {
    try {
      ...
      const pausedTest = await apiClient.post<ABTest>(`/ab-tests/${id}/pause`);
  ...
  const resumeTest = useCallback(async (id: string): Promise<ABTest> => {
    try {
      ...
      const resumedTest = await apiClient.post<ABTest>(`/ab-tests/${id}/resume`);
  ...
  const stopTest = useCallback(async (id: string): Promise<ABTest> => {
    try {
      ...
      const stoppedTest = await apiClient.post<ABTest>(`/ab-tests/${id}/stop`);
  ...
  const archiveTest = useCallback(async (id: string): Promise<ABTest> => {
    try {
      ...
      const archivedTest = await apiClient.post<ABTest>(`/ab-tests/${id}/archive`);
  ...
  const addVariant = useCallback(async (testId: string, variant: ABTestVariant): Promise<ABTest> => {
    try {
      ...
      const updatedTest = await apiClient.post<ABTest>(`/ab-tests/${testId}/variants`, { variant });
  ...
  const updateVariant = useCallback(async (testId: string, variantId: string, variant: ABTestVariant): Promise<ABTest> => {
    try {
      ...
      const updatedTest = await apiClient.put<ABTest>(`/ab-tests/${testId}/variants/${variantId}`, { variant });
  ...
  const removeVariant = useCallback(async (testId: string, variantId: string): Promise<ABTest> => {
    try {
      ...
      const updatedTest = await apiClient.delete<ABTest>(`/ab-tests/${testId}/variants/${variantId}`);
  ...
  const reorderVariants = useCallback(async (testId: string, variantIds: string[]): Promise<ABTest> => {
    try {
      ...
      const updatedTest = await apiClient.put<ABTest>(`/ab-tests/${testId}/variants/reorder`, { variantIds });
  ...
  const getResults = useCallback(async (id: string): Promise<ABTestResults> => {
    try {
      return await apiClient.get<ABTestResults>(`/ab-tests/${id}/results`);
  ...
  const getDetailedResults = useCallback(async (id: string): Promise<Record<string, any>> => {
    try {
      return await apiClient.get<Record<string, any>>(`/ab-tests/${id}/detailed-results`);
  ...
  const getConversionData = useCallback(async (id: string): Promise<Record<string, any>> => {
    try {
      return await apiClient.get<Record<string, any>>(`/ab-tests/${id}/conversion-data`);
  ...
  const getStatisticalSignificance = useCallback(async (id: string): Promise<number> => {
    try {
      const data = await apiClient.get<{ significance: number }>(`/ab-tests/${id}/statistical-significance`);

      return (data as any).significance;
    } catch (err) {
  ...
  const getWinner = useCallback(async (id: string): Promise<string | null> => {
    try {
      const data = await apiClient.get<{ winner: string | null }>(`/ab-tests/${id}/winner`);

      return (data as any).winner;
    } catch (err) {
  ...
  // Recommendations
  const getRecommendations = useCallback(async (id: string): Promise<Record<string, any>> => {
    try {
      return await apiClient.get<Record<string, any>>(`/ab-tests/${id}/recommendations`);
  ...
  const getOptimizationSuggestions = useCallback(async (id: string): Promise<Record<string, any>> => {
    try {
      return await apiClient.get<Record<string, any>>(`/ab-tests/${id}/optimization-suggestions`);
  ...
  const getNextTestIdeas = useCallback(async (targetId: string): Promise<Record<string, any>> => {
    try {
      return await apiClient.get<Record<string, any>>(`/ab-tests/next-test-ideas?targetId=${targetId}`);
