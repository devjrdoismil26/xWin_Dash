  ...
  const tests: TestCase[] = [
    {
      ...
      action: async () => {
        try {
          ...
          return { success: false, message: 'Falha na conexão', error: errorMessage};
        ...
        } },
    {
      ...
      action: async () => {
        try {
          ...
          const errorMessage = error instanceof Error ? getErrorMessage(error) : 'Erro desconhecido';
          return { success: false, message: 'Falha ao criar campanha', error: errorMessage};
        ...
        } },
    {
      ...
      action: async () => {
        try {
          ...
          const errorMessage = error instanceof Error ? getErrorMessage(error) : 'Erro desconhecido';
          return { success: false, message: 'Falha no gerenciamento', error: errorMessage};
        ...
        } },
    {
      ...
      action: async () => {
        try {
          ...
          const errorMessage = error instanceof Error ? getErrorMessage(error) : 'Erro desconhecido';
          return { success: false, message: 'Falha na performance', error: errorMessage};
        ...
        } },
    {
      ...
      action: async () => {
        try {
          ...
          const errorMessage = error instanceof Error ? getErrorMessage(error) : 'Erro desconhecido';
          return { success: false, message: 'Falha nos analytics', error: errorMessage};
  ...
  const statsConfig = {
    stats: [
      ...
      },
      {
        ...
        value: campaigns?.reduce((sum: unknown, c: unknown) => sum + c.budget, 0) || 0,
        color: 'purple'
      },
      {
        label: 'ROI Médio',
