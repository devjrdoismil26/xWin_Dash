 ...
 * @module modules/ADStool/types/adsCampaignTypes
 * @description
 * Definições de tipos e interfaces para campanhas de anúncios, incluindo
 * estruturas de dados, status, objetivos, targeting, orçamentos, agendamentos,
 * lances, criativos, audiências, placements, dispositivos, localizações,
 * idiomas, idade, gênero, interesses, comportamentos, palavras-chave,
 * grupos de anúncios, extensões, conversões, remarketing, frequência,
 * entrega, rotação, otimização, experimentos, rascunhos, templates e performance.
 * @since 1.0.0
 */
...
/**
 * Interface AdsCampaign - Campanha de Anúncios
 * @interface AdsCampaign
 * @description
 * Representa uma campanha de anúncios no sistema, contendo informações
 * sobre a conta associada, plataforma, status, orçamento, datas, objetivo e targeting.
 */
...
/**
 * Tipo AdsCampaignStatus - Status de Campanha
 * @typedef {('active' | 'paused' | 'deleted' | 'pending')} AdsCampaignStatus
 * @description
 * Tipo union que representa os possíveis status de uma campanha de anúncios.
 */
...
/**
 * Tipo AdsObjective - Objetivos de Campanha
 * @typedef {('awareness' | 'traffic' | 'engagement' | 'leads' | 'sales' | 'app_installs' | 'video_views' | 'conversions' | 'catalog_sales' | 'store_visits')} AdsObjective
 * @description
 * Tipo union que representa os possíveis objetivos de uma campanha de anúncios.
 */
