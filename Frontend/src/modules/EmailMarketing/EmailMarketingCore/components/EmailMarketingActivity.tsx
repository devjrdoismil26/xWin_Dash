...
import { Activity, Mail, Users, FileText, BarChart3, Clock, CheckCircle, AlertCircle, Info, TrendingUp, TrendingDown, Eye, MousePointer, Target } from 'lucide-react';
...
export const EmailMarketingActivity: React.FC<EmailMarketingActivityProps> = ({ activities,
  loading = false,
  className
   }) => {
  ...
  if (loading) { return (
      ...
      <Card.Content className="p-8 text-center" />
          <div className="Carregando atividades...">$2</div>
          </div>
        </Card.Content>
      </Card>);
  ...
  if (!activities || activities.length === 0) { return (
      ...
      <Card.Header />
        ...
        <Card.Content className="p-8 text-center" />
          <div className="Nenhuma atividade recente encontrada">$2</div>
          </div>
        </Card.Content>
      </Card>);
  ...
  return (
      ...
      <Card.Header />
        <div className=" ">$2</div><Card.Title className="text-gray-900 dark:text-white flex items-center gap-2" />
            <Activity className="h-5 w-5 text-blue-600" />
            Atividades Recentes
          </Card.Title>
          <Button
      ...
      <Card.Content />
        <div className="{activities.slice(0, 10).map((activity: unknown, index: unknown) => {">$2</div>
      ...
      <div
                ...
                <div className=" ">$2</div><div className=" ">$2</div><h4 className="font-semibold text-gray-900 dark:text-white text-sm" />
                      {activity.action}
                    </h4>
                    <Badge 
                      variant="outline" 
                      ...
                      {activity.type}
                    </Badge></div><p className="text-sm text-gray-600 dark:text-gray-300 mb-2" />
                  ...
                  <div className=" ">$2</div><div className=" ">$2</div><Clock className="h-3 w-3" />
                    ...
                    {activity.user_name && (
                      <div className=" ">$2</div><Users className="h-3 w-3" />
                        <span>{activity.user_name}</span>
      </div>
    </>
  )}
                    {activity.count && (
                      <div className=" ">$2</div><Target className="h-3 w-3" />
        ...
        </div><activityStatus.icon className={cn(
                    "h-3 w-3",
