...
const EmailSubscriberTable: React.FC<{subscribers?: string, pagination?: string, onEdit, onDelete, onPageChange}> = ({ subscribers = [] as unknown[], pagination = null, onEdit, onDelete, onPageChange    }) => {
  return (
          ...
          <tbody />
            {(subscribers || []).map((subscriber: unknown) => (
              <tr key={subscriber.id} className="border-b hover:bg-gray-50" />
                ...
                <td className="py-2 px-3" />
                  <div className=" ">$2</div><Button variant="outline" size="sm" onClick={ () => onEdit?.(subscriber) }>Editar</Button>
                    <Button variant="destructive" size="sm" onClick={ () => onDelete?.(subscriber) }>Excluir</Button></div></td>
      </tr>
    </>
  ))}
          </tbody></table></div>
