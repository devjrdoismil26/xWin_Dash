...
import { SegmentBuilder, SegmentCriteria, SegmentGroup, SegmentValidationError, UseSegmentBuilderReturn } from '../types';

export const useSegmentBuilder = (initialSegment?: Partial<EmailSegment>): UseSegmentBuilderReturn => {
  // Estado do builder
  ...
  const updateSegment = useCallback((updates: Partial<EmailSegment>) => {
    setBuilder(prev => ({
      ...prev,
      segment: {
        ...prev.segment,
  ...
  const validateSegment = useCallback((): { isValid: boolean; errors: SegmentValidationError[] } => {
    ...
    builder.criteria.forEach((criteria: unknown, index: unknown) => {
