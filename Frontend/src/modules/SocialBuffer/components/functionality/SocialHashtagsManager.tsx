...
interface SocialHashtagsManagerProps {
  className?: string;
  onHashtagCreated??: (e: any) => void;
  onHashtagUpdated??: (e: any) => void;
  onHashtagDeleted??: (e: any) => void;
  children?: React.ReactNode;
  style?: React.CSSProperties;
  onClick?: (e: any) => void;
  onChange?: (e: any) => void; }

const SocialHashtagsManager: React.FC<SocialHashtagsManagerProps> = ({ className = '',
  onHashtagCreated,
  onHashtagUpdated,
  onHashtagDeleted
   }) => {
  ...
  const trendingHashtags = useMemo(() => {
    return hashtags
      ...
      .sort((a: unknown, b: unknown) => b.trendingScore - a.trendingScore)
      .slice(0, 10);
  ...
  return (
      ...
      <div className={`social-hashtags-manager ${className} `}>
        ...
        <div className=" ">$2</div><Card className="p-4" />
            ...
            <div className="text-sm text-gray-600 dark:text-gray-400">Ativas</div></Card><Card className="p-4" />
            <div className="text-2xl font-bold text-orange-600">{hashtagStats.trending}</div>
            <div className="text-sm text-gray-600 dark:text-gray-400">Em Alta</div></Card><Card className="p-4" />
            <div className="text-2xl font-bold text-purple-600">{hashtagStats.categories}</div>
            <div className="text-sm text-gray-600 dark:text-gray-400">Categorias</div></Card></div>
        ...
        {trendingHashtags.length > 0 && (
          <Card className="p-6 mb-6" />
            ...
            <div className="{(trendingHashtags || []).map((hashtag: unknown) => (">$2</div>
                <span
                  key={ hashtag.id }
                  className="px-3 py-1 bg-orange-100 text-orange-800 rounded-full text-sm font-medium">
          #
        </span>{hashtag.tag} ({hashtag.trendingScore}/10)
                </span>
              ))}
            </div>
      </Card>
        ...
        <Card className="p-4 mb-6" />
          <div className=" ">$2</div><div className=" ">$2</div><input
              ...
              <select
                ...
                <option value="trending">Em Alta</option></select></div>
        </Card>

        {/* Bulk Actions */}
        {selectedHashtags.length > 0 && (
          <Card className="p-4 mb-6 bg-blue-50 dark:bg-blue-900/20" />
            <div className=" ">$2</div><span className="{selectedHashtags.length} hashtag(s) selecionada(s)">$2</span>
              </span>
        ...
        {/* Hashtags List */}
        <div className="{(filteredHashtags || []).map((hashtag: unknown) => (">$2</div>
            <Card key={hashtag.id} className="p-6" />
              <div className=" ">$2</div><div className=" ">$2</div><div className=" ">$2</div><input
                    ...
                    <span className="#{hashtag.tag}">$2</span>
                    </span>
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                      hashtag.isActive ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'
                    } `}>
           
        </span>{hashtag.isActive ? 'Ativa' : 'Inativa'}
                    </span>
                    {hashtag.trendingScore > 7 && (
                      <span className="Em Alta">$2</span>
      </span>
    </>
  )}
                  </div>
                  
                  <div className=" ">$2</div><span>Categoria: {hashtag.category}</span>
                  ...
                  {hashtag.lastUsed && (
                    <div className="Último uso: {hashtag.lastUsed.toLocaleDateString()}">$2</div>
    </div>
  )}
                </div>
                ...
                <div className=" ">$2</div><Button
                    ...
                    className={hashtag.isActive ? "bg-yellow-600 hover:bg-yellow-700" : "bg-green-600 hover:bg-green-700"} size="sm"
                  >
