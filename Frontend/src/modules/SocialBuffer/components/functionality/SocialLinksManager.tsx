...
interface SocialLinksManagerProps {
  className?: string;
  onLinkCreated??: (e: any) => void;
  onLinkUpdated??: (e: any) => void;
  onLinkDeleted??: (e: any) => void;
...
const SocialLinksManager: React.FC<SocialLinksManagerProps> = ({ className = '',
  ...
  const linkStats = useMemo(() => {
    return {
      ...
      totalClicks: links.reduce((sum: unknown, link: unknown) => sum + link.clickCount, 0),
      topPerformer: links.reduce((top: unknown, link: unknown) => 
  ...
  const topLinks = useMemo(() => {
    return links
      ...
      .sort((a: unknown, b: unknown) => b.clickCount - a.clickCount)
      .slice(0, 5);
  ...
  return (
      ...
      <div className={`social-links-manager ${className} `}>
        ...
        <div className=" ">$2</div><Card className="p-4" />
            ...
            <div className="text-sm text-gray-600 dark:text-gray-400">Total de Cliques</div></Card><Card className="p-4" />
            <div className="text-2xl font-bold text-orange-600">{linkStats.topPerformer.clickCount}</div>
            <div className="text-sm text-gray-600 dark:text-gray-400">Top: {linkStats.topPerformer.title}</div></Card></div>
        ...
        {topLinks.length > 0 && (
          <Card className="p-6 mb-6" />
            ...
            <div className="{(topLinks || []).map((link: unknown, index: unknown) => (">$2</div>
        ...
        </div><div className="font-medium text-gray-900 dark:text-white">{link.title}</div>
                      <div className="text-sm text-gray-600 dark:text-gray-400">{link.shortUrl}</div></div><div className=" ">$2</div><div className="text-lg font-bold text-purple-600">{link.clickCount}</div>
        ...
        <Card className="p-4 mb-6" />
          <div className=" ">$2</div><div className=" ">$2</div><input
              ...
              <select
                ...
                <option value="popular">Populares</option></select></div>
        </Card>

        {/* Bulk Actions */}
        {selectedLinks.length > 0 && (
          <Card className="p-4 mb-6 bg-blue-50 dark:bg-blue-900/20" />
            <div className=" ">$2</div><span className="{selectedLinks.length} link(s) selecionado(s)">$2</span>
              </span>
        ...
        {/* Links List */}
        <div className="{(filteredLinks || []).map((link: unknown) => (">$2</div>
            <Card key={link.id} className="p-6" />
              <div className=" ">$2</div><div className=" ">$2</div><div className=" ">$2</div><input
                    ...
                    <span className="{link.title}">$2</span>
                    </span>
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                      link.isActive ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'
                    } `}>
           
        </span>{link.isActive ? 'Ativo' : 'Inativo'}
                    </span></div><div className=" ">$2</div><div className=" ">$2</div><strong>Original:</strong> {link.originalUrl}
                    </div>
                    <div className=" ">$2</div><strong>Encurtado:</strong> {link.shortUrl}
                    </div>
                  ...
                  <div className=" ">$2</div><span>Cliques: {link.clickCount}</span>
                  ...
                  {link.lastClicked && (
                    <div className="Último clique: {link.lastClicked.toLocaleDateString()}">$2</div>
    </div>
  )}
                </div>
                ...
                <div className=" ">$2</div><Button
                  ...
                  <Button
                    ...
                    className={link.isActive ? "bg-yellow-600 hover:bg-yellow-700" : "bg-green-600 hover:bg-green-700"} size="sm"
                  >
