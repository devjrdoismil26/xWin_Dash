...
import { CheckCircle, Send, Calendar, Hash, Link as Link, Image, BarChart3, Heart, Users, Plus, FileText, Upload, Share2, Zap, TrendingUp, Award, Star } from 'lucide-react';
...
const SocialBufferSuccessState: React.FC<SuccessStateProps> = ({ type = 'post',
  title,
  message,
  actionLabel,
  onAction,
  ...
  return (
      ...
      <div className={`flex items-center justify-center min-h-[400px] ${className} `}>
        ...
        <Card className={`p-8 max-w-md w-full text-center ${successConfig.bgColor} ${successConfig.borderColor} border-2`} />
          <div className="{/* Ícone com animação */}">$2</div>
            <div className={`flex justify-center ${successConfig.color} `}>
           
        </div><Animated type="bounce" />
                {successConfig.icon}
              </Animated>
            </div>
        ...
        </div><h2 className={`text-xl font-semibold ${successConfig.color} mb-2`} />
                {finalTitle}
              </h2>
            ...
            { stats.length > 0 && (
              <div className="{(stats || []).map((stat: unknown, index: unknown) => (">$2</div>
                  <Animated key={index } />
                    <div className=" ">$2</div><div className="{ stat.icon && (">$2</div>
...
export const PostSuccessState: React.FC<Omit<SuccessStateProps, 'type'>> = (props: unknown) => (
  <SocialBufferSuccessState {...props} type="post" / />);

export const ScheduleSuccessState: React.FC<Omit<SuccessStateProps, 'type'>> = (props: unknown) => (
  <SocialBufferSuccessState {...props} type="schedule" / />);

export const HashtagSuccessState: React.FC<Omit<SuccessStateProps, 'type'>> = (props: unknown) => (
  <SocialBufferSuccessState {...props} type="hashtag" / />);

export const LinkSuccessState: React.FC<Omit<SuccessStateProps, 'type'>> = (props: unknown) => (
  <SocialBufferSuccessState {...props} type="link" / />);

export const MediaSuccessState: React.FC<Omit<SuccessStateProps, 'type'>> = (props: unknown) => (
  <SocialBufferSuccessState {...props} type="media" / />);

export const AnalyticsSuccessState: React.FC<Omit<SuccessStateProps, 'type'>> = (props: unknown) => (
  <SocialBufferSuccessState {...props} type="analytics" / />);

export const EngagementSuccessState: React.FC<Omit<SuccessStateProps, 'type'>> = (props: unknown) => (
  <SocialBufferSuccessState {...props} type="engagement" / />);

export const AccountSuccessState: React.FC<Omit<SuccessStateProps, 'type'>> = (props: unknown) => (
  <SocialBufferSuccessState {...props} type="account" / />);

export const BulkSuccessState: React.FC<Omit<SuccessStateProps, 'type'>> = (props: unknown) => (
  <SocialBufferSuccessState {...props} type="bulk" / />);

export const ImportSuccessState: React.FC<Omit<SuccessStateProps, 'type'>> = (props: unknown) => (
  <SocialBufferSuccessState {...props} type="import" / />);

export const ExportSuccessState: React.FC<Omit<SuccessStateProps, 'type'>> = (props: unknown) => (
