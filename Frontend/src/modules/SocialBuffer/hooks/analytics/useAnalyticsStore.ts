/**
 * Hook useAnalyticsStore - Store Orquestrador de Analytics
 *
 * @description
 * Store orquestrador que consolida todos os stores especializados de analytics:
 * métricas, hashtags e relatórios. Fornece interface unificada e re-exporta tipos.
 *
 * @module modules/SocialBuffer/hooks/analytics/useAnalyticsStore
 * @since 1.0.0
 */
 ...
 *
 * @description
 * Re-exporta todos os tipos TypeScript dos stores especializados de analytics.
 */
export type {
  AnalyticsParams,
  BasicMetrics,
...
export const useAnalyticsStore = () => {
  ...
  const fetchBasicMetrics = async (params?: string) => {
    return metricsStore.fetchBasicMetrics(params);};

  const fetchPlatformMetrics = async (params?: string) => {
    return metricsStore.fetchPlatformMetrics(params);};

  const fetchTimeSeriesMetrics = async (params?: string) => {
    return metricsStore.fetchTimeSeriesMetrics(params);};

  const fetchContentMetrics = async (params?: string) => {
    return metricsStore.fetchContentMetrics(params);};

  // ===== MÉTRICAS DE HASHTAGS =====

  const fetchHashtagMetrics = async (params?: string) => {
  ...
  const comparePeriods = async (currentPeriod: unknown, previousPeriod: unknown, params?: string) => {
    return reportsStore.comparePeriods(currentPeriod, previousPeriod, params);};

  // ===== FILTROS =====

  const setFilters = (filters: unknown) => {
