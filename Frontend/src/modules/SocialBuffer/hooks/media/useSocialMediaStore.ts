/**
 * Hook useSocialMediaStore - Store de Mídia Social
 *
 * @description
 * Store Zustand para gerenciamento de estado de mídia social do SocialBuffer.
 * Gerencia CRUD de mídia, upload, processamento, otimização, análise e galeria.
 *
 * @module modules/SocialBuffer/hooks/media/useSocialMediaStore
 * @since 1.0.0
 */
...
/**
 * Estado de mídia social
 *
 * @interface SocialMediaState
 * @property {SocialMedia[]} media - Lista de mídia
 * @property {SocialMedia | null} selectedMedia - Mídia selecionada
 * @property {MediaStats | null} mediaStats - Estatísticas de mídia
 * @property {Object} pagination - Dados de paginação
 * @property {MediaSearchParams} filters - Filtros de busca
 * @property {boolean} loading - Se está carregando
 * @property {string | null} error - Mensagem de erro
 * @property {boolean} uploading - Se está fazendo upload
 ...
 * @property {Record<string, number>} uploadProgress - Progresso de upload por arquivo
 * @property {Array} uploadQueue - Fila de uploads
 */
interface SocialMediaState {
  // Estado da mídia
  media: SocialMedia[];
...
interface SocialMediaActions {
  ...
  selectMultipleMedia?: (e: any) => void;
  toggleMediaSelection?: (e: any) => void;
  clearMultipleSelection??: (e: any) => void;
  // Ações de upload
  uploadMedia: (file: File, metadata?: string) => Promise<SocialMedia>;
  bulkUploadMedia: (files: File[], metadata?: string) => Promise<SocialMedia[]>;
  retryUpload: (uploadId: string) => Promise<void>;
  cancelUpload?: (e: any) => void;
  // Ações de CRUD
  updateMedia: (id: string, mediaData: UpdateMediaData) => Promise<SocialMedia>;
  deleteMedia: (id: string) => Promise<void>;
  duplicateMedia: (id: string) => Promise<SocialMedia>;
  // Ações de otimização
  optimizeMedia: (mediaId: string, options?: string) => Promise<void>;
  ...
  devtools(
    persist(
      (set: unknown, get: unknown) => ({
        ...
        uploadMedia: async (file: File, metadata?: string) => {
        ...
        bulkUploadMedia: async (files: File[], metadata?: string) => {
          set({ uploading: true, error: null });
        ...
        optimizeMedia: async (mediaId: string, options?: string) => {
