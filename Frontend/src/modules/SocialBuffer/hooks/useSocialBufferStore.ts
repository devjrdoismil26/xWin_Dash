/**
 * Store Orquestrador do SocialBuffer
 *
 * @description
 * Store Zustand principal que orquestra todos os stores especializados do SocialBuffer:
 * accounts, posts, schedules, analytics, hashtags, links, media e engagement.
 * Fornece estado global e operações em massa.
 *
 * @module modules/SocialBuffer/hooks/useSocialBufferStore
 * @since 1.0.0
 */
...
interface SocialBufferActions {
  ...
  setAutoSync?: (e: any) => void;
  // Ações de cache
  clearAllCache??: (e: any) => void;
  invalidateAllCache?: (e: any) => void;
  refreshAllCache: () => Promise<void>;
  // Ações de operações em lote
  startBulkOperation?: (e: any) => void;
  updateBulkProgress?: (e: any) => void;
  completeBulkOperation??: (e: any) => void;
  cancelBulkOperation??: (e: any) => void;
  // Ações de estado global
  setGlobalLoading?: (e: any) => void;
  setGlobalError?: (e: any) => void;
  clearGlobalError??: (e: any) => void;
        ...
        immer((set: unknown, get: unknown) => ({
          ...
          initialize: async () => {
            ...
            try {
              ...
              set(state => {
                state.globalError = getErrorMessage(error);
          ...
          syncStore: async (storeName: string) => {
            ...
            try {
              ...
              set(state => {
                state.globalLoading = false;
                state.globalError = error instanceof Error ? (error as any).message : `Erro ao sincronizar ${storeName}`;
              });
            ...
            try {
              ...
              const updatedPosts = posts.map(post => {
                ...
                return {
                  ...
                  total_clicks: postLinks.reduce((sum: unknown, link: unknown) => sum + link.clicks, 0)};
          ...
          getCombinedStats: () => {
            ...
            return {
              accounts: {
                ...
                by_platform: accountsStore.accounts.reduce((acc: unknown, account: unknown) => {
              ...
              links: {
                ...
                total_clicks: linksStore.links.reduce((sum: unknown, link: unknown) => sum + link.clicks, 0),
                total_unique_clicks: linksStore.links.reduce((sum: unknown, link: unknown) => sum + link.unique_clicks, 0)
  },
              media: {
                total: mediaStore.media.length,
                by_type: mediaStore.media.reduce((acc: unknown, media: unknown) => {
