/**
 * Serviço de Analytics do SocialBuffer
 *
 * @description
 * Serviço responsável por todas as operações relacionadas a analytics e métricas.
 * Gerencia métricas básicas, por plataforma, por post, relatórios e cache.
 *
 * @module modules/SocialBuffer/services/analyticsService
 * @since 1.0.0
 */

import { apiClient } from '@/services';
import { SocialAccount, SocialPost, SocialSchedule, SocialLink } from '../types/socialTypes';

/**
 * Cache para analytics
 *
 * @description
 * Cache em memória para analytics com TTL de 5 minutos.
 */
...
class AnalyticsService {
  ...
  async getBasicMetrics(params: AnalyticsParams = {}): Promise<BasicMetrics> {
    try {
      ...
      // Cache do resultado
      analyticsCache.set(cacheKey, { data: (response as any).data, timestamp: Date.now() });
