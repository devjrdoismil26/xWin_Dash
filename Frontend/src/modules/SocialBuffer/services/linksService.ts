/**
 * Serviço de Links do SocialBuffer
 *
 * @description
 * Serviço responsável por todas as operações relacionadas a links encurtados.
 * Gerencia CRUD de links, rastreamento, estatísticas, validação e cache.
 *
 * @module modules/SocialBuffer/services/linksService
 * @since 1.0.0
 */

import { apiClient } from '@/services';
import { SocialLink, SocialLinkType } from '../types/socialTypes';

/**
 * Cache para links
 *
 * @description
 * Cache em memória para links com TTL de 5 minutos.
 */
...
class LinksService {
  ...
  async getLinkById(id: string): Promise<ShortenedLink> {
    try {
      ...
      // Cache do resultado
      linksCache.set(cacheKey, { data: (response as any).data, timestamp: Date.now() });
  ...
  async getExpiredLinks(): Promise<ShortenedLink[]> {
    try {
      ...
      return ((result.data || []) as unknown[]).filter(link => link.expires_at && new Date(link.expires_at) < new Date());
  ...
  async getLinksExpiringSoon(days: number = 7): Promise<ShortenedLink[]> {
    try {
      ...
      return ((result.data || []) as unknown[]).filter(link => 
        link.expires_at && 
  ...
  private validateLinkData(data: CreateLinkData): void {
    if (!data.original_url || (data as any).original_url.trim().length === 0) {
