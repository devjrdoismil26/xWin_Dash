/**
 * Serviço de Mídia do SocialBuffer
 *
 * @description
 * Serviço responsável por todas as operações relacionadas a mídia social.
 * Gerencia CRUD de mídia, upload, processamento, estatísticas e cache.
 *
 * @module modules/SocialBuffer/services/mediaService
 * @since 1.0.0
 */

import { apiClient } from '@/services';
import { SocialMedia, SocialMediaType } from '../types/socialTypes';

/**
 * Cache para mídia
 *
 * @description
 * Cache em memória para mídia com TTL de 10 minutos.
 */
const mediaCache = new Map<string, { data: unknown; timestamp: number }>();
...
class MediaService {
  ...
  async getMediaById(id: string): Promise<SocialMedia> {
    try {
      ...
      // Cache do resultado
      mediaCache.set(cacheKey, { data: (response as any).data, timestamp: Date.now() });
  ...
  async uploadMedia(data: UploadMediaData): Promise<SocialMedia> {
    try {
      ...
      formData.append('is_public', (data as any).is_public.toString());

      if (data.title) formData.append('title', (data as any).title);

      if (data.description) formData.append('description', (data as any).description);
  ...
  async getPublicMedia(): Promise<SocialMedia[]> {
    try {
      ...
      return ((result.data || []) as unknown[]).filter(media => media.is_public);
