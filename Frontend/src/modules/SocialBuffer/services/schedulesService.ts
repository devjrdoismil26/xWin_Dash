/**
 * Serviço de Agendamentos do SocialBuffer
 *
 * @description
 * Serviço responsável por todas as operações relacionadas a agendamentos de posts.
 * Gerencia CRUD de agendamentos, sugestões de horários ideais, validação e cache.
 *
 * @module modules/SocialBuffer/services/schedulesService
 * @since 1.0.0
 */

import { apiClient } from '@/services';
import { SocialSchedule, SocialPlatform, SocialScheduleFrequency, SocialPost } from '../types/socialTypes';

/**
 * Cache para agendamentos
 *
 * @description
 * Cache em memória para agendamentos com TTL de 3 minutos.
 */
const schedulesCache = new Map<string, { data: unknown; timestamp: number }>();
...
class SchedulesService {
  ...
  async getScheduleById(id: number): Promise<SocialSchedule> {
    try {
      ...
      // Cache do resultado
      schedulesCache.set(cacheKey, { data: (response as any).data, timestamp: Date.now() });
  ...
  async executeSchedule(id: number): Promise<SocialPost[]> {
    try {
      ...
      return (response as any).data.posts || [];
    } catch (error) {
  ...
  async getCalendarEvents(accountId?: number, dateFrom?: string, dateTo?: string): Promise<CalendarEvent[]> {
    try {
      ...
      return (response as any).data.events || [];
    } catch (error) {
  ...
  private validateScheduleData(data: CreateScheduleData): void {
    ...
    if (!data.content_template || (data as any).content_template.trim().length === 0) {
      throw new Error('Template de conteúdo é obrigatório');

    }

    if (!data.scheduled_times || (data as any).scheduled_times.length === 0) {
