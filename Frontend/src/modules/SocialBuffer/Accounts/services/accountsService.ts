...
class AccountsService {
  ...
  async createSocialAccount(data: SocialAccountFormData): Promise<SocialAccountResponse> {
    ...
    }, 'createSocialAccount', { accountId: (data as any).username });
   ...
   */
  async connectAccount(platform: string, authData: unknown): Promise<SocialAccountResponse> {
    return withErrorHandling(async () => {
   ...
   */
  async getAccountInsights(id: string, params: Record<string, any> = {}): Promise<SocialAccountResponse> {
  ...
  getDefaultAccountSettings(platform: string): Partial<SocialAccountFormData> {
    ...
    const validPlatforms: SocialAccountFormData['platform'][] = ['facebook', 'twitter', 'instagram', 'linkedin', 'youtube', 'tiktok', 'pinterest'];
    const platformType = validPlatforms.includes(platform as SocialAccountFormData['platform']) 
      ? (platform as SocialAccountFormData['platform'])
      : 'facebook'; // fallback
