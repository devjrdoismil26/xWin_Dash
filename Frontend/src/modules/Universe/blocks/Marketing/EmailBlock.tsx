...
import { Mail, Users, Send, BarChart3, Settings, Play, Pause, Eye, Target, TrendingUp, Clock, CheckCircle, AlertCircle } from 'lucide-react';
...
const EmailBlock: React.FC<EmailBlockProps> = ({ data, 
  isConnectable = true,
  isSelected = false,
  onUpdate,
  onDelete,
  ...
  return (
     ...
     >
          {/* Header Icon */}
      <div className=" ">$2</div><Mail className="w-4 h-4" />
      </div>
      {/* Campaign Info */}
      <div className=" ">$2</div><div className=" ">$2</div><h4 className="font-medium text-sm text-gray-800 truncate" />
            {campaign.name}
          </h4>
          <Badge className={`text-xs ${getStatusColor(campaign.status)} `} />
            {getStatusIcon(campaign.status)}
            <span className="ml-1 capitalize">{campaign.status}</span></Badge></div>
      ...
      {campaign.status === 'sending' && (
        <div className=" ">$2</div><div className=" ">$2</div><span className="text-gray-600">Sending...</span>
            <span className="{formatNumber(campaign.sent)} / {formatNumber(campaign.recipients)}">$2</span>
            </span></div><Progress 
      ...
      <div className=" ">$2</div><div className=" ">$2</div><div className=" ">$2</div><Users className="w-3 h-3 text-blue-600" />
            <span className="text-xs text-blue-600 font-medium">Recipients</span></div><p className="text-sm font-bold text-blue-800" />
            {formatNumber(campaign.recipients)}
          </p></div><div className=" ">$2</div><div className=" ">$2</div><Eye className="w-3 h-3 text-green-600" />
            <span className="text-xs text-green-600 font-medium">Opened</span></div><p className="text-sm font-bold text-green-800" />
            {formatNumber(campaign.opened)}
          </p>
          <p className="text-xs text-green-600" />
            {formatPercentage(metrics.openRate)}
          </p></div><div className=" ">$2</div><div className=" ">$2</div><Target className="w-3 h-3 text-purple-600" />
            <span className="text-xs text-purple-600 font-medium">Clicked</span></div><p className="text-sm font-bold text-purple-800" />
          ...
          <p className="text-xs text-purple-600" />
            ...
            <span className="text-xs text-orange-600 font-medium">Sent</span></div><p className="text-sm font-bold text-orange-800" />
            {formatNumber(campaign.sent)}
          </p>
        </div>
      {/* Actions */}
      <div className="{campaign.status === 'draft' && (">$2</div>
          <Button 
            size="sm" 
            onClick={ handleSend }
      ...
      {campaign.scheduledAt && campaign.status === 'scheduled' && (
        <div className="Scheduled for {new Date(campaign.scheduledAt).toLocaleString()}">$2</div>
    </div>
  )}
    </BaseBlock>);};
