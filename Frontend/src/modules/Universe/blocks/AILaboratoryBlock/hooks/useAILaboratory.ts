...
import { AIGenerationRequest, AIGenerationResult, PyLabConnectionStatus, PyLabCapabilities, PyLabSystemStatus, AILaboratoryBlockData, AILaboratoryError, DEFAULT_AI_CONFIG, DEFAULT_GENERATION_CONFIG } from '../types';
        ...
        setGenerations(prev => prev.map(gen => 
          gen.id === taskId 
            ? { 
                ...
                metadata: { ...gen.metadata, message: progress.message } : gen
        ));
    ...
    return {
      ...
      averageTime: generations.length > 0 ? 
        generations
          ...
          .reduce((acc: unknown, g: unknown) => acc + (g.generation_time || 0), 0) / generations.length : 0,
