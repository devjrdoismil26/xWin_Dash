...
import { X, Send, Brain, Lightbulb, Wand2, Settings, Copy, ThumbsUp, ThumbsDown, Sparkles, Zap, Target, TrendingUp, MessageSquare, Mic, MicOff, Volume2, VolumeX } from 'lucide-react';
...
interface UniverseAIChatProps {
  ...
  onNodeAction??: (e: any) => void;
  className?: string;
  children?: React.ReactNode;
  style?: React.CSSProperties;
  onClick?: (e: any) => void;
  onChange?: (e: any) => void; }
const UniverseAIChat: React.FC<UniverseAIChatProps> = ({ onClose, 
  selectedBlock,
  onNodeAction,
  className 
   }) => {
  ...
  return (
      ...
      <div className=" ">$2</div><div className=" ">$2</div><div className=" ">$2</div><div className=" ">$2</div><Brain className="w-4 h-4" /></div><div>
           
        </div><h2 className="font-semibold text-gray-800">AI Assistant</h2>
              <p className="text-xs text-gray-500">Universe workflow helper</p></div><Button size="sm" variant="ghost" onClick={onClose} className="p-1" />
      ...
      {/* Messages */}
      <div className="{(messages || []).map((message: unknown) => (">$2</div>
          <div key={message.id} className={`flex ${message.sender === 'user' ? 'justify-end' : 'justify-start'} `}>
        ...
        </div>{message.sender === 'ai' && (
                <div className=" ">$2</div><Brain className="w-3 h-3" />
              ...
              {message.suggestions && message.suggestions.length > 0 && (
                <div className=" ">$2</div><div className="text-xs font-medium text-gray-600 mb-1">Suggestions:</div>
                  {(message.suggestions || []).map((suggestion: unknown) => (
                    <button
                      key={ suggestion.id }
                    ...
                    >
                      <div className=" ">$2</div><Lightbulb className="w-3 h-3" />
                        <span className="font-medium">{suggestion.title}</span>
                        <Badge variant="secondary" className="text-xs ml-auto" />
                          {Math.round(suggestion.confidence * 100)}%
                        </Badge></div><p className="text-gray-600 mt-1">{suggestion.description}</p>
      </button>
    </>
  ))}
                </div>
              )}
              <div className=" ">$2</div><span className="{message.timestamp.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}">$2</span>
                </span>
                {message.sender === 'ai' && (
                  <div className=" ">$2</div><button className="p-1 hover:bg-gray-200 rounded" />
                      <ThumbsUp className="w-3 h-3" /></button><button className="p-1 hover:bg-gray-200 rounded" />
                      <ThumbsDown className="w-3 h-3" /></button><button className="p-1 hover:bg-gray-200 rounded" />
        ...
        {isTyping && (
          <div className=" ">$2</div><div className=" ">$2</div><div className=" ">$2</div><Brain className="w-3 h-3" />
                <span className="text-xs font-medium">AI Assistant is thinking...</span>
                <div className=" ">$2</div><div className=" ">$2</div><div className="w-1 h-1 bg-gray-400 rounded-full animate-bounce" style={animationDelay: '0.1s' } >
           
        </div><div className="w-1 h-1 bg-gray-400 rounded-full animate-bounce" style={animationDelay: '0.2s' } / /></div></div>
        )}
        <div ref={ messagesEndRef  }>
        </div>{/* Quick Actions */}
      <div className=" ">$2</div><div className="text-xs font-medium text-gray-600 mb-2">Quick Actions</div>
        <div className=" ">$2</div><Button size="sm" variant="outline" className="text-xs" onClick={() => handleSuggestionClick({
            id: 'optimize',
            type: 'optimization',
      ...
      <div className=" ">$2</div><div className=" ">$2</div><Input
        ...
        <div className=" ">$2</div><div className=" ">$2</div><Button 
            ...
            </Button></div><div className="Press Enter to send, Shift+Enter for new line">$2</div>
          </div>
      </div>);};
