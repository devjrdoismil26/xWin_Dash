...
import { ReactFlow, Background, Controls, MiniMap, ConnectionMode, addEdge, useNodesState, useEdgesState, Node, Edge, OnConnect, ReactFlowProvider, Panel, NodeOrigin, ReactFlowInstance,  } from 'reactflow';
...
interface UniverseCanvasProps {
  ...
  onBlocksChange?: (e: any) => void;
  onConnectionsChange?: (e: any) => void;
  selectedBlock?: UniverseBlock | null;
  onBlockSelect??: (e: any) => void;
  isRunning?: boolean;
...
const UniverseCanvas: React.FC<UniverseCanvasProps> = ({ blocks,
  connections,
  onBlocksChange,
  onConnectionsChange,
  selectedBlock,
  ...
  useEffect(() => {
    const reactFlowNodes: Node[] = (blocks || []).map(block => ({
      ...
      data: {
        ...block.data,
        onUpdate: (data: unknown) => onBlockUpdate?.(block.id, data),
  ...
  const onConnect: OnConnect = useCallback(
    (params: unknown) => {
      ...
      setEdges((eds: unknown) => addEdge(newEdge, eds));
  ...
  return (
        <>
      <div 
      className={cn("h-full w-full", className)} ref={ reactFlowWrapper }
      onDrop={ onDrop }
      onDragOver={ onDragOver  }>
       ...
       >
        ...
        <Panel position="top-left" />
          <div className=" ">$2</div><div className="text-xs font-medium text-gray-600 mb-1">Canvas Controls</div>
        ...
        <Panel position="bottom-right" />
          <div className=" ">$2</div><div className="font-medium text-gray-600">Canvas Stats</div>
