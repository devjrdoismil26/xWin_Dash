...
import { ProjectCore, ProjectCoreFilters, ProjectCoreMetrics, ProjectCoreActivity, ProjectCoreSearchResult, ProjectStatus } from '../types/projectsCoreTypes';
...
export const useProjectsCore = (): UseProjectsCoreReturn => {
  ...
  const recentProjects = useMemo(() => 
    [...projects].sort((a: unknown, b: unknown) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime()).slice(0, 5),
    [projects]);
