...
import { ActivityLog, ActivityLogFilters, ActivityLogStats, ActivityLogResponse } from '../types';
...
export const useActivityLogs = () => {
  ...
  const fetchLogs = useCallback(async (filters: ActivityLogFilters = {}) => {
    ...
    try {
      ...
      const result = await apiClient.get<ActivityLogResponse>(`/api/activity-logs`, { params: Object.fromEntries(params) });
  ...
  const fetchLogById = useCallback(async (id: string): Promise<ActivityLog | null> => {
    try {
      const result = await apiClient.get<{ data: ActivityLog }>(`/api/activity-logs/${id}`);
  ...
  const getLogStats = useCallback(async (): Promise<ActivityLogStats | null> => {
    try {
      const result = await apiClient.get<{ data: ActivityLogStats }>('/api/activity-logs/stats');
  ...
  const exportLogs = useCallback(async (filters: ActivityLogFilters = {}, format: 'csv' | 'json' = 'csv') => {
    try {
      ...
      await apiClient.download(`/api/activity-logs/export?${params.toString()}`, `activity-logs-${new Date().toISOString().split('T')[0]}.${format}`);
  ...
  const clearOldLogs = useCallback(async (daysToKeep: number = 30): Promise<boolean> => {
    try {
      await apiClient.delete('/api/activity-logs/clear', { data: { days_to_keep: daysToKeep } );
