 ...
 * @module modules/Analytics/types/analyticsProviders
 * @description
 * Providers específicos para o módulo Analytics, incluindo interfaces para
 * Google Analytics, Facebook Analytics, Twitter Analytics, LinkedIn Analytics,
 * Instagram Analytics, YouTube Analytics, TikTok Analytics, API customizada,
 * Webhook, Database e File Import. Também inclui classe base e factory para criação de providers.
 * @since 1.0.0
 */
...
/**
 * Interface AnalyticsProvider - Interface Base para Providers de Analytics
 * @interface AnalyticsProvider
 * @description
 * Interface base para todos os providers de analytics, definindo propriedades
 * comuns como id, name, type, version, enabled, config, credentials, lastSync,
 * syncInterval e status.
 * @property {string} id - ID único do provider
 * @property {string} name - Nome do provider
 * @property {string} type - Tipo do provider
 * @property {string} version - Versão do provider
 * @property {boolean} enabled - Se o provider está habilitado
 * @property {Record<string, any>} config - Configurações do provider
 * @property {Record<string, any>} [credentials] - Credenciais do provider (opcional)
 * @property {string} [lastSync] - Data/hora da última sincronização (opcional)
 * @property {number} syncInterval - Intervalo de sincronização em ms
 * @property {'connected' | 'disconnected' | 'error' | 'pending'} status - Status do provider
 */
...
export interface ProviderMethods {
  ...
  validateConfig(): Promise<{ isValid: boolean;
  errors: string[];
}>;
  getStatus(): Promise<{
    connected: boolean;
...
export abstract class BaseAnalyticsProvider implements AnalyticsProvider, ProviderMethods {
  ...
  protected handleError(error: unknown, context: string): void {
    ...
    this.log(`Erro em ${context}: ${errorMessage}`, 'error');
...
export class AnalyticsProviderFactory {
  private static providers: Map<string, new (config: unknown) => BaseAnalyticsProvider> = new Map();

  static register(type: string, providerClass: new (config: unknown) => BaseAnalyticsProvider): void {
    this.providers.set(type, providerClass);

  }

  static create(type: string, config: unknown): BaseAnalyticsProvider {
 ...
 */
export const AVAILABLE_PROVIDERS: unknown = {
