 ...
 * @module modules/Analytics/types/analyticsTypes
 * @description
 * Tipos principais do módulo Analytics, incluindo métricas, dispositivos,
 * fontes de tráfego, relatórios, períodos, status, formatos de exportação,
 * insights, dashboards, gráficos, filtros, dados de tempo real e integrações.
 * @since 1.0.0
 */

/**
 * Tipo AnalyticsMetricType - Tipos de Métricas
 * @typedef {string} AnalyticsMetricType
 * @description
 * Tipo que representa os tipos de métricas de analytics disponíveis.
 */
...
export type AnalyticsRealTimeData ={ active_users: number;
  page_views: number;
  top_pages: Array<{
    page: string;
    views: number;
...
export type GoogleAnalyticsData ={ property_id: string;
  view_id: string;
  metrics: AnalyticsMetric[];
  dimensions: Array<{
    name: string;
