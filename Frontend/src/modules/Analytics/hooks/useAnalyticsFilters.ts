...
export const useAnalyticsFilters = () => {
  ...
  const applyFiltersWithLoading = useCallback(async (newFilters: AnalyticsFilters) => {
    ...
    try {
      ...
      const errorMessage = getErrorMessage(err) || 'Erro ao aplicar filtros';
      setError(errorMessage);

      showError('Erro ao aplicar filtros', errorMessage);
  ...
  const clearFiltersWithLoading = useCallback(async () => {
    ...
    try {
      ...
      showError('Erro ao limpar filtros', errorMessage);

    } finally {
      setLoading(false);

    } , [clearFilters, showSuccess, showError]);
    ...
    for (const [key, value] of params.entries()) {
      switch (key) {
        ...
        case 'metrics':
          ...
          break;
        case 'devices':
          newFilters.devices = value.split(',') as unknown[];
          break;
        case 'traffic_sources':
          newFilters.traffic_sources = value.split(',') as unknown[];
          break;
        default:
