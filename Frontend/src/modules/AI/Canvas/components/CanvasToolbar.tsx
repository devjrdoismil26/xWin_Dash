...
const CanvasToolbar = ({ mode, onModeChange, onOperation, onUndo, onRedo, canUndoRedo }) => {
  return (
            <div className=" ">$2</div><ToggleGroup value={mode} onValueChange={onModeChange} className="flex gap-1" />
        <Button variant={mode === 'edit' ? 'primary' : 'outline'} size="sm" onClick={ () => onModeChange('edit')  }>
          <Edit className="w-4 h-4" /></Button><Button variant={mode === 'preview' ? 'primary' : 'outline'} size="sm" onClick={ () => onModeChange('preview')  }>
          <Eye className="w-4 h-4" /></Button></ToggleGroup>
      <div className=" ">$2</div><Button onClick={() => onOperation('save')} size="sm">
