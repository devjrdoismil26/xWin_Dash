/**
 * Validadores do módulo AI
 *
 * @description
 * Funções de validação para o módulo AI incluindo validação de provedores,
 * tipos de geração, prompts e requisições de geração.
 *
 * @module modules/AI/utils/aiValidators
 * @since 1.0.0
 */
 ...
 *
 * @description
 * Verifica se o provedor fornecido é válido.
 *
 * @param {string} provider - Provedor a validar
 * @returns {boolean} Se o provedor é válido (type guard)
 */
 ...
 * @description
 * Verifica se o tipo de geração fornecido é válido.
 *
 * @param {string} type - Tipo de geração a validar
 * @returns {boolean} Se o tipo é válido (type guard)
 *
 * @example
 * ```typescript
 * validateGenerationType('text'); // true
 * ```
 */
 ...
 * @description
 * Valida uma requisição completa de geração de texto com IA.
 *
 * @param {GenerateTextRequest} request - Requisição a validar
 * @returns {valid: boolean, errors: string[]} Resultado da validação
 *
 * @example
 * ```typescript
 * const result = validateTextGenerationRequest({
 ...
 * @description
 * Valida uma requisição completa de geração de vídeo com IA.
 *
 * @param {GenerateVideoRequest} request - Requisição a validar
 * @returns {valid: boolean, errors: string[]} Resultado da validação
 *
 * @example
 * ```typescript
 ...
 * Validar API key
 * @description
 * Valida uma API key de um provedor específico, verificando formato e prefixos
 * esperados para cada provedor.
 * 
 * @param {string} apiKey - API key a validar
 * @param {AIProvider} provider - Provedor da API key
 * @returns {valid: boolean; errors: string[]} Resultado da validação com erros
 * 
 * @example
 * ```typescript
 * const result = validateApiKey('sk-123...', 'openai');
 ...
 */
export const validateModelConfig = (config: Record<string, any>): { valid: boolean; errors: string[] } => {
  const errors: string[] = [];
 ...
 */
export const validateHistoryFilters = (filters: Record<string, any>): { valid: boolean; errors: string[] } => {
  const errors: string[] = [];
 ...
 */
export const validateExportConfig = (config: Record<string, any>): { valid: boolean; errors: string[] } => {
  const errors: string[] = [];
 ...
 */
export const validateNotificationConfig = (config: Record<string, any>): { valid: boolean; errors: string[] } => {
  const errors: string[] = [];
 ...
 */
export const validateGeneralConfig = (config: Record<string, any>): { valid: boolean; errors: string[] } => {
  const errors: string[] = [];
