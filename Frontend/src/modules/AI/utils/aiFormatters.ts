/**
 * Formatadores do módulo AI
 *
 * @description
 * Funções de formatação específicas para o módulo AI incluindo
 * formatação de resultados de geração, estatísticas e metadados.
 *
 * @module modules/AI/utils/aiFormatters
 * @since 1.0.0
 */
...
/**
 * Formata resultado de geração AI
 *
 * @description
 * Formata resultado de geração baseado no tipo (text, image, video, audio, code).
 *
 * @param {AIGeneration} generation - Geração a formatar
...
export const formatGenerationsList = (generations: AIGeneration[]): string => {
  ...
  const summary = generations.reduce((acc: unknown, gen: unknown) => {
 ...
 */
export const formatAnalyticsSummary = (data: Record<string, any>): string => {
