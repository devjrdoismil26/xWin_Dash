...
/**
 * Hook useAIProviders - Hook de Provedores de IA
 * @function
 * @description
 * Hook especializado para gerenciar provedores de IA, incluindo status,
 * configuração, seleção e estatísticas de provedores.
 *
 * @returns {object} Objeto com estado, provedores e ações
 *
 * @example
 * ```typescript
 * const {
...
export const useAIProviders = () => {
  ...
  const loadProviders = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? getErrorMessage(err) : 'Erro ao carregar provedores';
  ...
  const loadServicesStatus = useCallback(async () => {
    try {
      ...
      const errorMessage = err instanceof Error ? getErrorMessage(err) : 'Erro ao carregar status dos serviços';
      setError(errorMessage);
