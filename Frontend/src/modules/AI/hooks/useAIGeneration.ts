...
import { GenerateTextRequest, GenerateImageRequest, GenerateVideoRequest, AIGeneration, AIFilters } from '../types';
import { getErrorMessage } from '@/utils/errorHelpers';

/**
 * Interface UseAIGenerationReturn - Retorno do Hook useAIGeneration
 * @interface UseAIGenerationReturn
 * @description
 * Interface que define o retorno do hook useAIGeneration, incluindo estado,
 * gerações e ações para gerenciar gerações de IA.
 */
interface UseAIGenerationReturn {
  // Estado
...
export const useAIGeneration = (): UseAIGenerationReturn => {
  ...
  const loadGenerations = useCallback(async (filters?: AIFilters) => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? getErrorMessage(err) : 'Erro ao carregar gerações';
  ...
  const createGeneration = useCallback(async (data: Partial<AIGeneration>) => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? getErrorMessage(err) : 'Erro ao criar geração';
      setError(errorMessage);
  ...
  const updateGeneration = useCallback(async (id: string, data: Partial<AIGeneration>) => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? getErrorMessage(err) : 'Erro ao atualizar geração';
      setError(errorMessage);
  ...
  const deleteGeneration = useCallback(async (id: string) => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? getErrorMessage(err) : 'Erro ao excluir geração';
      setError(errorMessage);
