...
/**
 * Hook useAIAnalytics - Hook de Analytics de IA
 * @function
 * @description
 * Hook especializado para gerenciar analytics do módulo AI, incluindo carregamento
 * de analytics por período, cálculo de métricas em tempo real, tendências e insights.
 *
 * @returns {object} Objeto com estado, analytics, ações e métricas
 *
 * @example
 * ```typescript
 * const {
...
export const useAIAnalytics = () => {
  ...
  const { showError, showSuccess } = useAdvancedNotifications();
  ...
  const loadAnalytics = useCallback(async (analyticsPeriod: AIPeriod = period) => {
    ...
    try {
      ...
      const errorMessage = getErrorMessage(err) || 'Erro ao carregar analytics';
      setError(errorMessage);

      showError('Erro ao carregar analytics', errorMessage);
  ...
  const calculateRealTimeMetrics = useCallback(() => {
    ...
    const calculateMetrics = (generations: typeof allGenerations) => {
      ...
      const totalCost = generations.reduce((sum: unknown, gen: unknown) => sum + (gen.metadata.cost || 0), 0);

      const totalTokens = generations.reduce((sum: unknown, gen: unknown) => sum + (gen.metadata.tokens || 0), 0);

      const avgQuality = generations.reduce((sum: unknown, gen: unknown) => sum + (gen.metadata.quality || 0), 0) / totalGenerations || 0;

      const providerUsage = generations.reduce((acc: unknown, gen: unknown) => {
        if (!acc[gen.provider]) {
