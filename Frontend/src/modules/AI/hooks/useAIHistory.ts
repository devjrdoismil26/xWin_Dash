 ...
 * @module modules/AI/hooks/useAIHistory
 * @description
 * Hook especializado para gerenciar histórico de gerações, chat e análises de IA,
 * incluindo filtros, busca, exportação e estatísticas do histórico.
 * @since 1.0.0
 */
...
export const useAIHistory = () => {
  ...
  const loadHistory = useCallback(async (newFilters?: AIFilters) => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? getErrorMessage(err) : 'Erro ao carregar histórico';
  ...
  const getAllHistory = useCallback((): AIHistoryItem[] => {
    ...
    return allGenerations.sort((a: unknown, b: unknown) => 
  ...
  const getHistoryStats = useCallback(() => {
    ...
    const providerStats = allHistory.reduce((acc: unknown, item: unknown) => {
      acc[item.provider] = (acc[item.provider] || 0) + 1;
      return acc;
    }, {} as Record<string, number>);

    const totalCost = allHistory.reduce((sum: unknown, item: unknown) => sum + (item.metadata.cost || 0), 0);

    const totalTokens = allHistory.reduce((sum: unknown, item: unknown) => sum + (item.metadata.tokens || 0), 0);
  ...
  const getChatSessions = useCallback((): AIChatSession[] => {
    return chatHistory.sort((a: unknown, b: unknown) => 
