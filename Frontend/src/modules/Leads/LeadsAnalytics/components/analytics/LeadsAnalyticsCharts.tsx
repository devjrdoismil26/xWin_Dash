...
const AnalyticsCharts: React.FC<AnalyticsChartsProps> = ({ analytics,
  ...
  const renderConversionFunnel = () => {
    ...
    return (
              <div className="{(funnelData || []).map((stage: unknown, index: unknown) => (">$2</div>
          <div key={stage.name} className="relative">
           
        </div><div className=" ">$2</div><span className="text-sm font-medium text-gray-700">{stage.name}</span>
              <span className="text-sm text-gray-500">{stage.count} ({formatPercentage(stage.percentage)})</span></div><div className=" ">$2</div><div
                className="bg-blue-600 h-2 rounded-full transition-all duration-300"
                style={width: `${stage.percentage} %` } >
           
        </div>{index < funnelData.length - 1 && (
              <div className=" ">$2</div><TrendingDown className="w-4 h-4 text-gray-400" />
              </div>
            )}
          </div>
        ))}
  ...
  const renderLeadScoring = () => {
    ...
    return (
              <div className=" ">$2</div><div className=" ">$2</div><div className="{metrics?.average_score?.toFixed(1) || 67.2}">$2</div>
          </div>
          <div className="text-sm text-gray-600">Score Médio</div>
        <div className="{(scoringData || []).map((item: unknown) => (">$2</div>
        ...
        </div><span className="text-sm text-gray-700">{item.range}</span></div><div className=" ">$2</div><span className="text-sm text-gray-600">{item.count}</span>
  ...
  const renderGeographicDistribution = () => {
    ...
    return (
        ...
        </div><h4 className="font-medium text-gray-900 mb-3">Por País</h4>
          <div className="{(countries || []).map((country: unknown) => (">$2</div>
              <div key={country.name} className="flex items-center justify-between">
           
        </div><span className="text-sm text-gray-700">{country.name}</span>
                <div className=" ">$2</div><span className="text-sm text-gray-600">{country.count}</span>
                  <div className=" ">$2</div><div
                      className="bg-blue-600 h-2 rounded-full"
                      style={width: `${country.percentage} %` } / />
           
        </div><span className="text-xs text-gray-500 w-8">{formatPercentage(country.percentage)}</span>
      </div>
    </>
  ))}
          </div>
        ...
        </div><h4 className="font-medium text-gray-900 mb-3">Por Cidade</h4>
          <div className="{[">$2</div>
        ...
        </div><span className="text-sm text-gray-700">{city.name}</span>
                <div className=" ">$2</div><span className="text-sm text-gray-600">{city.count}</span>
                  <div className=" ">$2</div><div
                      className="bg-green-600 h-2 rounded-full"
                      style={width: `${city.percentage} %` } / />
           
        </div><span className="text-xs text-gray-500 w-8">{formatPercentage(city.percentage)}</span>
      </div>
    </>
  ))}
          </div>
  ...
  return (
      ...
      <div className={className  }>
      </div><div className="{/* Conversion Funnel */}">$2</div>
        <Card className="p-6" />
          <div className=" ">$2</div><BarChart3 className="w-5 h-5 text-blue-600" />
        ...
        {/* Lead Scoring */}
        <Card className="p-6" />
          <div className=" ">$2</div><PieChart className="w-5 h-5 text-green-600" />
        ...
        <Card className="p-6 lg:col-span-2" />
          <div className=" ">$2</div><LineChart className="w-5 h-5 text-purple-600" />
