...
import { Lead } from '../types/core/Lead';
...
export const useLeadTags = () => {
  ...
  const getLeadsByTags = useCallback(async (tagFilter: TagFilter): Promise<Lead[]> => {
    setLoading(true);

    setError(null);

    try {
      const result = await apiClient.post<{ data: Lead[] }>('/api/v1/leads/by-tags', tagFilter);
