...
vi.mock("../../shared/components/PageTransition", () => ({
  default: function MockPageTransition({
    children,
  }: {
    children: React.ReactNode;
  }) {
...
describe("MediaLibraryModule - Integration Tests", () => {
  ...
  describe("Interação com mídia", () => {
    ...
    it("deve permitir navegação entre pastas", async () => {
      ...
      await waitFor(() => {
        expect(mockMediaLibraryStore.setCurrentFolder).toHaveBeenCalledWith(
          "1",);
  ...
  describe("Busca e filtros", () => {
    it("deve permitir busca por texto", async () => {
      ...
      await waitFor(() => {
        expect(mockMediaLibraryStore.setSearchQuery).toHaveBeenCalledWith(
          "test",);
  ...
  describe("Operações em lote", () => {
    it("deve permitir seleção múltipla", async () => {
      ...
      await waitFor(() => {
        expect(mockMediaLibraryStore.setSelectedMedia).toHaveBeenCalledWith([
          "1",
          "2",
        ]);
