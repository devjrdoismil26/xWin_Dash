...
export const useMedia = (options: UseMediaOptions = {}) => {
  ...
  const loadMedia = useCallback(async () => {
    ...
    try {
      ...
      setMedia(Array.isArray(response?.data?.data) ? (response as any).data.data : []);
  ...
  const uploadFiles = useCallback(async (files: File[] | FileList, extra: { folder_id?: string | number; tags?: string[] } = {}) => {
    ...
    list.forEach((f: unknown) => formData.append('files[]', f));
    ...
    try {
      ...
      const newMedia: MediaItem[] = Array.isArray(response?.data?.data) ? (response as any).data.data : [];
      setMedia((prev: unknown) => [...newMedia, ...prev]);
  ...
  const deleteMedia = useCallback(async (id: string | number) => {
    try {
      ...
      setMedia((prev: unknown) => prev.filter((m: unknown) => m.id !== id));
  ...
  const organizeMedia = useCallback(async (payload: { folder_id?: string | number; ids: Array<string | number> }) => {
    try {
      ...
      setMedia((prev: unknown) => prev.slice());
