...
import { User, UserProfile, UserRole, UserActivity, UserNotification, UserStats, SystemStats, UserFilters, PaginationMeta } from '../types/user.types';
...
interface UsersActions {
  ...
  updatePassword: (passwordData: { current_password: string;
  new_password: string;
  ...
  assignRole: (userId: string, roleId: string) => Promise<Record<string, any>>;
  removeRole: (userId: string, roleId: string) => Promise<Record<string, any>>;
  getUserRoles: (userId: string) => Promise<UserRole[]>;

  // Ações para atividades
  getUserActivity: (userId: string, filters?: Record<string, any>) => Promise<UserActivity[]>;
  getActivityStats: (userId: string, filters?: Record<string, any>) => Promise<Record<string, any>>;

  // Ações para notificações
  fetchNotifications: (filters?: Record<string, any>) => Promise<void>;
  markNotificationAsRead: (notificationId: string) => Promise<Record<string, any>>;
  markAllNotificationsAsRead: () => Promise<Record<string, any>>;
    ...
    (set: unknown, get: unknown) => ({
      ...
      fetchUsers: async (filters: UserFilters = {}) => {
        ...
        try {
          ...
          set({ 
            ...
            pagination: (data as any).meta || null,
            loading: false 
          });
      ...
      uploadAvatar: async (file: File) => {
        ...
        try {
          ...
          set(state => ({
            profile: state.profile ? { ...state.profile, avatar: (data as any).avatar } : null,
            loading: false
          }));
      ...
      fetchNotifications: async (filters: Record<string, any> = {}) => {
        ...
        try {
          ...
          set({ 
            notifications: (data as any).data || data,
            loading: false 
          });
      ...
      getActiveUsers: () => {
        ...
        return users.filter(user => user.status === 'active' || (user as { is_active?: boolean }).is_active);
      ...
      getInactiveUsers: () => {
        ...
        return users.filter(user => user.status === 'inactive' || !(user as { is_active?: boolean }).is_active);
