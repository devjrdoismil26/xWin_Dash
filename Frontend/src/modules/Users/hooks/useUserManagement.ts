...
export const useUsersAdvanced = (): UseUsersAdvancedReturn => {
  ...
  const loadUsers = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? getErrorMessage(err) : 'Erro ao carregar usuários';
  ...
  const updateUserPermissions = useCallback(async (userId: string, permissions: string[]) => {
    try {
      ...
      const errorMessage = err instanceof Error ? getErrorMessage(err) : 'Erro ao atualizar permissões';
  ...
  const updateUserRoles = useCallback(async (userId: string, roles: string[]) => {
    try {
      ...
      const errorMessage = err instanceof Error ? getErrorMessage(err) : 'Erro ao atualizar funções';
