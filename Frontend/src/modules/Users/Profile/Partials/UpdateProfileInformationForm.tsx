...
interface UpdateProfileInformationFormProps {
  ...
  onClick?: (e: any) => void;
  onChange?: (e: any) => void; }
const UpdateProfileInformationForm: React.FC<UpdateProfileInformationFormProps> = ({ mustVerifyEmail, 
  status, 
  className = '' 
   }) => {
  ...
  return (
      ...
      <Card />
        <div className=" ">$2</div><header className="mb-6" />
            ...
            </p></header><form onSubmit={handleSubmit} className="space-y-6" />
            <div>
           
        </div><InputLabel htmlFor="name" value="Nome" / />
              <Input
                id="name"
                type="text"
                value={ data.name }
                ...
                autoFocus
                autoComplete="name" />
              <InputError text={errors.name} className="mt-2" /></div><div>
              ...
              {mustVerifyEmail && user.email_verified_at === null && (
                <div>
        ...
        </div><p className="text-sm mt-2 text-gray-800" />
                  ...
                  {status === 'verification-link-sent' && (
                    <div className="Um novo link de verificação foi enviado para seu endereço de email.">$2</div>
    </div>
  )}
                </div>
              )}
