...
const NotificationItem: React.FC<{
...
}> = ({ notification, onRemove, onMarkAsRead }) => { const IconComponent = NOTIFICATION_ICONS[notification.type];
  ...
  return (
      ...
      notification.read ? 'opacity-75' : ''
      ...
      </div><div className=" ">$2</div><div className={cn('flex-shrink-0', styles.icon)  }>
        </div><IconComponent className="w-5 h-5" /></div><div className=" ">$2</div><div className=" ">$2</div><h4 className={cn('text-sm font-medium', styles.title) } />
              {notification.title}
            </h4>
            <div className="{!notification.read && (">$2</div>
                <button
          ...
          {notification?.actions?.length && notification.actions.length > 0 && (
            <div className="{(notification.actions || []).map((action: unknown, index: unknown) => (">$2</div>
                <Button 
                  key={ index }
...
const NotificationContainer: React.FC<{
  ...
  return (
      ...
      </div>{(notifications || []).map((notification: unknown) => (
        <NotificationItem 
          key={ notification.id }
          notification={ notification }
