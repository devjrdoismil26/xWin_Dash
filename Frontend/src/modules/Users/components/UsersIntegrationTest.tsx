...
const UsersIntegrationTest: React.FC<UsersIntegrationTestProps> = ({ 
  className = '',
  autoRun = false,
  showDetails = true,
  onTestComplete
  ...
  const runAllTests = async () => {
    ...
    try {
      ...
      addLog(`Erro durante execução: ${error instanceof Error ? error.message : 'Erro desconhecido'}`);
    } finally {
      setIsRunning(false);
  ...
  const runTestSuite = async (suite: TestSuite): Promise<Partial<TestSuite>> => {
    ...
    try {
      ...
      addLog(`  Erro na suite ${suite.name}: ${error instanceof Error ? error.message : 'Erro desconhecido'}`);
      return {
        status: 'failed',
        endTime: new Date(),
  ...
  const runTest = async (test: Test): Promise<Partial<Test>> => {
    ...
    try {
      ...
      addLog(`    ✗ ${test.name} - ${duration}ms - ${error instanceof Error ? error.message : 'Erro desconhecido'}`);
  ...
  return (
    <div className={`space-y-6 ${className}`}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h3 className="text-lg font-semibold text-gray-900">Testes de Integração</h3>
          <p className="text-gray-600">Validação completa do sistema de usuários</p>
        </div>
          ...
          {results && (
            <div className="flex items-center gap-4 text-sm">
              <div className="flex items-center gap-1">
                ...
                <span>{results.passedTests} passaram</span>
              </div>
              <div className="flex items-center gap-1">
                <XCircle className="w-4 h-4 text-red-600" />
                <span>{results.failedTests} falharam</span>
              </div>
              <div className="flex items-center gap-1">
      ...
      <div className="space-y-4">
        {testSuites.map((suite, index) => (
          <Animated key={suite.id} delay={index * 100}>
            <Card className="p-4">
