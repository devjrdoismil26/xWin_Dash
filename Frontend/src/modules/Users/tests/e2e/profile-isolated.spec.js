...
test.describe("Perfil de Usuário - Testes Isolados", () => {
  test.describe("Página de Perfil Principal", () => {
    ...
    test("deve exibir informações do usuário", async ({ page }) => {
      ...
      const profileElements = await page.evaluate(() => {
        return {
          hasProfileSection: !!document.querySelector(
            '[data-testid*="profile"], [class*="profile"], [id*="profile"]',
          ),
          hasUserInfo: !!document.querySelector(
            '[data-testid*="user-info"], [class*="user-info"], [id*="user-info"]',
          ),
          hasAvatar: !!document.querySelector(
            'img[alt*="avatar"], [data-testid*="avatar"], [class*="avatar"]',
          ),
          hasName: !!document.querySelector(
            '[data-testid*="name"], [class*="name"], [id*="name"]',
          ),
      ...
      const editElements = await page.evaluate(() => {
        return {
          hasEditButton: !!document.querySelector(
            'button[data-testid*="edit"], [class*="edit"], [id*="edit"]',
          ),
          hasEditLink: !!document.querySelector(
            'a[href*="edit"], a[data-testid*="edit"]',
          ),
    ...
    test("deve ter seções organizadas", async ({ page }) => {
      ...
      const pageStructure = await page.evaluate(() => {
        return {
          ...
          hasHeaders: document.querySelectorAll("h1, h2, h3, h4, h5, h6")
            .length,
          hasCards: document.querySelectorAll(
            '[class*="card"], [data-testid*="card"]',
          ).length,
          hasGrid: document.querySelectorAll('[class*="grid"], [class*="flex"]')
            .length,
        };
      });
    ...
    test("deve exibir formulário de edição", async ({ page }) => {
      const formElements = await page.evaluate(() => {
        return {
          ...
          hasPhoneInput: !!document.querySelector(
            'input[name="phone"], input[data-testid*="phone"]',
          ),
          hasBioInput: !!document.querySelector(
            'textarea[name="bio"], textarea[data-testid*="bio"]',
          ),
      ...
      const uploadElements = await page.evaluate(() => {
        return {
          ...
          hasUploadButton: !!document.querySelector(
            'button[data-testid*="upload"], [class*="upload"]',
          ),
          hasImagePreview: !!document.querySelector(
            'img[data-testid*="preview"], [class*="preview"]',
          ),
          hasDragDrop: !!document.querySelector(
            '[data-testid*="drop"], [class*="drop"]',
          ),
        };
      });
  ...
  test.describe("Página de Preferências", () => {
    ...
    test("deve carregar a página de preferências corretamente", async ({
      page,
    }) => {
      ...
      const preferenceElements = await page.evaluate(() => {
        return {
          hasThemeToggle: !!document.querySelector(
            '[data-testid*="theme"], [class*="theme"], [id*="theme"]',
          ),
          ...
          hasPrivacySettings: !!document.querySelector(
            '[data-testid*="privacy"], [class*="privacy"]',
          ),
          hasSecuritySettings: !!document.querySelector(
            '[data-testid*="security"], [class*="security"]',
          ),
          totalSwitches: document.querySelectorAll(
    ...
    test("deve ter configurações de notificação", async ({ page }) => {
      const notificationElements = await page.evaluate(() => {
        return {
          hasEmailNotifications: !!document.querySelector(
            '[data-testid*="email-notification"], [class*="email-notification"]',
          ),
          hasPushNotifications: !!document.querySelector(
            '[data-testid*="push-notification"], [class*="push-notification"]',
          ),
          hasSMSNotifications: !!document.querySelector(
            '[data-testid*="sms-notification"], [class*="sms-notification"]',
          ),
          hasWebhookSettings: !!document.querySelector(
            '[data-testid*="webhook"], [class*="webhook"]',
          ),
          totalNotificationSettings: document.querySelectorAll(
            '[data-testid*="notification"], [class*="notification"]',
          ).length,
        };
      });
    ...
    test("deve ter configurações de tema", async ({ page }) => {
      const themeElements = await page.evaluate(() => {
        return {
          hasLightMode: !!document.querySelector(
            '[data-testid*="light"], [class*="light"]',
          ),
          hasDarkMode: !!document.querySelector(
            '[data-testid*="dark"], [class*="dark"]',
          ),
          hasAutoMode: !!document.querySelector(
            '[data-testid*="auto"], [class*="auto"]',
          ),
          hasColorPicker: !!document.querySelector(
            'input[type="color"], [data-testid*="color"]',
          ),
          totalThemeOptions: document.querySelectorAll(
  ...
  test.describe("Página de Configurações de Conta", () => {
    ...
    test("deve carregar a página de configurações corretamente", async ({
      page,
    }) => {
    ...
    test("deve exibir configurações de conta", async ({ page }) => {
      const accountElements = await page.evaluate(() => {
        return {
          hasAccountSection: !!document.querySelector(
            '[data-testid*="account"], [class*="account"], [id*="account"]',
          ),
          hasPasswordChange: !!document.querySelector(
            '[data-testid*="password"], [class*="password"]',
          ),
          hasTwoFactor: !!document.querySelector(
            '[data-testid*="two-factor"], [class*="two-factor"]',
          ),
          hasAPIKeys: !!document.querySelector(
            '[data-testid*="api"], [class*="api"]',
          ),
          hasIntegrations: !!document.querySelector(
            '[data-testid*="integration"], [class*="integration"]',
          ),
          hasBackup: !!document.querySelector(
            '[data-testid*="backup"], [class*="backup"]',
          ),
          hasExport: !!document.querySelector(
            '[data-testid*="export"], [class*="export"]',
          ),
          hasImport: !!document.querySelector(
            '[data-testid*="import"], [class*="import"]',
          ),
          totalSettings: document.querySelectorAll(
            '[data-testid*="setting"], [class*="setting"]',
          ).length,
        };
      });
    ...
    test("deve ter alteração de senha", async ({ page }) => {
      const passwordElements = await page.evaluate(() => {
        return {
          hasCurrentPassword: !!document.querySelector(
            'input[name*="current-password"], input[data-testid*="current-password"]',
          ),
          hasNewPassword: !!document.querySelector(
            'input[name*="new-password"], input[data-testid*="new-password"]',
          ),
          hasConfirmPassword: !!document.querySelector(
            'input[name*="confirm-password"], input[data-testid*="confirm-password"]',
          ),
          hasPasswordStrength: !!document.querySelector(
            '[data-testid*="strength"], [class*="strength"]',
          ),
          hasPasswordRequirements: !!document.querySelector(
            '[data-testid*="requirements"], [class*="requirements"]',
          ),
          totalPasswordInputs: document.querySelectorAll(
            'input[type="password"]',
          ).length,
        };
      });
    ...
    test("deve ter autenticação de dois fatores", async ({ page }) => {
      const twoFactorElements = await page.evaluate(() => {
        return {
          has2FAEnabled: !!document.querySelector(
            '[data-testid*="2fa-enabled"], [class*="2fa-enabled"]',
          ),
          has2FADisable: !!document.querySelector(
            '[data-testid*="2fa-disable"], [class*="2fa-disable"]',
          ),
          hasQRCode: !!document.querySelector(
            '[data-testid*="qr"], [class*="qr"]',
          ),
          hasBackupCodes: !!document.querySelector(
            '[data-testid*="backup-codes"], [class*="backup-codes"]',
          ),
          hasRecoveryCodes: !!document.querySelector(
            '[data-testid*="recovery"], [class*="recovery"]',
          ),
          total2FAElements: document.querySelectorAll(
            '[data-testid*="2fa"], [class*="2fa"]',
          ).length,
        };
      });
  ...
  test.describe("Página de Atividade do Usuário", () => {
    ...
    test("deve carregar a página de atividade corretamente", async ({
      page,
    }) => {
    ...
    test("deve exibir histórico de atividades", async ({ page }) => {
      const activityElements = await page.evaluate(() => {
        return {
          ...
          totalActivities: document.querySelectorAll(
    ...
    test("deve ter filtros de atividade", async ({ page }) => {
      const filterElements = await page.evaluate(() => {
        return {
          ...
          hasTypeFilter: !!document.querySelector(
  ...
  test.describe("Acessibilidade das Páginas de Perfil", () => {
    ...
    test("deve ter contraste adequado", async ({ page }) => {
      ...
      const darkElements = await page.evaluate(() => {
        return {
          darkText: document.querySelectorAll(
            ".dark\\:text-white, .dark\\:text-gray-100",
          ).length,
          darkBg: document.querySelectorAll(
            ".dark\\:bg-gray-800, .dark\\:bg-gray-900",
          ).length,
          darkBorder: document.querySelectorAll(
    ...
    test("deve navegar entre seções do perfil", async ({ page }) => {
      ...
      const navigationElements = await page.evaluate(() => {
        return {
          ...
          hasPreferencesLink: !!document.querySelector(
            'a[href*="preferences"]',
          ),
      ...
      const breadcrumbElements = await page.evaluate(() => {
        return {
          hasBreadcrumbs: !!document.querySelector(
            '[data-testid*="breadcrumb"], [class*="breadcrumb"], nav[aria-label*="breadcrumb"]',
          ),
          breadcrumbItems: document.querySelectorAll(
            '[data-testid*="breadcrumb-item"], [class*="breadcrumb-item"]',
          ).length,
          hasHomeLink: !!document.querySelector(
