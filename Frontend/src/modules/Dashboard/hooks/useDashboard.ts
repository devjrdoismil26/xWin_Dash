/**
 * @module modules/Dashboard/hooks/useDashboard
 * @description
 * Hook principal do Dashboard - Interface simplificada.
 * 
 * Expõe funcionalidades essenciais do dashboard:
 * - Acesso aos dados de métricas e widgets
 * - Funções de conveniência (getDashboardSummary, getPerformanceMetrics, getTrendAnalysis)
 * - Gerenciamento de alertas
 * - Compartilhamento e assinaturas
  ...
  const getTrendAnalysis = useCallback(() => {
    ...
    return {
      ...
      averageGrowth: Object.values(performanceMetrics.growthRate)
        .reduce((sum: unknown, rate: unknown) => sum + rate, 0) / Object.keys(performanceMetrics.growthRate).length,};
