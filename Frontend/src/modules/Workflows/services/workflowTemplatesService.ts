...
import { WorkflowTemplate, WorkflowCanvasData, WorkflowStep, WorkflowTrigger } from '../types/workflowTypes';

// Cache para templates
const templateCache = new Map<string, { data: unknown; timestamp: number }>();
...
class WorkflowTemplatesService {
  ...
  async getTemplateById(templateId: string): Promise<WorkflowTemplate> {
    try {
      ...
      // Cache do resultado
      templateCache.set(cacheKey, { data: (response as any).data, timestamp: Date.now() });
  ...
  private validateTemplateData(data: CreateTemplateData): void {
    ...
    if (!data.description || (data as any).description.trim().length === 0) {
