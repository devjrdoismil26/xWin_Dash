...
import { WorkflowExecutionQueue, WorkflowExecutionQueueStatus, WorkflowExecutionPriority, WorkflowExecutionQueueStats, WorkflowExecutionQueueFilter, WorkflowExecutionQueueSort, WorkflowExecutionQueuePagination, WorkflowExecutionQueueResponse, WorkflowQueueProcessResult, WorkflowQueueRetryResult, WorkflowQueueClearResult, WorkflowQueueStatus } from '../types/workflowTypes';

// Cache para filas
const queueCache = new Map<string, { data: unknown; timestamp: number }>();
...
class WorkflowQueueService {
  ...
  async getExecutionQueue(params: QueueFilterParams = {}): Promise<WorkflowExecutionQueueResponse> {
    try {
      ...
      // Cache do resultado
      queueCache.set(cacheKey, { data: (response as any).data, timestamp: Date.now() });
