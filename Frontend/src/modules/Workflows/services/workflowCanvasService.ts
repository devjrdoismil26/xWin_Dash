import { WorkflowCanvasData, WorkflowCanvasNode, WorkflowCanvasEdge, WorkflowStep, WorkflowTrigger } from '../types/workflowTypes';

// Cache para layouts de canvas
const canvasCache = new Map<string, { data: unknown; timestamp: number }>();
...
class WorkflowCanvasService {
  ...
  private applyHorizontalLayout(nodes: WorkflowCanvasNode[], config: LayoutConfig): Array<{
    ...
    let currentX = 0;
    levels.forEach((levelNodes: unknown, levelIndex: unknown) => {
      let currentY = 0;
      const levelHeight = Math.max(...levelNodes.map(() => 100)); // Altura padrão
      
      levelNodes.forEach((node: unknown, nodeIndex: unknown) => {
        result.push({
          id: node.id,
          position: {
            x: currentX,
  ...
  private calculateOptimizationScore(
    ...
    optimizedEdges: Array<{ id: string;
      source: string;
      target: string;
      sourceHandle?: string;
      targetHandle?: string;
