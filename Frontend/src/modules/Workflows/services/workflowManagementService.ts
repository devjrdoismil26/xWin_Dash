...
import { Workflow, WorkflowStatus, WorkflowTrigger, WorkflowStep, WorkflowCanvasData, WorkflowValidationResult } from '../types/workflowTypes';

// Cache para workflows
const workflowCache = new Map<string, { data: unknown; timestamp: number }>();
...
class WorkflowManagementService {
  ...
  async getWorkflowById(id: number): Promise<Workflow> {
    try {
      ...
      // Cache do resultado
      workflowCache.set(cacheKey, { data: (response as any).data, timestamp: Date.now() });
  ...
  private validateWorkflowData(data: CreateWorkflowData): void {
    ...
    if (!data.steps || (data as any).steps.length === 0) {
