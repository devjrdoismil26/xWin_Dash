...
import { Zap, Play, Clock, Calendar, Mail, MessageSquare, Database, Globe, Smartphone, Bell, MousePointer, FileText, User } from 'lucide-react';
import { cn } from '@/lib/utils';
const TriggerNode: React.FC<{ id: string; data: Record<string, any>; selected?: boolean; onUpdate??: (e: any) => void; onDelete??: (e: any) => void; onConnect???: (e: any) => void; onDisconnect???: (e: any) => void }> = ({ data, isConnectable, selected }) => {
      ...
      <div className=" ">$2</div><p className="text-xs text-gray-600 mb-2" />
        ...
        {data?.webhookUrl && (
          <div className=" ">$2</div><Globe className="h-3 w-3 text-gray-500" />
            <span className="{data.webhookUrl}">$2</span>
            </span>
      </div>
    </>
  )}
        ...
        { data?.isActive !== undefined && (
          <div className=" ">$2</div><div className={cn(
              ...
              (data as any).isActive ? 'bg-green-500' : 'bg-gray-400'
            )  }>
        </div><span className="{data.isActive ? 'Ativo' : 'Inativo'}">$2</span>
            </span>
      </div>
    </>
  )}
      </div>
      <div className=" ">$2</div><div className=" ">$2</div><Play className="h-3 w-3 text-green-600" />
          <span className="text-xs text-green-600 font-medium">TRIGGER</span></div></div>);};
