...
import { Share2, Repeat, Users, Heart, MessageCircle, Repeat, Bookmark, Eye, MousePointer, TrendingUp, TrendingDown, BarChart, PieChart, Globe, Smartphone, Monitor, Image, Video, FileText, Link, Hash, AtSign, Calendar, Clock, Zap, Settings, Play, Pause, Square, CheckCircle, XCircle, AlertTriangle } from 'lucide-react';
import { cn } from '@/lib/utils';
const SocialMediaAutomationNode: React.FC<{ id: string; data: Record<string, any>; selected?: boolean; onUpdate??: (e: any) => void; onDelete??: (e: any) => void; onConnect???: (e: any) => void; onDisconnect???: (e: any) => void }> = ({ data, isConnectable, selected }) => {
  const getAutomationIcon = (type: unknown) => {
    switch (type) {
      case 'post':
  ...
  const getAutomationColor = (type: unknown) => {
    switch (type) {
      case 'post':
  ...
  return (
      ...
      / />
      <div className=" ">$2</div><div className="{getAutomationIcon(automationType)}">$2</div>
        </div>
        <div className="{automationName}">$2</div>
        </div>
      <div className=" ">$2</div><p className="text-xs text-gray-600 mb-2" />
        ...
        {data?.automationType && (
          <div className=" ">$2</div><span className="text-xs font-medium text-gray-700">Tipo:</span>
            <span className="{automationType}">$2</span>
            </span>
      </div>
    </>
  )}
        {data?.platforms && (data as any).platforms.length > 0 && (
          <div className=" ">$2</div><span className="text-xs font-medium text-gray-700">Plataformas:</span>
            <div className="{data.platforms.slice(0, 2).map((platform: unknown, index: unknown) => (">$2</div>
                <div key={index} className="text-xs text-gray-600 bg-white rounded px-2 py-1">
            {platform}
          </div>
              ))}
              {data.platforms.length > 2 && (
                <div className="+{data.platforms.length - 2} mais...">$2</div>
    </div>
  )}
            </div>
        )}
        ...
        {data?.frequency && (
          <div className=" ">$2</div><Clock className="h-3 w-3 text-gray-500" />
            <span className="Frequência: {data.frequency}">$2</span>
            </span>
      </div>
    </>
  )}
      ...
      <div className=" ">$2</div><div className=" ">$2</div><Share2 className="h-3 w-3 text-gray-600" />
          <span className="text-xs text-gray-600 font-medium">AUTOMAÇÃO</span></div></div>);};
