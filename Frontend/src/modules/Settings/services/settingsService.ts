...
class SettingsService {
  ...
  async getSetting(key: string): Promise<SettingsResponse> {
    try {
      ...
      if (response.success && (response as any).data) {
        settingsCacheService.cacheSetting(key, (response as any).data);
   ...
   */
  async updateSetting(key: string, value: unknown): Promise<SettingsResponse> {
    try {
      // Validar dados
  ...
  async getSettingsByCategory(category: string): Promise<SettingsResponse> {
    try {
      ...
      if (response.success && (response as any).data) {
        settingsCacheService.cacheCategory(category, (response as any).data);
  ...
  async updateMultipleSettings(settings: Record<string, any>): Promise<SettingsResponse> {
    try {
      ...
      return settingsErrorService.handleError(error, {
        operation: 'updateMultipleSettings',
        timestamp: Date.now()
  });
   ...
   */
  async importSettings(data: unknown): Promise<SettingsResponse> {
    try {
