...
import { Settings, Shield, Users, Database, Mail, Zap, Globe, Key, ChevronRight, ChevronDown } from 'lucide-react';
...
const SettingsSidebar: React.FC<SettingsSidebarProps> = ({ activeCategory,
  onCategoryChange,
  collapsed = false,
  stats,
  className = ''
  ...
  if (collapsed) {
    return (
      ...
      <div className="{(settingsCategories || []).map((category: unknown, index: unknown) => {">$2</div>
  ...
  return (
      ...
      <div className="{/* Header da Sidebar */}">$2</div>
        <div className=" ">$2</div><h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-2" />
            Configurações
          </h2>
          {stats && (
            <div className=" ">$2</div><p>{stats.totalSettings} configurações em {stats.totalCategories} categorias</p>
              {stats.lastUpdated && (
        ...
        <div className="{(settingsCategories || []).map((category: unknown, index: unknown) => {">$2</div>
            ...
            return (
        <>
      <Animated key={category.id} type="slideRight" />
  ...
  >
                    <div className=" ">$2</div><IconComponent className={`h-5 w-5 mr-3 ${
                        isActive 
                          ? `text-${category.color} -600 dark:text-${category.color}-400` 
                          : 'text-gray-500'
                      }`} / />
                      <div className=" ">$2</div><div className=" ">$2</div><span className="font-medium">{category.name}</span>
                      ...
                      {hasSubcategories && (
                        <div className="{isExpanded ? (">$2</div>
      <ChevronDown className="h-4 w-4" />
                  ...
                  {hasSubcategories && isExpanded && (
                    <div className="{category.subcategories!.map((subcategory: unknown) => {">$2</div>
  ...
  >
                            <div className=" ">$2</div><span>{subcategory.name}</span>
                              <div className="{subcategory.count && (">$2</div>
