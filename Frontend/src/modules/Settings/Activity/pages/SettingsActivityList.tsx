...
export default function ActivityList() {
  ...
  const filteredActivities = (activities || []).filter((activity: unknown) => {
  ...
  return (
        ...
        <Card.Content />
          <div className=" ">$2</div><Input placeholder="Buscar" value={searchTerm} onChange={ (e: React.ChangeEvent<HTMLInputElement>) => setSearchTerm(e.target.value) } />
            <select value={filterType} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFilterType(e.target.value)} className="px-3 py-2 border rounded">
          ...
          <ul className="divide-y" />
            {(filteredActivities || []).map((activity: unknown) => (
