/**
 * @module modules/Aura/AuraChats/hooks/useAuraChats
 * @description
 * Hook especializado para gerenciamento de chats do módulo Aura.
 * 
 * Gerencia:
 * - Chats e conversas (fetch, send message, archive, block)
 * - Mensagens (fetch, send, mark as read)
 * - Contatos (fetch, create, update)
 * - Utilitários (getChatsByStatus, getActiveChats, getUnreadChats, getChatStats, getMessageStats)
 * 
 * @example
 * ```typescript
 * import { useAuraChats } from './hooks/useAuraChats';
 * 
 * const ChatComponent = () => {
 *   const {
 *     chats,
...
export const useAuraChats = () => {
  ...
  const fetchChats = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error
        ? getErrorMessage(err)
        : (err as { response?: { data?: { message?: string } })?.response?.data?.message || 'Erro ao carregar chats';
  ...
  const fetchMessages = useCallback(async (chatId: string) => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error
        ? getErrorMessage(err)
        : (err as { response?: { data?: { message?: string } })?.response?.data?.message || 'Erro ao carregar mensagens';
      setError(errorMessage);
  ...
  const sendMessage = useCallback(async (chatId: string, content: string, type: MessageType = 'text') => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error
        ? getErrorMessage(err)
        : (err as { response?: { data?: { message?: string } })?.response?.data?.message || 'Erro ao enviar mensagem';
      setError(errorMessage);
  ...
  const fetchContacts = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error
        ? getErrorMessage(err)
        : (err as { response?: { data?: { message?: string } })?.response?.data?.message || 'Erro ao carregar contatos';
      setError(errorMessage);
  ...
  const createContact = useCallback(async (contactData: Partial<AuraContact>) => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error
        ? getErrorMessage(err)
        : (err as { response?: { data?: { message?: string } })?.response?.data?.message || 'Erro ao criar contato';
      setError(errorMessage);
  ...
  const updateContact = useCallback(async (id: string, contactData: Partial<AuraContact>) => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error
        ? getErrorMessage(err)
        : (err as { response?: { data?: { message?: string } })?.response?.data?.message || 'Erro ao atualizar contato';
      setError(errorMessage);
  ...
  const archiveChat = useCallback(async (id: string) => {
    try {
      ...
      const errorMessage = err instanceof Error
        ? getErrorMessage(err)
        : (err as { response?: { data?: { message?: string } })?.response?.data?.message || 'Erro ao arquivar chat';
      setError(errorMessage);
  ...
  const blockChat = useCallback(async (id: string) => {
    try {
      ...
      const errorMessage = err instanceof Error
        ? getErrorMessage(err)
        : (err as { response?: { data?: { message?: string } })?.response?.data?.message || 'Erro ao bloquear chat';
      setError(errorMessage);
  ...
  const markMessagesAsRead = useCallback(async (chatId: string) => {
    try {
      ...
      const errorMessage = err instanceof Error
        ? getErrorMessage(err)
        : (err as { response?: { data?: { message?: string } })?.response?.data?.message || 'Erro ao marcar mensagens como lidas';
      setError(errorMessage);
