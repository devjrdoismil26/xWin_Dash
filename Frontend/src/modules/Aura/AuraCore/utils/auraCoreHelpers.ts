/**
 * @module modules/Aura/AuraCore/utils/auraCoreHelpers
 * @description
 * Funções auxiliares e utilitários para o módulo AuraCore.
 * 
 * Inclui helpers para:
 * - Formatação de estatísticas, módulos, ações rápidas e notificações
 * - Cálculos de scores de saúde e performance
 * - Filtragem e ordenação de dados
 * - Formatação de datas, duração, bytes
 * - Funções utilitárias (debounce, throttle, validações)
 * - Manipulação de strings (slugify, capitalize)
 * 
 * @example
 * ```typescript
 * import { formatAuraStats, calculateHealthScore, formatDate } from './auraCoreHelpers';
 * 
 * const stats = { total_connections: 100, uptime: 99.5, response_time: 150};
...
/**
 * Formata estatísticas do Aura para exibição
 *
 * @description
 * Formata estatísticas do Aura incluindo conexões, fluxos, mensagens,
 * tempo de resposta e uptime.
 *
 * @param {AuraStats} stats - Estatísticas do Aura
...
export const sortModulesByActivity = (modules: AuraModule[]) => {
  return modules.sort((a: unknown, b: unknown) => {
...
export const sortNotificationsByTimestamp = (notifications: AuraNotification[]) => {
  return notifications.sort((a: unknown, b: unknown) => 
...
export const calculateAverage = (values: number[]): number => {
  ...
  const sum = values.reduce((acc: unknown, val: unknown) => acc + val, 0);
