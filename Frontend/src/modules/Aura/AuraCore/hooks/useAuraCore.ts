/**
 * @module modules/Aura/AuraCore/hooks/useAuraCore
 * @description
 * Hook principal do módulo AuraCore - Orquestrador.
 * 
 * Coordena todos os hooks especializados do AuraCore e fornece:
 * - Acesso ao store do AuraCore (stats, modules, quick_actions, notifications, dashboardData)
 * - Funções de conveniência (getStatsSummary, getModulesByStatus, getActiveModules, etc.)
 * - Funções de análise (getSystemHealth, getPerformanceScore, getTrendData)
 * - Funções de exportação e atualização (exportDashboardData, refreshAllData)
 * 
 * @example
 * ```typescript
 ...
 *   return <div>Saúde do Sistema: {health}</div>;
 *};
...
export const useAuraCore = () => {
  ...
  const getRecentActivity = useCallback(() => {
    ...
    return activities.sort((a: unknown, b: unknown) => 
  ...
  const exportDashboardData = useCallback(async (format: string = 'json') => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error
        ? getErrorMessage(err)
        : (err as { response?: { data?: { message?: string } })?.response?.data?.message || 'Erro ao exportar dados do dashboard';
  ...
  const refreshAllData = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error
        ? getErrorMessage(err)
        : (err as { response?: { data?: { message?: string } })?.response?.data?.message || 'Erro ao atualizar dados';
      setError(errorMessage);
