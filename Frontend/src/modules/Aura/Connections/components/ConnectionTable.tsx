...
const ConnectionTable = ({ connections: propConnections = [] as unknown[], onEdit, onView, onDelete }) => {
  const { 
  ...
  const handleTest = async (connection: unknown) => {
  ...
  const handleConnect = async (connection: unknown) => {
  ...
  const handleDisconnect = async (connection: unknown) => {
  ...
  const copyWebhookUrl = (connection: unknown) => {
  ...
  if (connectionLoading) {
    return (
              <div className=" ">$2</div><div className=" ">$2</div><Loader2 className="w-8 h-8 animate-spin text-blue-600 mx-auto mb-4" />
          ...
          {displayConnections.length === 0 ? (
            <TableRow />
              <TableCell colSpan={6} className="text-center text-gray-500 py-8" />
                <div className=" ">$2</div><WifiOff className="w-12 h-12 text-gray-300" />
                  <div>
           
        </div><p className="font-medium">Nenhuma conexão encontrada</p>
                    <p className="text-sm text-gray-400">Crie sua primeira conexão WhatsApp para começar</p></div></TableCell>
      </TableRow>
    </>
  ) : (
            (displayConnections || []).map((connection: unknown) => (
              <TableRow key={connection.id} className="hover:bg-gray-50" />
                <TableCell className="font-medium" />
                  <div className=" ">$2</div><div className={`w-3 h-3 rounded-full ${
                      connection.is_active ? 'bg-green-500' : 'bg-gray-300'
                    } `} />
           
        </div><div>
           
        </div><div className="font-medium">{connection.name}</div>
                ...
                <TableCell />
                  <div className=" ">$2</div><Phone className="w-4 h-4 text-gray-400" />
                ...
                <TableCell />
                  <div className=" ">$2</div><div className="{connection.statistics?.messages_sent || 0}">$2</div>
                    </div>
                    <div className="text-xs text-gray-400">enviadas</div></TableCell><TableCell className="text-right" />
