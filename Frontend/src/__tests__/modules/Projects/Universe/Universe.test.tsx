...
const Universe = () => {
  ...
  const [selectedUniverse, setSelectedUniverse] = React.useState<any>(null);
  ...
  const filteredProjects = selectedUniverse
    ? (projects || []).filter(
        (project) => project.universeId === selectedUniverse.id,
      )
    : projects;

  const stats = {
    totalUniverses: universes.length,
    activeUniverses: (universes || []).filter((u) => u.status === "active")
      .length,
    inactiveUniverses: (universes || []).filter((u) => u.status === "inactive")
      .length,
  ...
  const deleteUniverse = (id) => {
    ...
    setProjects(
      (projects || []).filter((project) => project.universeId !== id),);};
  ...
  return (
      ...
      </div><h1 data-testid="universe-title">Gestão de Universos</h1>

      <div data-testid="universe-tabs" className="universe-tabs">
           
        </div><button
          data-testid="tab-universes"
          className={activeTab === "universes" ? "active" : ""} onClick={ () => setActiveTab("universes")  }>
          Universos
        </button>
        <button
          data-testid="tab-projects"
          className={activeTab === "projects" ? "active" : ""} onClick={ () => setActiveTab("projects")  }>
          Projetos
        </button>
        <button
          ...
          Visão Geral
        </button></div><div data-testid="universe-stats" className="universe-stats">
           
        </div><div data-testid="stat-total-universes">
          Universos: 
        </div>{stats.totalUniverses}
        </div>
        <div data-testid="stat-active-universes">
          Ativos: 
        </div>{stats.activeUniverses}
        </div>
        <div data-testid="stat-total-projects">
          Projetos: 
        </div>{stats.totalProjects}
        </div>
        <div data-testid="stat-active-projects">
          Ativos: 
        </div>{stats.activeProjects}
        </div>
        <div data-testid="stat-total-budget">
          Orçamento Total: R$ 
        </div>{stats.totalBudget.toLocaleString()}
        </div>
        <div data-testid="stat-avg-progress">
          Progresso Médio: 
        </div>{stats.avgProgress.toFixed(1)}%
        </div>

      {isLoading && (
        <div data-testid="universe-loading">Carregando dados...</div>
      )}

      {activeTab === "universes" && (
        <div data-testid="universes-tab" className="universes-tab">
           
        </div><div data-testid="universes-actions" className="universes-actions">
          ...
          {showCreateUniverse && (
            <div
              ...
              className="create-universe-form">
           
        </div><h3>Criar Novo Universo</h3>
              <form
                onSubmit={(e) => {
                  e.preventDefault();
        ...
        </div>{(filteredUniverses || []).map((universe) => (
              <div
                key={ universe.id }
                data-testid={`universe-${universe.id}`}
                className="universe-card">
           
        </div><div data-testid={`universe-name-${universe.id}`}>
           
        </div>{universe.name}
                </div>
                <div data-testid={`universe-description-${universe.id}`}>
           
        </div>{universe.description}
                </div>
                <div data-testid={`universe-status-${universe.id}`}>
           
        </div>{universe.status}
                </div>
                <div data-testid={`universe-projects-count-${universe.id}`}>
          Projetos: 
        </div>{universe.projectsCount}
                </div>
                <div data-testid={`universe-budget-${universe.id}`}>
          Orçamento: R$ 
        </div>{universe.totalBudget.toLocaleString()}
                </div>
          ...
          Tags: 
                ...
                <div className=" ">$2</div><button
                  ...
                  <button data-testid={`edit-universe-${universe.id}`} />
                    Editar
                  </button>
                  <button data-testid={`delete-universe-${universe.id}`} />
                    Excluir
                  </button>
      </div>
    </>
      ...
      {activeTab === "projects" && (
        <div data-testid="projects-tab" className="projects-tab">
        ...
        </div><div data-testid="projects-actions" className="projects-actions">
           
        </div>{selectedUniverse && (
              <div data-testid="selected-universe-info">
          Projetos do universo: 
        </div>{selectedUniverse.name}
                <button
          ...
          {showCreateProject && (
            <div
              ...
              <form
                ...
                <input
                  name="team"
                  placeholder="Equipe (separada por vírgula)"
                / />
                <button type="submit">Criar</button>
                <button
                  type="button"
        ...
        </div><div data-testid={`project-name-${project.id}`}>
          ...
          Orçamento: R$ 
        </div>{project.budget.toLocaleString()}
                </div>
          ...
          Equipe: 
        </div>{project.team.join(", ")}
                </div>
                <div className=" ">$2</div><button data-testid={`edit-project-${project.id}`} />
                    Editar
                  </button>
      ...
      {activeTab === "overview" && (
        ...
        </div><div data-testid="overview-content" className="overview-content">
           
        </div><h3>Visão Geral dos Universos</h3>
            <div data-testid="overview-metrics">
           
        </div><div>Total de Universos: {stats.totalUniverses}</div>
        ...
        </div><h4>Distribuição por Universo</h4>
              {(universes || []).map((universe) => (
                <div key={universe.id} data-testid={`breakdown-${universe.id}`}>
           
        </div>{universe.name}: {universe.projectsCount} projetos, R${" "}
                  {universe.totalBudget.toLocaleString()} de orçamento
                </div>
              ))}
            </div>
    </div>
...
describe("Universe Module", () => {
  ...
  it("should display universe statistics", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByTestId("stat-total-universes")).toHaveTextContent(
        "Universos: 3",);

      expect(screen.getByTestId("stat-active-universes")).toHaveTextContent(
        "Ativos: 2",);

      expect(screen.getByTestId("stat-total-projects")).toHaveTextContent(
        "Projetos: 3",);

      expect(screen.getByTestId("stat-active-projects")).toHaveTextContent(
        "Ativos: 2",);

      expect(screen.getByTestId("stat-total-budget")).toHaveTextContent(
  ...
  it("should display universe details", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId("universe-name-1")).toHaveTextContent(
        "Universo Principal",);

      expect(screen.getByTestId("universe-description-1")).toHaveTextContent(
        "Universo principal da empresa",);

      expect(screen.getByTestId("universe-status-1")).toHaveTextContent(
        "active",);

      expect(screen.getByTestId("universe-projects-count-1")).toHaveTextContent(
        "Projetos: 15",);

      expect(screen.getByTestId("universe-budget-1")).toHaveTextContent(
        "Orçamento: R$ 2.500.000",);

      expect(screen.getByTestId("universe-owner-1")).toHaveTextContent(
        "Proprietário: João Silva",);

      expect(screen.getByTestId("universe-tags-1")).toHaveTextContent(
        "Tags: principal, empresa",);
  ...
  it("should create new universe", async () => {
    ...
    const form = screen
      .getByTestId("create-universe-form")
      .querySelector("form")!;
    ...
    await waitFor(() => {
      ...
      expect(screen.getByTestId("universe-name-4")).toHaveTextContent(
        "Novo Universo",);
  ...
  it("should navigate to universe projects", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByTestId("selected-universe-info")).toHaveTextContent(
        "Projetos do universo: Universo Principal",);
  ...
  it("should display project details", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByTestId("project-progress-1")).toHaveTextContent(
        "Progresso: 75%",);

      expect(screen.getByTestId("project-team-1")).toHaveTextContent(
        "Equipe: João Silva, Ana Lima, Carlos Santos",);
  ...
  it("should create new project", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByTestId("project-name-4")).toHaveTextContent(
        "Novo Projeto",);
  ...
  it("should handle refresh action", async () => {
    ...
    await waitFor(
      () => {
        expect(
          screen.queryByTestId("universe-loading"),
