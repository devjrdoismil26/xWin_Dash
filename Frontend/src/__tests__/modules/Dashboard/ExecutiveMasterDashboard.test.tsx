...
const ExecutiveMasterDashboard = () => {
  ...
  return (
      ...
      </div><h1 data-testid="executive-title">Executive Master Dashboard</h1>
      
      <div data-testid="dashboard-header" className="dashboard-header">
           
        </div><div data-testid="dashboard-controls" className="dashboard-controls">
            ...
            <option value="custom">Custom</option></select><button data-testid="refresh-btn" onClick={ refreshData } />
            Refresh
          </button>
      ...
      <div data-testid="kpis-section" className="kpis-section">
        ...
        </div><h2>Key Performance Indicators</h2>
        <div data-testid="kpis-grid" className="kpis-grid">
           
        </div><div data-testid="kpi-revenue" className="kpi-card">
           
        </div><span data-testid="revenue-value">R$ {dashboardData.kpis.totalRevenue.toLocaleString()}</span>
            <span data-testid="revenue-label">Total Revenue</span></div><div data-testid="kpi-users" className="kpi-card">
           
        </div><span data-testid="users-value">{dashboardData.kpis.totalUsers.toLocaleString()}</span>
            <span data-testid="users-label">Total Users</span></div><div data-testid="kpi-projects" className="kpi-card">
           
        </div><span data-testid="projects-value">{dashboardData.kpis.activeProjects}</span>
            <span data-testid="projects-label">Active Projects</span></div><div data-testid="kpi-conversion" className="kpi-card">
           
        </div><span data-testid="conversion-value">{dashboardData.kpis.conversionRate}%</span>
            <span data-testid="conversion-label">Conversion Rate</span></div><div data-testid="kpi-satisfaction" className="kpi-card">
           
        </div><span data-testid="satisfaction-value">{dashboardData.kpis.customerSatisfaction}/5</span>
            <span data-testid="satisfaction-label">Customer Satisfaction</span></div><div data-testid="kpi-growth" className="kpi-card">
           
        </div><span data-testid="growth-value">{dashboardData.kpis.monthlyGrowth}%</span>
            <span data-testid="growth-label">Monthly Growth</span></div></div>

      <div data-testid="charts-section" className="charts-section">
           
        </div><h2>Analytics & Trends</h2>
        ...
        </div>{item.month}: R$ {item.revenue.toLocaleString()}
            </div>
          ))}
        </div>
      ...
      <div data-testid="activity-section" className="activity-section">
        ...
        </div>{(dashboardData.recentActivity || []).map(activity => (
            <div key={activity.id} data-testid={`activity-${activity.id}`} className="activity-item">
           
        </div><span data-testid={`activity-action-${activity.id}`}>{activity.action}</span>
