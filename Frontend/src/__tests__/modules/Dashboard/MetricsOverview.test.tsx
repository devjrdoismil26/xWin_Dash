...
const MetricsOverview = () => {
  ...
  return (
      ...
      </div><h1 data-testid="metrics-title">Metrics Overview</h1>
      
      <div data-testid="metrics-header" className="metrics-header">
           
        </div><div data-testid="metrics-controls" className="metrics-controls">
            ...
            <option value="1y">Last year</option></select><select
            ...
            <option value="conversions">Conversions</option></select><select
            data-testid="view-mode-selector"
            value={ viewMode }
            onChange={ (e) => setViewMode(e.target.value)  }>
            <option value="cards">Cards View</option>
            <option value="table">Table View</option>
            <option value="chart">Chart View</option></select><button data-testid="refresh-btn" onClick={ refreshMetrics } />
            Refresh
          </button>
      ...
      <div data-testid="overview-section" className="overview-section">
        ...
        </div><div data-testid="metric-total-users" className="metric-card">
           
        </div><span data-testid="total-users-value">{metrics.overview.totalUsers.toLocaleString()}</span>
            <span data-testid="total-users-label">Total Users</span>
            <span data-testid="total-users-growth">+{metrics.overview.userGrowth}%</span></div><div data-testid="metric-active-users" className="metric-card">
           
        </div><span data-testid="active-users-value">{metrics.overview.activeUsers.toLocaleString()}</span>
            <span data-testid="active-users-label">Active Users</span></div><div data-testid="metric-new-users" className="metric-card">
           
        </div><span data-testid="new-users-value">{metrics.overview.newUsers}</span>
            <span data-testid="new-users-label">New Users</span></div><div data-testid="metric-total-revenue" className="metric-card">
           
        </div><span data-testid="total-revenue-value">R$ {metrics.overview.totalRevenue.toLocaleString()}</span>
            <span data-testid="total-revenue-label">Total Revenue</span>
            <span data-testid="total-revenue-growth">+{metrics.overview.revenueGrowth}%</span></div><div data-testid="metric-monthly-revenue" className="metric-card">
           
        </div><span data-testid="monthly-revenue-value">R$ {metrics.overview.monthlyRevenue.toLocaleString()}</span>
            <span data-testid="monthly-revenue-label">Monthly Revenue</span></div><div data-testid="metric-conversion-rate" className="metric-card">
           
        </div><span data-testid="conversion-rate-value">{metrics.overview.conversionRate}%</span>
            <span data-testid="conversion-rate-label">Conversion Rate</span></div><div data-testid="metric-bounce-rate" className="metric-card">
           
        </div><span data-testid="bounce-rate-value">{metrics.overview.bounceRate}%</span>
            <span data-testid="bounce-rate-label">Bounce Rate</span></div><div data-testid="metric-session-duration" className="metric-card">
           
        </div><span data-testid="session-duration-value">{metrics.overview.avgSessionDuration}</span>
            <span data-testid="session-duration-label">Avg Session</span></div><div data-testid="metric-page-views" className="metric-card">
           
        </div><span data-testid="page-views-value">{metrics.overview.pageViews.toLocaleString()}</span>
            <span data-testid="page-views-label">Page Views</span></div><div data-testid="metric-unique-visitors" className="metric-card">
           
        </div><span data-testid="unique-visitors-value">{metrics.overview.uniqueVisitors.toLocaleString()}</span>
            <span data-testid="unique-visitors-label">Unique Visitors</span></div></div>
      ...
      <div data-testid="top-pages-section" className="top-pages-section">
        ...
        </div>{(metrics.topPages || []).map((page, index) => (
            <div key={index} data-testid={`top-page-${index}`} className="top-page-item">
           
        </div><span data-testid={`page-url-${index}`}>{page.page}</span>
