...
const Workflows = () => {
  const [workflows, setWorkflows] = React.useState([
    {
      ...
      description: "Workflow de teste 1",
    },
    {
      id: "2",
      name: "Workflow 2",
      ...
      description: "Workflow de teste 2",
    },
    {
      id: "3",
      name: "Workflow 3",
      ...
      description: "Workflow de teste 3",
    },
  ]);
  ...
  const createWorkflow = (workflowData: unknown) => {
    const newWorkflow = {
  ...
  const updateWorkflowStatus = (id: string, status: string) => {
    setWorkflows(
      (workflows || []).map((w) => (w.id === id ? { ...w, status } : w)),);};
  ...
  return (
      ...
      </div><h1>Workflows Management</h1>

      {/* Estatísticas */}
      <div data-testid="stats">
           
        </div><div>Total de workflows: {stats.total}</div>
      ...
      <div data-testid="filters">
        ...
        >
          ...
          <option value="completed">Concluído</option></select></div>
      ...
      <div data-testid="actions">
          ...
          onClick={() = />
            createWorkflow({
              ...
              status: "active",
              steps: 3,
              description: "Novo workflow de teste",
            })
  }
      ...
      <div data-testid="workflows-list">
        ...
        </div>{(filteredWorkflows || []).map((workflow) => (
          <div
            key={ workflow.id }
            data-testid={`workflow-${workflow.id}`}
            className="workflow-card">
           
        </div><h3>{workflow.name}</h3>
            ...
            <div>
                ...
                data-testid={`toggle-status-${workflow.id}`}
  >
                Alternar Status
              </button>
              <button
  ...
  >
              ...
              <button
                ...
                data-testid={`delete-${workflow.id}`}
  >
                Excluir
              </button>
      </div>
...
describe("Workflows Module", () => {
  ...
  it("should display workflow details correctly", () => {
    ...
    expect(
      screen.getByText("Descrição: Workflow de teste 1"),
    ).toBeInTheDocument();
