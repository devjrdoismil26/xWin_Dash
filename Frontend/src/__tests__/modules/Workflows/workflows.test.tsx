...
import WorkflowsDashboard from '../shared/components/WorkflowsDashboard';
import WorkflowsStats from '../shared/components/WorkflowsStats';
import WorkflowsFilters from '../shared/components/WorkflowsFilters';
import WorkflowsList from '../shared/components/WorkflowsList';
import WorkflowsGrid from '../shared/components/WorkflowsGrid';
...
vi.mock('@/components/ui/Card', () => ({
  default: ({ children, ...props }: unknown) => <div data-testid="card" {...props}>{children}</div>,
  Card.Content: ({ children, ...props }: unknown) => <div data-testid="card-content" {...props}>{children}</div>,
  Card.Header: ({ children, ...props }: unknown) => <div data-testid="card-header" {...props}>{children}</div>,
  Card.Title: ({ children, ...props }: unknown) => <h3 data-testid="card-title" {...props}>{children}</h3>
  }));

vi.mock('@/components/ui/Button', () => ({
  default: ({ children, onClick, ...props }: unknown) => (
    <button onClick={onClick} {...props}>{children}</button>
  )
  }));

vi.mock('@/components/ui/Input', () => ({
  default: ({ onChange, ...props }: unknown) => (
...
describe('Workflows Components', () => {
  ...
  describe('WorkflowsStats', () => { it('should render stats correctly', () => {
    ...
    it('should show loading state', () => { render(<WorkflowsStats loading={true } />);

      // Verificar se o loading está sendo exibido
    });

    it('should handle empty stats', () => { render(<WorkflowsStats stats={null } />);
