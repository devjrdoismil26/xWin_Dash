...
const SocialBuffer = () => {
  const [posts, setPosts] = React.useState([
    ...
    {
      id: "2",
      content: "Dicas de marketing digital",
      platform: "LinkedIn",
      status: "published",
  ...
  const createPost = (postData: unknown) => {
    const newPost = {
  ...
  const schedulePost = (id: string, scheduledAt: string) => {
    ...
    if (post) {
      ...
      setPosts(
        (posts || []).map((p) => (p.id === id ? { ...p, scheduledAt } : p)),);
  ...
  return (
      ...
      </div><h1>Social Buffer Management</h1>

      {/* Estatísticas */}
      <div data-testid="stats">
           
        </div><div>Total de posts: {stats.total}</div>
      ...
      <div data-testid="filters">
        ...
        >
          ...
          <option value="Facebook">Facebook</option></select><select
      ...
      <div data-testid="posts-list">
          ...
          <div
            ...
            {post.publishedAt && (
              <p>Publicado em: {new Date(post.publishedAt).toLocaleString()}</p>
            )}
            <div>
           
        </div>{post.status === "draft" && (
                <button
                  onClick={ () => schedulePost(post.id, "2024-01-25T15:00:00Z") }
...
describe("SocialBuffer Module", () => {
  ...
  it("should display post details correctly", () => {
    ...
    expect(
      screen.getByText("Conteúdo: Novo produto lançado! 🚀"),
    ).toBeInTheDocument();
