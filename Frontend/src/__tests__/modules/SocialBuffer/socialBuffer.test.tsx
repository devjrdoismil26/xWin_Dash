...
import SocialBufferDashboard from "../shared/components/SocialBufferDashboard";
import SocialBufferStats from "../shared/components/SocialBufferStats";
...
vi.mock("@/shared/components/ui/AdvancedAnimations", () => ({
  ...
  PageTransition: ({ children }: unknown) => <div>{children}</div>,
...
vi.mock("@/shared/components/ui/ResponsiveSystem", () => ({
  ResponsiveGrid: ({ children, cols, gap }: unknown) => (
    <div
...
vi.mock("@/shared/components/ui/Tooltip", () => ({
  default: ({ children, content }: unknown) => (
    <div title={content} className="tooltip">
            {children}
          </div>
  ),
}));
...
describe("SocialBufferService", () => {
  ...
  describe("Estatísticas Globais", () => {
    it("deve obter estatísticas globais", async () => {
      // Mock dos services
      vi.spyOn(
        socialBufferService.accounts,
        "getAccountsStats",
      ).mockResolvedValue(mockAccountsStats);

      vi.spyOn(socialBufferService.posts, "getPostsStats").mockResolvedValue(
        mockPostsStats,);

      vi.spyOn(
        socialBufferService.schedules,
        "getSchedulesStats",
      ).mockResolvedValue(mockSchedulesStats);

      vi.spyOn(
        socialBufferService.analytics,
        "getBasicMetrics",
      ).mockResolvedValue(mockBasicMetrics);
  ...
  describe("Cache", () => {
    ...
    it("deve obter estatísticas de todos os caches", () => {
      const getCacheStatsSpy = vi
        .spyOn(socialBufferService.accounts, "getCacheStats")
  ...
  describe("Sincronização", () => {
    it("deve executar sincronização completa", async () => {
      const syncSpy = vi
        .spyOn(socialBufferService.accounts, "syncAllAccounts")
...
describe("SocialBuffer Components", () => {
  ...
  const renderWithQueryClient = (component: React.ReactElement) => { return render(
  ...
  describe("SocialBufferDashboard", () => {
    it("deve renderizar o dashboard com dados", async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          screen.getByText("Visão geral das suas métricas e performance"),
        ).toBeInTheDocument();
  ...
  describe("SocialBufferStats", () => {
    ...
    it("deve exibir métricas de engajamento", async () => { renderWithQueryClient(<SocialBufferStats showDetails={true } />);
    ...
    it("deve exibir performance por plataforma", async () => { renderWithQueryClient(<SocialBufferStats showDetails={true } />);

      await waitFor(() => {
        expect(
          screen.getByText("Performance por Plataforma"),
        ).toBeInTheDocument();
    ...
    it("deve exibir interações", async () => { renderWithQueryClient(<SocialBufferStats showDetails={true } />);
