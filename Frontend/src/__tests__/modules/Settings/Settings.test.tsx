...
const Settings = () => {
  const [settings, setSettings] = React.useState([
    ...
    },
    {
      ...
      category: "api",
      type: "number",
      description: "Timeout da API (segundos)",
    },
  ]);
  ...
  const resetAllSettings = () => {
    ...
    setSettings(
      (settings || []).map((s) => ({
        ...s,
        value: defaultValues[s.key] || s.value,
      })),);};
  ...
  const getStats = () => {
    ...
    const appearance = (settings || []).filter(
      (s) => s.category === "appearance",
    ).length;
    const files = (settings || []).filter((s) => s.category === "files").length;
    const notifications = (settings || []).filter(
  ...
  return (
      ...
      </div><h1>Settings Management</h1>

      {/* Estatísticas */}
      <div data-testid="stats">
           
        </div><div>Total de configurações: {stats.total}</div>
      ...
      <div data-testid="filters">
        ...
        >
          ...
          <option value="api">API</option></select><select
          value={ selectedType }
        ...
        >
          ...
          <option value="select">Seleção</option></select></div>

      {/* Configurações avançadas */}
      {showAdvanced && (
        <div data-testid="advanced-settings" className="advanced-settings">
           
        </div><h3>Configurações Avançadas</h3>
          <div>
           
        </div><label>Debug Mode:</label>
            <input type="checkbox" data-testid="debug-mode" / /></div><div>
            ...
            <div>
              ...
              {setting.type === "boolean" && (
                <input
                  ...
                  onChange={ (e) = />
                    updateSetting(setting.id, e.target.checked.toString())
   }
                  data-testid={`input-${setting.id}`} />
              )}
              {setting.type === "select" && (
  ...
  >
      ...
      {isLoading && (
        <div data-testid="loading">Carregando configurações...</div>
      )}
    </div>);};
...
describe("Settings Module", () => {
  ...
  it("should display setting details correctly", () => {
    ...
    expect(
      screen.getByText("Descrição: Nome da aplicação"),
    ).toBeInTheDocument();
