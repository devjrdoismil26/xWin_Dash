 ...
 *
 * @description
 * Sistema centralizado de design tokens que define variantes, tamanhos,
 * cores, transições e efeitos visuais para todos os componentes da
 * aplicação. Fornece consistência visual e facilita manutenção do design system.
 *
 * Funcionalidades principais:
 * - Transições e animações padronizadas
 * - Efeitos visuais (sombras, bordas, outlines)
 * - Variantes de componentes (cores, estilos)
 * - Tamanhos padronizados (sm, md, lg, xl)
 * - Funções utilitárias para classes CSS
 *
 * @module components/ui/design-tokens
 * @since 1.0.0
 *
 * @example
 * ```tsx
 * import { getVariantClasses, getSizeClasses, ENHANCED_TRANSITIONS } from '@/shared/components/ui/design-tokens';
 *
 * // Usar variantes
 * const buttonClasses = getVariantClasses('primary', 'button');

 *
 * // Usar tamanhos
 * const iconClasses = getSizeClasses('md', 'icon');

 *
 * // Usar transições
 * const cardClasses = ENHANCED_TRANSITIONS.card;
 * ```
 */

// ===== TRANSIÇÕES E ANIMAÇÕES =====
export const ENHANCED_TRANSITIONS: unknown = {
  // Transições base
...
// ===== EFEITOS VISUAIS =====
export const VISUAL_EFFECTS: unknown = {
  // Sombras melhoradas
  shadows: {
...
// ===== MAPEAMENTO DE CORES =====
export const VARIANT_COLORS: unknown = {
  default: {
    bg: "bg-gray-100",
    text: "text-gray-600",
...
export const SIZE_CLASSES: unknown = {
...
export const BADGE_VARIANTS: unknown = {
...
// ===== UTILITÁRIOS =====
export const getVariantClasses = (variant: ComponentVariant,
  type: "solid" | "outline" | "ghost" | "card" = "solid",
) => {
...
export const getSizeClasses = (size: ComponentSize,
  component: keyof typeof SIZE_CLASSES,
) => {
  return SIZE_CLASSES[component][size];};

// ===== MAPEAMENTO DE ESTADOS DE ERRO =====
export const ERROR_VARIANTS: unknown = {
