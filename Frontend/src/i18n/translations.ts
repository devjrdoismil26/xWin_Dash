 ...
 *
 * @description
 * Tradução unificada para todos os módulos da aplicação.
...
/**
 * Idioma suportado pela aplicação
 *
 ...
 * @description Define todas as chaves de tradução disponíveis na aplicação
 */
export interface Translation {
  // === NAVEGAÇÃO E LAYOUT ===
  /** Traduções relacionadas à navegação e layout */
  navigation: {
    dashboard: string;
  leads: string;
  campaigns: string;
 ...
 *
 * @description
 * Contém todas as traduções da aplicação organizadas por idioma.
 * Chaves de tradução podem incluir placeholders (ex: "{min}", "{max}") que são substituídos dinamicamente.
 *
 * @constant {Record<Language, Translation>} translations
 * @example
 * ```tsx
 * const t = translations['pt'];
 * console.log(t.navigation.dashboard); // "Dashboard"
 * console.log(t.forms.minLength); // "Mínimo de {min} caracteres"
 * ```
 */
