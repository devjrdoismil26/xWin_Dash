/**
 * Hook useSidebarRoutes - Rotas da Sidebar
 *
 * @description
 * Hook que fornece as rotas configuradas para a sidebar da aplicação,
 * incluindo menus principais, submenus, ícones, badges e informações
 * de categorização. Configura rotas baseadas no estado da aplicação
 * e permissões do usuário.
 *
 * Funcionalidades principais:
 * - Rotas principais e submenus
 * - Ícones por rota
 * - Badges e indicadores (novo, pro, beta)
 * - Categorização de rotas
 * - Filtragem por permissões
 * - Ordenação e priorização
 *
 * @module hooks/useSidebarRoutes
 * @since 1.0.0
 *
 * @example
 * ```tsx
 ...
 *
 * const { menuItems, primaryRoutes, allRoutes } = useSidebarRoutes();
...
import { Home, Brain, Users, Mail, Share2, MessageCircle, Target, BarChart3, Image, Package, Activity, Settings, Zap, Star, Clock, TrendingUp, Sparkles, FileText, Palette, Bot, Database, Globe, Shield, Workflow,  } from 'lucide-react';

/**
 * Interface de item de submenu
 *
 * @description
 * Define a estrutura de um item de submenu na sidebar.
 *
 * @interface SubMenuItem
 * @property {string} name - Nome do submenu
 * @property {string} route - Rota do submenu
 * @property {React.ReactNode} [icon] - Ícone do submenu
 * @property {string} [badge] - Badge do submenu
 * @property {string} [description] - Descrição do submenu
 * @property {boolean} [isNew] - Se é novo
 * @property {boolean} [isPro] - Se é pro
 * @property {boolean} [isBeta] - Se é beta
 */
interface SubMenuItem {
  name: string;
  route: string;
...
export const useSidebarRoutes = () => {
  ...
  const getActiveItem = useMemo(() => {
    return menuItems.find(
      (item: unknown) =>
        ...
        item.subItems?.some((sub: unknown) => url.startsWith(sub.route)),);
  ...
  const getActiveSubmenu = useMemo(() => {
    ...
    return getActiveItem.subItems.find((sub: unknown) => url.startsWith(sub.route));
