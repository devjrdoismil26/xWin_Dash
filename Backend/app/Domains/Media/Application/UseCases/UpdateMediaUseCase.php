<?php

namespace App\Domains\Media\Application\UseCases;

use App\Domains\Media\Application\Commands\UpdateMediaCommand;
use App\Domains\Media\Application\Handlers\UpdateMediaHandler;
use App\Domains\Media\Domain\Services\CrossModuleValidationServiceInterface;
use App\Domains\Shared\Domain\Services\EventDispatcherInterface;
use App\Domains\Media\Domain\Events\MediaUpdatedEvent;
use Illuminate\Support\Facades\Log;

class UpdateMediaUseCase
{
    public function __construct(
        private UpdateMediaHandler $updateHandler,
        private CrossModuleValidationServiceInterface $crossModuleValidator,
        private EventDispatcherInterface $eventDispatcher
    ) {
    }

    public function execute(UpdateMediaCommand $command): array
    {
        try {
            // Validar comando
            if (!$command->isValid()) {
                return [
                    'success' => false,
                    'errors' => $command->getValidationErrors(),
                    'message' => 'Dados inválidos para atualização da mídia'
                ];
            }

            // Validar regras de negócio cross-module
            $crossModuleValidation = $this->crossModuleValidator->validateMediaUpdate($command);
            if (!$crossModuleValidation['valid']) {
                return [
                    'success' => false,
                    'errors' => $crossModuleValidation['errors'],
                    'message' => 'Regras de negócio não atendidas'
                ];
            }

            // Executar atualização
            $this->updateHandler->handle($command);

            // Disparar evento de atualização
            $this->eventDispatcher->dispatch(
                new MediaUpdatedEvent(
                    $command->mediaId,
                    $command->userId,
                    $command->toArray()
                )
            );

            Log::info('Media updated successfully via Use Case', [
                'media_id' => $command->mediaId,
                'user_id' => $command->userId
            ]);

            return [
                'success' => true,
                'message' => 'Mídia atualizada com sucesso',
                'data' => [
                    'media_id' => $command->mediaId,
                    'updated_at' => now()->toISOString()
                ]
            ];
        } catch (\Throwable $exception) {
            Log::error('Error in UpdateMediaUseCase', [
                'error' => $exception->getMessage(),
                'media_id' => $command->mediaId,
                'user_id' => $command->userId
            ]);

            return [
                'success' => false,
                'errors' => ['Erro interno durante atualização da mídia'],
                'message' => 'Falha ao atualizar mídia'
            ];
        }
    }
}
