...
/**
 * 🚀 Facebook Ads Creative Service
 *
 * Serviço especializado para operações de criativos no Facebook Ads
 * Responsabilidade única: gerenciar criativos e anúncios
 */
class FacebookAdsCreativeService extends BaseExternalApiService
{
    ...
    public function getAd(string $adId, array $fields = []): array
    {
        ...
        $params = [
            ...
            'fields' => implode(',', $fields ?: ['id', 'name', 'adset_id', 'status', 'creative', 'created_time', 'updated_time'])
        ];
     ...
     * Cria um criativo
     */
    public function createCreative(array $creativeData): array
    {
        if (!$this->adAccountId) {
            throw new FacebookAdsApiException('ID da conta de anúncios não configurado.');
        }

        $endpoint = "/act_{$this->adAccountId}/adcreatives";
        $creativeData['access_token'] = $this->accessToken;
    ...
    /**
     * Obtém informações de um criativo
     */
    public function getCreative(string $creativeId, array $fields = []): array
    {
        $endpoint = "/{$creativeId}";
        $params = [
            'access_token' => $this->accessToken,
            'fields' => implode(',', $fields ?: ['id', 'name', 'object_story_spec', 'body', 'image_url', 'link_url', 'created_time', 'updated_time'])
        ];
     ...
     */
    public function listCreatives(array $filters = [], int $limit = 100): array
    {
        $endpoint = "/act_{$this->adAccountId}/adcreatives";
        $params = [
            'access_token' => $this->accessToken,
            'limit' => $limit,
            'fields' => 'id,name,object_story_spec,body,image_url,link_url,created_time,updated_time'
        ];
