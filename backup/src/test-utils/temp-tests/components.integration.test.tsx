...
import SocialAccountsManager from '../../components/functionality/SocialAccountsManager';
import SocialPostsManager from '../../components/functionality/SocialPostsManager';
import SocialBufferLoadingSkeleton from '../../components/ui/SocialBufferLoadingSkeleton';
import SocialBufferErrorState from '../../components/ui/SocialBufferErrorState';
import SocialBufferEmptyState from '../../components/ui/SocialBufferEmptyState';
import SocialBufferSuccessState from '../../components/ui/SocialBufferSuccessState';
...
jest.mock('@/components/ui/AdvancedAnimations', () => ({
  Animated: ({ children, delay }: any) => <div data-testid="animated" data-delay={delay}>{children}</div>
}));

jest.mock('@/components/ui/ResponsiveSystem', () => ({
  ResponsiveGrid: ({ children, columns, gap }: any) => (
    <div data-testid="responsive-grid" data-columns={JSON.stringify(columns)} data-gap={gap}>
      {children}
    </div>
  )
}));
...
const mockPostsStore = (overrides = {}) => ({
  posts: [
    {
      id: '1',
      title: 'Test Post',
...
describe('SocialBuffer Components Integration Tests', () => {
  ...
  describe('SocialAccountsManager Integration', () => {
    it('deve renderizar corretamente com dados', () => {
      ...
      (require('../../hooks/useSocialBufferUI').useAccountsUI as jest.Mock).mockReturnValue(mockUI);
      (require('../../hooks/useAccountsStore').useAccountsStore as jest.Mock).mockReturnValue(mockStore);
    ...
    it('deve deletar conta com confirmação', async () => {
      ...
      expect(window.confirm).toHaveBeenCalledWith('Tem certeza que deseja desconectar a conta Test Account?');
  ...
  describe('SocialPostsManager Integration', () => {
    it('deve renderizar corretamente com dados', () => {
      ...
      (require('../../hooks/useSocialBufferUI').usePostsUI as jest.Mock).mockReturnValue(mockUI);
      (require('../../hooks/usePostsStore').usePostsStore as jest.Mock).mockReturnValue(mockStore);
    ...
    it('deve alternar entre visualização grid e lista', async () => {
      ...
      expect(screen.getByTestId('responsive-grid')).toHaveAttribute('data-columns', '{"sm":1,"md":2,"lg":3}');
    });
  ...
  describe('UI Components Integration', () => {
    ...
    it('deve renderizar empty state com ações', async () => {
      const user = userEvent.setup();
  ...
  describe('Complete Integration Flow', () => {
    it('deve completar fluxo completo de criação de post', async () => {
      ...
      // 2. Simular sucesso
      await waitFor(() => {
        expect(mockUI.handleSuccess).toHaveBeenCalledWith('Post criado com sucesso!');
      });
