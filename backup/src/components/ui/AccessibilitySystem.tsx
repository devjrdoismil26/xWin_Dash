/**
 * Accessibility System - xWin Dash
 * Sistema completo de acessibilidade WCAG 2.1 AA/AAA
 */
...
export const AccessibilityProvider: React.FC<AccessibilityProviderProps> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const checkHighContrast = () => {
      ...
      setPreferences(prev => ({ ...prev, highContrast: isHighContrast }));
    };
    ...
    const checkScreenReader = () => {
      ...
      const isScreenReader = hasVoiceOver || hasJAWS || hasNVDA || 
...
export const AccessibleButton: React.FC<AccessibleButtonProps> = ({
  ...
  const handleClick = useCallback((e: React.MouseEvent<HTMLButtonElement>) => {
    if (announce) {
      announceToScreenReader('Botão ativado');
    }
    onClick?.(e);
    ...
    >
      ...
      <FocusManager trap restoreOnUnmount>
        ...
        >
          ...
          {description && (
            <p id={descriptionId} className="text-sm text-gray-600 dark:text-gray-400 mb-4">
              {description}
            </p>
          )}
