...
describe('Security Tests', () => {
  ...
  describe('Password Security', () => {
    ...
    it('should verify passwords correctly', () => {
      ...
      expect(mockSecurityUtils.verifyPassword(password, hashedPassword)).toBe(true);
      expect(mockSecurityUtils.verifyPassword('wrongPassword', hashedPassword)).toBe(false);
    });
  ...
  describe('JWT Security', () => {
    it('should generate valid JWT tokens', () => {
      ...
      const token = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOiIxMjMiLCJyb2xlIjoidXNlciJ9.signature';
  ...
  describe('Rate Limiting', () => {
    it('should implement rate limiting for API calls', () => {
      ...
      const checkRateLimit = (ip: string) => {
        ...
        // Remove old requests
        const recentRequests = userRequests.filter((time: number) => now - time < rateLimiter.window);
