...
export const mockRoute = (routes: Record<string, ((params?: any) => string) | string>) => {
...
export const mockPaginatedResponse = <T>(
  ...
  return {
    ...
    meta: {
      ...
      to: Math.min(currentPage * perPage, total || data.length)
    },
    links: {
      first: '/api/resource?page=1',
      last: `/api/resource?page=${Math.ceil((total || data.length) / perPage)}`,
      prev: currentPage > 1 ? `/api/resource?page=${currentPage - 1}` : null,
      next: currentPage < Math.ceil((total || data.length) / perPage) ? `/api/resource?page=${currentPage + 1}` : null
    }
  };
};
