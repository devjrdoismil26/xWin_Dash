...
const Analytics = () => {
  const [analytics, setAnalytics] = React.useState({
    ...
    devices: [
      { device: 'Desktop', visitors: 6000, percentage: 70.6 },
  ...
  return (
    <div data-testid="analytics-module" className="analytics-module">
      ...
      <div data-testid="analytics-overview" className="analytics-overview">
        <div data-testid="metric-pageviews" className="metric-card">
          <span data-testid="pageviews-value">{analytics.overview.pageViews.toLocaleString()}</span>
          <span data-testid="pageviews-label">Page Views</span>
        </div>
        <div data-testid="metric-visitors" className="metric-card">
          <span data-testid="visitors-value">{analytics.overview.uniqueVisitors.toLocaleString()}</span>
          <span data-testid="visitors-label">Unique Visitors</span>
        </div>
        <div data-testid="metric-bounce" className="metric-card">
          <span data-testid="bounce-value">{analytics.overview.bounceRate}%</span>
          <span data-testid="bounce-label">Bounce Rate</span>
        </div>
        <div data-testid="metric-session" className="metric-card">
          <span data-testid="session-value">{analytics.overview.avgSessionDuration}</span>
          <span data-testid="session-label">Avg Session</span>
        </div>
        <div data-testid="metric-conversion" className="metric-card">
          <span data-testid="conversion-value">{analytics.overview.conversionRate}%</span>
          <span data-testid="conversion-label">Conversion Rate</span>
        </div>
        <div data-testid="metric-revenue" className="metric-card">
          <span data-testid="revenue-value">R$ {analytics.overview.revenue.toLocaleString()}</span>
          <span data-testid="revenue-label">Revenue</span>
        </div>
      </div>

      <div data-testid="traffic-chart" className="traffic-chart">
        <h3>Traffic Overview</h3>
        {analytics.traffic.map((item, index) => (
          <div key={index} data-testid={`traffic-item-${index}`}>
            {item.date}: {item.visitors} visitors, {item.pageViews} page views
          </div>
        ))}
      </div>
      ...
      <div data-testid="devices-chart" className="devices-chart">
        ...
        {analytics.devices.map((device, index) => (
          <div key={index} data-testid={`device-item-${index}`}>
            {device.device}: {device.visitors} visitors ({device.percentage}%)
          </div>
        ))}
      </div>
    </div>
...
describe('Analytics Module', () => {
  ...
  it('should display devices data', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByTestId('device-item-0')).toHaveTextContent('Desktop: 6000 visitors (70.6%)');
