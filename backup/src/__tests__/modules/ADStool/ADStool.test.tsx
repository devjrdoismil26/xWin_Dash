  ...
  return (
    <div data-testid="adstool-module">
      ...
      <div data-testid="stats">
        ...
        <p>Orçamento total: R$ {campaigns.reduce((sum, c) => sum + c.budget, 0).toFixed(2)}</p>
        <p>Gasto total: R$ {campaigns.reduce((sum, c) => sum + c.spent, 0).toFixed(2)}</p>
        <p>Cliques totais: {campaigns.reduce((sum, c) => sum + c.clicks, 0)}</p>
        <p>Impressões totais: {campaigns.reduce((sum, c) => sum + c.impressions, 0).toLocaleString()}</p>
      </div>
      ...
      {showCreateForm && (
        <div data-testid="create-form">
          <form onSubmit={(e) => { e.preventDefault(); createCampaign(); }}>
            <input
              type="text"
      ...
      <div data-testid="campaigns-list">
        {filteredCampaigns.map(campaign => (
          <div key={campaign.id} data-testid={`campaign-${campaign.id}`}>
            ...
            <p>Status: <span style={{ color: getStatusColor(campaign.status) }}>{campaign.status}</span></p>
            
            <div data-testid={`actions-${campaign.id}`}>
              <button
...
describe('ADStool Module', () => {
  ...
  it('should display campaign statistics', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('Orçamento total: R$ 2400.00')).toBeInTheDocument();
      ...
      expect(screen.getByText('Impressões totais: 105.000')).toBeInTheDocument();
    });
  });
  ...
  it('should show empty state when no campaigns match filters', async () => {
    ...
    const searchInput = screen.getByTestId('search-input');
    fireEvent.change(searchInput, { target: { value: 'Campanha Inexistente' } });
