...
const AI = () => {
  const [messages, setMessages] = React.useState([
    { id: 1, type: 'user', content: 'Hello AI', timestamp: '2024-01-16T10:00:00Z' },
    { id: 2, type: 'assistant', content: 'Hello! How can I help you today?', timestamp: '2024-01-16T10:00:01Z' }
  ]);
  ...
  return (
    <div data-testid="ai-module" className="ai-module">
      <h1 data-testid="ai-title">AI Assistant</h1>
      ...
      <div data-testid="chat-messages" className="chat-messages">
        {messages.map(message => (
          <div key={message.id} data-testid={`message-${message.id}`} className={`message ${message.type}`}>
            <span data-testid={`message-content-${message.id}`}>{message.content}</span>
            <span data-testid={`message-timestamp-${message.id}`}>{message.timestamp}</span>
          </div>
        ))}
      </div>
...
describe('AI Module', () => {
  ...
  it('should handle message sending', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('AI Response to: Test message')).toBeInTheDocument();
    }, { timeout: 2000 });
  });
