...
const AIChat = () => {
  const [messages, setMessages] = React.useState([
    ...
    },
    {
      id: 3,
      type: 'ai',
      content: 'Based on the latest data, you have 5 active projects with 3 completed this month.',
  ...
  const [chatHistory, setChatHistory] = React.useState([
    { id: 1, title: 'Project Status Discussion', lastMessage: 'What is the current status of our projects?', timestamp: '2024-01-15 10:01:00' },
    { id: 2, title: 'Revenue Analysis', lastMessage: 'Can you analyze our revenue trends?', timestamp: '2024-01-14 15:30:00' }
  ]);
  ...
  return (
    <div data-testid="ai-chat" className="ai-chat">
      <h1 data-testid="ai-chat-title">AI Chat Assistant</h1>
      
      <div data-testid="ai-chat-header" className="ai-chat-header">
        <div data-testid="provider-selector" className="provider-selector">
        ...
        <div data-testid="chat-actions" className="chat-actions">
          <button 
          ...
          <div data-testid="history-list" className="history-list">
            {chatHistory.map(chat => (
              ...
              >
                ...
                <div data-testid={`history-last-message-${chat.id}`}>{chat.lastMessage}</div>
                <div data-testid={`history-timestamp-${chat.id}`}>{chat.timestamp}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div data-testid="messages-container" className="messages-container">
          {messages.map(message => (
            ...
            >
              ...
              <div data-testid={`message-timestamp-${message.id}`}>{message.timestamp}</div>
            </div>
          ))}
      ...
      <div data-testid="chat-info" className="chat-info">
        <span data-testid="provider-status">Provider: {providers.find(p => p.id === selectedProvider)?.name}</span>
        <span data-testid="message-count">Messages: {messages.length}</span>
        <span data-testid="active-chat">Active Chat: {activeChatId || 'New Chat'}</span>
      </div>
    </div>
  );
};
...
describe('AIChat Module', () => {
  ...
  it('should display chat history', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByTestId('history-title-1')).toHaveTextContent('Project Status Discussion');
      expect(screen.getByTestId('history-last-message-1')).toHaveTextContent('What is the current status of our projects?');
    });
  });
  ...
  it('should load chat from history', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('active-chat')).toHaveTextContent('Active Chat: 1');
    });
  });
  ...
  it('should display message content correctly', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('message-content-1')).toHaveTextContent('Hello! How can I help you today?');
      expect(screen.getByTestId('message-content-2')).toHaveTextContent('What is the current status of our projects?');
      expect(screen.getByTestId('message-content-3')).toHaveTextContent('Based on the latest data, you have 5 active projects with 3 completed this month.');
    });
  });
  ...
  it('should send message on button click', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByTestId('message-content-4')).toHaveTextContent('Test message');
    });
  });
  ...
  it('should start new chat', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('active-chat')).toHaveTextContent('Active Chat: New Chat');
    });
  });
  ...
  it('should display chat info', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByTestId('provider-status')).toHaveTextContent('Provider: GPT-4');
