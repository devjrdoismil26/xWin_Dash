...
const Projects = () => {
  const [projects, setProjects] = React.useState([
    ...
    },
    {
      ...
      spent: 7500,
      team: [
        { id: 3, name: 'Pedro Costa', role: 'Marketer' }
      ],
      tasks: [
  ...
  const updateProject = (id: number, updates: any) => {
  ...
  return (
    <div data-testid="projects-module" className="projects-module">
      <h1 data-testid="projects-title">Projects Management</h1>
      ...
      <div data-testid="projects-list" className="projects-list">
        {filteredProjects.map(project => (
          <div key={project.id} data-testid={`project-${project.id}`} className="project-item">
            ...
            <div data-testid={`project-description-${project.id}`}>{project.description}</div>
            <div data-testid={`project-status-${project.id}`}>{project.status}</div>
            <div data-testid={`project-progress-${project.id}`}>{project.progress}%</div>
            <div data-testid={`project-budget-${project.id}`}>R$ {project.budget}</div>
            <div data-testid={`project-spent-${project.id}`}>R$ {project.spent}</div>
            <div data-testid={`project-team-count-${project.id}`}>Team: {project.team.length}</div>
            <div data-testid={`project-tasks-count-${project.id}`}>Tasks: {project.tasks.length}</div>
            <button 
...
describe('Projects Module', () => {
  ...
  it('should display projects data', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('Projeto de desenvolvimento')).toBeInTheDocument();
  ...
  it('should display project metrics', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByTestId('project-budget-1')).toHaveTextContent('R$ 50000');
      expect(screen.getByTestId('project-spent-1')).toHaveTextContent('R$ 37500');
      expect(screen.getByTestId('project-team-count-1')).toHaveTextContent('Team: 2');
      expect(screen.getByTestId('project-tasks-count-1')).toHaveTextContent('Tasks: 2');
    });
  });
  ...
  it('should display projects count', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('projects-count')).toHaveTextContent('Total: 2');
    });
  });
