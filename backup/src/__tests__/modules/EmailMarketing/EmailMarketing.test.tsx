...
const EmailMarketing = () => {
  ...
  const scheduleCampaign = (id, scheduledFor) => {
    setCampaigns(campaigns.map(campaign => 
      campaign.id === id ? { ...campaign, status: 'scheduled', scheduledFor } : campaign
    ));
  };

  return (
    <div data-testid="email-marketing-module" className="email-marketing-module">
      <h1 data-testid="email-marketing-title">Email Marketing</h1>
      ...
      {activeTab === 'campaigns' && (
        <div data-testid="campaigns-section">
          <div data-testid="campaigns-header" className="campaigns-header">
            <div data-testid="campaigns-stats" className="campaigns-stats">
              <span data-testid="total-campaigns">Total: {campaigns.length}</span>
              <span data-testid="active-campaigns">Active: {campaigns.filter(c => c.status === 'active').length}</span>
              <span data-testid="scheduled-campaigns">Scheduled: {campaigns.filter(c => c.status === 'scheduled').length}</span>
            </div>
          ...
          <div data-testid="campaigns-filters" className="campaigns-filters">
            <input
          ...
          {editingCampaign && (
            <div data-testid="edit-form" className="edit-form">
              ...
              <form onSubmit={(e) => {
                ...
                <input name="name" defaultValue={editingCampaign.name} required />
                <input name="subject" defaultValue={editingCampaign.subject} required />
          ...
          {isLoading && <div data-testid="campaigns-loading">Loading campaigns...</div>}

          <div data-testid="campaigns-list" className="campaigns-list">
            {filteredCampaigns.map(campaign => (
              <div key={campaign.id} data-testid={`campaign-${campaign.id}`} className="campaign-card">
                <div data-testid={`campaign-name-${campaign.id}`}>{campaign.name}</div>
                <div data-testid={`campaign-subject-${campaign.id}`}>{campaign.subject}</div>
                <div data-testid={`campaign-status-${campaign.id}`}>{campaign.status}</div>
                <div data-testid={`campaign-recipients-${campaign.id}`}>Recipients: {campaign.recipients}</div>
                <div data-testid={`campaign-open-rate-${campaign.id}`}>Open Rate: {campaign.openRate}%</div>
                <div data-testid={`campaign-click-rate-${campaign.id}`}>Click Rate: {campaign.clickRate}%</div>
                
                <div data-testid={`campaign-actions-${campaign.id}`} className="campaign-actions">
                  <button 
                  ...
                  {campaign.status === 'draft' && (
                    <button 
                      ...
                      onClick={() => scheduleCampaign(campaign.id, '2024-01-25')}
                    >
                      Schedule
                    </button>
                  )}
      ...
      {activeTab === 'analytics' && (
        <div data-testid="analytics-section">
          ...
          <div data-testid="analytics-overview" className="analytics-overview">
            <div data-testid="avg-open-rate">Average Open Rate: 15.2%</div>
...
describe('EmailMarketing Module', () => {
  ...
  it('should display campaign details', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('campaign-name-1')).toHaveTextContent('Welcome Campaign');
      expect(screen.getByTestId('campaign-subject-1')).toHaveTextContent('Welcome to our platform!');
      expect(screen.getByTestId('campaign-status-1')).toHaveTextContent('active');
      expect(screen.getByTestId('campaign-recipients-1')).toHaveTextContent('Recipients: 1500');
      expect(screen.getByTestId('campaign-open-rate-1')).toHaveTextContent('Open Rate: 25.5%');
    });
  });
  ...
  it('should display campaigns statistics', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByTestId('total-campaigns')).toHaveTextContent('Total: 3');
      expect(screen.getByTestId('active-campaigns')).toHaveTextContent('Active: 1');
      expect(screen.getByTestId('scheduled-campaigns')).toHaveTextContent('Scheduled: 1');
    });
  });
  ...
  it('should create new campaign', async () => {
    ...
    fireEvent.change(subjectInput, { target: { value: 'New campaign subject' } });
    ...
    await waitFor(() => {
      ...
      expect(screen.getByTestId('campaign-name-4')).toHaveTextContent('New Campaign');
    });
  });
  ...
  it('should update campaign', async () => {
    ...
    fireEvent.change(nameInput, { target: { value: 'Updated Welcome Campaign' } });

    const updateBtn = screen.getByText('Update');
    fireEvent.click(updateBtn);

    await waitFor(() => {
      expect(screen.getByTestId('campaign-name-1')).toHaveTextContent('Updated Welcome Campaign');
    });
  });
  ...
  it('should schedule campaign', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('campaign-status-3')).toHaveTextContent('scheduled');
    });
  });
  ...
  it('should display analytics', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('analytics-section')).toBeInTheDocument();
      expect(screen.getByTestId('avg-open-rate')).toHaveTextContent('Average Open Rate: 15.2%');
      expect(screen.getByTestId('avg-click-rate')).toHaveTextContent('Average Click Rate: 4.1%');
    });
  });
