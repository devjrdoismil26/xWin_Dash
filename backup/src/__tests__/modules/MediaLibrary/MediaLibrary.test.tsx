...
const MediaLibrary = () => {
  const [mediaFiles, setMediaFiles] = React.useState([
    { id: '1', name: 'image1.jpg', type: 'image', size: 1024000, url: '/media/image1.jpg', tags: ['product', 'marketing'], uploadedAt: '2024-01-20T10:00:00Z' },
    { id: '2', name: 'video1.mp4', type: 'video', size: 5120000, url: '/media/video1.mp4', tags: ['tutorial', 'demo'], uploadedAt: '2024-01-19T15:30:00Z' },
    { id: '3', name: 'document1.pdf', type: 'document', size: 256000, url: '/media/document1.pdf', tags: ['manual', 'guide'], uploadedAt: '2024-01-21T09:15:00Z' }
  ]);
  ...
  const updateFileTags = (id: string, tags: string[]) => {
    setMediaFiles(mediaFiles.map(f => f.id === id ? { ...f, tags } : f));
  };
  ...
  const getStats = () => {
    ...
    const documents = mediaFiles.filter(f => f.type === 'document').length;
  ...
  return (
    <div data-testid="medialibrary-module">
      ...
      <div data-testid="stats">
        <div>Total de arquivos: {stats.total}</div>
      ...
      <div data-testid="controls">
        ...
        <div data-testid="view-controls">
          <button onClick={() => setViewMode('grid')} data-testid="grid-view" className={viewMode === 'grid' ? 'active' : ''}>
            Grade
          </button>
          <button onClick={() => setViewMode('list')} data-testid="list-view" className={viewMode === 'list' ? 'active' : ''}>
            Lista
          </button>
        </div>
      </div>
      ...
      <div data-testid="actions">
        <button onClick={() => uploadFile({ name: 'newfile.jpg', type: 'image', size: 512000, url: '/media/newfile.jpg', tags: ['new'] })} data-testid="upload-file-btn">
          Upload Arquivo
        </button>
        <button onClick={refreshFiles} data-testid="refresh-btn" disabled={isLoading}>
      ...
      <div data-testid={`media-${viewMode}`} className={`media-${viewMode}`}>
        {filteredMedia.map(file => (
          <div key={file.id} data-testid={`file-${file.id}`} className="media-file">
            <h3>{file.name}</h3>
            <p>Tipo: {file.type}</p>
            ...
            <div>
              <button onClick={() => updateFileTags(file.id, [...file.tags, 'updated'])} data-testid={`update-tags-${file.id}`}>
                Atualizar Tags
              </button>
              <button onClick={() => deleteFile(file.id)} data-testid={`delete-${file.id}`}>
                Excluir
              </button>
            </div>
          </div>
