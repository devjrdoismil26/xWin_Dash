...
const Activity = () => {
  const [activities, setActivities] = React.useState([
    { id: '1', type: 'login', user: 'João Silva', description: 'Usuário fez login', timestamp: '2024-01-20T10:00:00Z', severity: 'info', module: 'auth' },
    { id: '2', type: 'create', user: 'Maria Santos', description: 'Criou novo produto', timestamp: '2024-01-20T09:30:00Z', severity: 'success', module: 'products' },
    { id: '3', type: 'error', user: 'Pedro Costa', description: 'Erro ao salvar arquivo', timestamp: '2024-01-20T08:45:00Z', severity: 'error', module: 'files' },
    { id: '4', type: 'update', user: 'Ana Lima', description: 'Atualizou configurações', timestamp: '2024-01-20T08:00:00Z', severity: 'warning', module: 'settings' }
  ]);
  ...
  const clearOldActivities = () => {
    ...
    setActivities(activities.filter(a => new Date(a.timestamp) > oneDayAgo));
  };
  ...
  const getStats = () => {
    ...
    const successes = activities.filter(a => a.severity === 'success').length;
  ...
  return (
    <div data-testid="activity-module">
      ...
      <div data-testid="stats">
        <div>Total de atividades: {stats.total}</div>
      ...
      <div data-testid="controls">
        ...
        <button onClick={() => createActivity({ type: 'test', user: 'Sistema', description: 'Atividade de teste', severity: 'info', module: 'system' })} data-testid="create-activity-btn">
          Criar Atividade
        </button>
        <button onClick={clearOldActivities} data-testid="clear-old-btn">
          Limpar Antigas
        </button>
        <button onClick={refreshActivities} data-testid="refresh-btn" disabled={isLoading}>
      ...
      <div data-testid="activities-list">
        {filteredActivities.map(activity => (
          <div key={activity.id} data-testid={`activity-${activity.id}`} className={`activity-card ${activity.severity}`}>
            ...
            <div>
              <button onClick={() => deleteActivity(activity.id)} data-testid={`delete-${activity.id}`}>
                Excluir
              </button>
            </div>
          </div>
...
describe('Activity Module', () => {
  ...
  it('should display activity details correctly', () => {
    ...
    expect(screen.getByText('Descrição: Usuário fez login')).toBeInTheDocument();
