...
describe('useAI', () => {
  ...
  beforeEach(() => {
    ...
    require('../hooks/useAIGeneration').useAIGeneration.mockReturnValue(mockGeneration);
    require('../hooks/useAIProviders').useAIProviders.mockReturnValue(mockProviders);
    require('../hooks/useAIHistory').useAIHistory.mockReturnValue(mockHistory);
    require('../hooks/useAIAnalytics').useAIAnalytics.mockReturnValue(mockAnalytics);
  ...
  describe('Carregamento de gerações', () => {
    it('deve carregar gerações com sucesso', async () => {
      ...
      expect(mockNotifications.showSuccess).toHaveBeenCalledWith('Gerações carregadas com sucesso!');
    });
    ...
    it('deve lidar com erro ao carregar gerações', async () => {
      ...
      expect(mockGeneration.loadGenerations).toHaveBeenCalled();
      expect(mockNotifications.showError).toHaveBeenCalledWith('Erro ao carregar gerações', 'Erro ao carregar gerações');
    });
  });
  ...
  describe('Criação de gerações', () => {
    it('deve criar geração com sucesso', async () => {
      ...
      await act(async () => {
        const createdGeneration = await result.current.createGeneration(mockGenerationData);
        expect(createdGeneration).toEqual(mockResult);
      });

      expect(mockGeneration.createGeneration).toHaveBeenCalledWith(mockGenerationData);
      expect(mockNotifications.showSuccess).toHaveBeenCalledWith('Geração criada com sucesso!');
    });
    ...
    it('deve lidar com erro ao criar geração', async () => {
      ...
      expect(mockGeneration.createGeneration).toHaveBeenCalledWith(mockGenerationData);
      expect(mockNotifications.showError).toHaveBeenCalledWith('Erro ao criar geração', 'Erro ao criar geração');
    });
  });
  ...
  describe('Atualização de gerações', () => {
    it('deve atualizar geração com sucesso', async () => {
      ...
      await act(async () => {
        const updatedGeneration = await result.current.updateGeneration('1', mockUpdateData);
        expect(updatedGeneration).toEqual(mockResult);
      });

      expect(mockGeneration.updateGeneration).toHaveBeenCalledWith('1', mockUpdateData);
      expect(mockNotifications.showSuccess).toHaveBeenCalledWith('Geração atualizada com sucesso!');
    });
  });
  ...
  describe('Exclusão de gerações', () => {
    it('deve excluir geração com sucesso', async () => {
      ...
      expect(mockNotifications.showSuccess).toHaveBeenCalledWith('Geração excluída com sucesso!');
    });
  });
