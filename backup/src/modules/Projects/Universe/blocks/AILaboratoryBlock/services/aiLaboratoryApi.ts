...
const handleApiError = (error: any, operation: string): AILaboratoryError => {
...
const pylabApi = {
  ...
  async checkConnection(): Promise<PyLabConnectionStatus> {
    try {
      ...
      return {
        status: response.data.connected ? 'connected' : 'error',
        lastChecked: new Date(),
        error: response.data.connected ? undefined : 'Connection failed'
      };
    } catch (error) {
  ...
  async comprehensiveBusinessAnalysis(data: any): Promise<AIGenerationResult> {
    try {
...
export const aiLaboratoryApi = {
  ...
  // Business Intelligence
  async businessIntelligenceAnalysis(data: any): Promise<AIGenerationResult> {
