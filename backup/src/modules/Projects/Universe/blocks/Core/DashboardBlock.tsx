...
const DashboardBlock: React.FC<DashboardBlockProps> = ({ 
  ...
  }, [data.autoRefresh, data.refreshInterval]);
  ...
  return (
    ...
    >
      ...
      />
      {/* Header Icon */}
      <div className="absolute top-3 left-3 p-1 bg-blue-500 rounded text-white">
      ...
      <div className="grid grid-cols-2 gap-2 mt-8">
        ...
        <div className="p-2 bg-orange-50 rounded-lg">
          ...
          <Progress value={metrics.conversion} className="h-1 mt-1" />
        </div>
        <div className="p-2 bg-purple-50 rounded-lg">
          <div className="flex items-center gap-1">
            <BarChart3 className="w-3 h-3 text-purple-600" />
            <span className="text-xs text-purple-600 font-medium">Growth</span>
          </div>
          ...
          <div className="flex items-center gap-1 text-xs">
            {getGrowthIcon(metrics.growth)}
            <span className={getGrowthColor(metrics.growth)}>
              {metrics.growth > 0 ? 'Growing' : metrics.growth < 0 ? 'Declining' : 'Stable'}
            </span>
          </div>
        </div>
      </div>
