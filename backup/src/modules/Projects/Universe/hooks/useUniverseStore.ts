...
interface UniverseActions {
  ...
  useTemplate: (templateId: string, templateData: any) => Promise<any>;

  // Ações de IA
  getAISuggestions: (suggestionData: any) => Promise<any>;
  personalizeTemplate: (templateId: string, personalizationData: any) => Promise<any>;

  // Ações de canvas
  setCanvasData: (data: any) => void;
  setSelectedBlocks: (blocks: UniverseBlock[]) => void;
  ...
  testInstancesManagement: () => Promise<{ success: boolean; message: string; data?: any }>;
  testTemplatesSystem: () => Promise<{ success: boolean; message: string; data?: any }>;
  testAIIntegration: () => Promise<{ success: boolean; message: string; data?: any }>;
    ...
    (set, get) => ({
      ...
      fetchInstances: async (filters: Record<string, any> = {}) => {
        ...
        try {
          ...
          set({ 
            instances: response.data || response,
      ...
      useTemplate: async (templateId: string, templateData: any) => {
        set({ loading: true, error: null });
        try {
      ...
      // Ações de IA
      getAISuggestions: async (suggestionData: any) => {
        set({ loading: true, error: null });
        try {
      ...
      personalizeTemplate: async (templateId: string, personalizationData: any) => {
        set({ loading: true, error: null });
        try {
