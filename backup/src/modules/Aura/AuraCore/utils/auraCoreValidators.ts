/**
 * Validadores para o módulo AuraCore
 */
 ...
 */
export const validateDashboardData = (data: any): {
  isValid: boolean;
  errors: string[];
} => {
  const errors: string[] = [];
  ...
  if (data.stats && !validateAuraStats(data.stats).isValid) {
  ...
  if (data.quick_actions && Array.isArray(data.quick_actions)) {
    data.quick_actions.forEach((action: any, index: number) => {
 ...
 */
export const validateAuraStats = (stats: any): {
  isValid: boolean;
  errors: string[];
} => {
  const errors: string[] = [];
 ...
 */
export const validateAuraFilters = (filters: any): {
  isValid: boolean;
  errors: string[];
} => {
  const errors: string[] = [];
  ...
  if (filters.status && !['active', 'inactive', 'error', 'loading', 'pending'].includes(filters.status)) {
    errors.push('Status do filtro deve ser active, inactive, error, loading ou pending');
  }

  // Validar tipo de módulo se fornecido
  if (filters.module_type && !['connections', 'flows', 'chats', 'stats'].includes(filters.module_type)) {
