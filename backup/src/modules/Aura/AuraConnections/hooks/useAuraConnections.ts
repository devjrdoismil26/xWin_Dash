/**
 * Hook especializado para conexões do Aura
 * Gerencia conexões WhatsApp Business e outras integrações
 */
...
export const useAuraConnections = () => {
  ...
  const updateConnection = useCallback(async (id: string, connectionData: Partial<AuraConnection>) => {
    ...
    try {
      ...
      notify('success', 'Conexão atualizada com sucesso!');
    } catch (err: any) {
  ...
  const deleteConnection = useCallback(async (id: string) => {
    ...
    try {
      ...
      notify('success', 'Conexão excluída com sucesso!');
    } catch (err: any) {
  ...
  const testConnection = useCallback(async (id: string) => {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        notify('success', 'Teste de conexão realizado com sucesso!');
      }, 1000);
  ...
  const syncConnection = useCallback(async (id: string) => {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        notify('success', 'Sincronização realizada com sucesso!');
      }, 1500);
  ...
  const toggleConnection = useCallback(async (id: string) => {
    try {
      ...
      const errorMessage = err.message || 'Erro ao alterar status da conexão';
      setError(errorMessage);
      showError('Erro ao carregar conexões', errorMessage);
    }
  ...
  const setPrimary = useCallback(async (id: string) => {
    try {
      ...
      notify('success', 'Conexão definida como primária!');
    } catch (err: any) {
      const errorMessage = err.message || 'Erro ao definir conexão primária';
      setError(errorMessage);
      showError('Erro ao carregar conexões', errorMessage);
    }
  }, [updateConnection]);
  ...
  const getHealthStats = useCallback(() => {
    ...
    const averageHealth = connections.length > 0 
      ? connections.reduce((sum, c) => sum + c.health_score, 0) / connections.length 
      : 0;
