// ========================================
// UTILITÁRIOS DE PERFORMANCE - ANALYTICS
// ========================================
// Funções para otimização de performance, debounce, throttling e lazy loading
    ...
    if (cached && Date.now() - cached.timestamp < ttl) {
      ...
      if (options?.staleWhileRevalidate) {
        ...
        fetcher()
          ...
          .catch(err => console.warn('Erro ao revalidar cache:', err));
      }
      return;
    }
...
export const compressData = (data: any): string => {
  try {
    ...
    console.warn('Falha na compressão:', error);
...
export const decompressData = <T>(compressedData: string): T | null => {
  try {
    ...
    console.warn('Falha na descompressão:', error);
    return null;
  }
};
