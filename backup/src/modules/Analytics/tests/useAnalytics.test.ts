...
describe('useAnalytics Hook', () => {
  ...
  it('should handle metric label formatting', () => {
    ...
    expect(result.current.getMetricLabel('page_views')).toBe('Visualizações de Página');
    expect(result.current.getMetricLabel('unique_visitors')).toBe('Visitantes Únicos');
    expect(result.current.getMetricLabel('bounce_rate')).toBe('Taxa de Rejeição');
    expect(result.current.getMetricLabel('avg_session_duration')).toBe('Duração Média da Sessão');
    expect(result.current.getMetricLabel('conversion_rate')).toBe('Taxa de Conversão');
    expect(result.current.getMetricLabel('revenue')).toBe('Receita');
    expect(result.current.getMetricLabel('unknown_metric')).toBe('unknown_metric');
  });
  ...
  it('should handle time range label formatting', () => {
    ...
    expect(result.current.getTimeRangeLabel('unknown_range')).toBe('unknown_range');
  });

  it('should handle device label formatting', () => {
    const { result } = renderHook(() => useAnalytics());

    expect(result.current.getDeviceLabel('desktop')).toBe('Desktop');
    expect(result.current.getDeviceLabel('mobile')).toBe('Mobile');
    expect(result.current.getDeviceLabel('tablet')).toBe('Tablet');
    expect(result.current.getDeviceLabel('unknown_device')).toBe('unknown_device');
  });
  ...
  it('should handle traffic source label formatting', () => {
    ...
    expect(result.current.getTrafficSourceLabel('unknown_source')).toBe('unknown_source');
  });
  ...
  it('should aggregate loading states from specialized hooks', () => {
    ...
    const mockUseAnalyticsDashboard = require('../hooks/useAnalyticsDashboard').useAnalyticsDashboard;
    const mockUseAnalyticsFilters = require('../hooks/useAnalyticsFilters').useAnalyticsFilters;
    const mockUseAnalyticsRealTime = require('../hooks/useAnalyticsRealTime').useAnalyticsRealTime;
    const mockUseAnalyticsReports = require('../hooks/useAnalyticsReports').useAnalyticsReports;
