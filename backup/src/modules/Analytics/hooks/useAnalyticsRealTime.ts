...
export const useAnalyticsRealTime = () => {
  ...
  const startPolling = useCallback(() => {
    ...
    intervalRef.current = setInterval(async () => {
      try {
        ...
        setError('Erro ao obter dados em tempo real');
  ...
  const enableRealTimeWithStatus = useCallback(() => {
    ...
    try {
      ...
      notify('success', 'Analytics em tempo real habilitado!');
    } catch (err: any) {
      setError('Erro ao habilitar tempo real');
      notify('error', 'Erro ao habilitar tempo real');
    } finally {
      setLoading(false);
    }
  }, [enableRealTime, connectWebSocket, startPolling]);
  ...
  const disableRealTimeWithStatus = useCallback(() => {
    ...
    try {
      ...
      notify('success', 'Analytics em tempo real desabilitado!');
    } catch (err: any) {
      setError('Erro ao desabilitar tempo real');
      notify('error', 'Erro ao desabilitar tempo real');
    } finally {
      setLoading(false);
    }
  ...
  const enableAutoRefreshWithStatus = useCallback(() => {
    try {
      enableAutoRefresh();
      notify('success', 'Auto refresh habilitado!');
    } catch (err: any) {
      setError('Erro ao habilitar auto refresh');
      notify('error', 'Erro ao habilitar auto refresh');
    }
  }, [enableAutoRefresh]);
  ...
  const disableAutoRefreshWithStatus = useCallback(() => {
    try {
      disableAutoRefresh();
      notify('success', 'Auto refresh desabilitado!');
    } catch (err: any) {
      setError('Erro ao desabilitar auto refresh');
      notify('error', 'Erro ao desabilitar auto refresh');
    }
  }, [disableAutoRefresh]);
