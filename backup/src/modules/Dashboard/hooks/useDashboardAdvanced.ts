...
interface UseDashboardAdvancedReturn {
  ...
  createLayout: (layout: any) => Promise<boolean>;
  updateLayout: (layoutId: string, layout: any) => Promise<boolean>;
  ...
  shareDashboard: (dashboardId: string, permissions: any, expiresAt?: string) => Promise<boolean>;
  getSharedDashboard: (token: string) => Promise<any>;
...
export const useDashboardAdvanced = (): UseDashboardAdvancedReturn => {
  ...
  const createLayout = useCallback(async (layout: any): Promise<boolean> => {
    try {
  ...
  const updateLayout = useCallback(async (layoutId: string, layout: any): Promise<boolean> => {
    try {
  ...
  const fetchUniverseData = useCallback(async () => {
    ...
    try {
      ...
      setUniverseError(error.message);
    } finally {
