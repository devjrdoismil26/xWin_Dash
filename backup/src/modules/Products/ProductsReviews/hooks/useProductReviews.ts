...
} from '../services/productsReviewsService';
import {
  ProductReview,
...
interface UseProductReviewsReturn {
  ...
  // Operações básicas
  loadReviews: (productId: string, filters?: any) => Promise<void>;
  ...
  // Utilitários
  validateData: (data: ProductReviewFormData) => { isValid: boolean; errors: string[] };
...
export const useProductReviews = (): UseProductReviewsReturn => {
  ...
  const loadReviews = useCallback(async (productId: string, filters?: any) => {
    setLoading(true);
    setError(null);
  ...
  const loadReview = useCallback(async (reviewId: string) => {
    ...
    try {
      ...
      setError(err.message || 'Erro inesperado ao carregar review');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const createNewReview = useCallback(async (productId: string, data: ProductReviewFormData): Promise<ProductResponse> => {
    ...
    try {
      ...
      const errorResponse = {
        success: false,
        error: err.message || 'Erro inesperado ao criar review'
      };
  ...
  const updateExistingReview = useCallback(async (reviewId: string, data: Partial<ProductReviewFormData>): Promise<ProductResponse> => {
    ...
    try {
      ...
      const errorResponse = {
        success: false,
        error: err.message || 'Erro inesperado ao atualizar review'
      };
      setError(errorResponse.error);
      return errorResponse;
    } finally {
  ...
  const deleteExistingReview = useCallback(async (reviewId: string): Promise<ProductResponse> => {
    ...
    try {
      ...
      const errorResponse = {
        success: false,
        error: err.message || 'Erro inesperado ao deletar review'
      };
      setError(errorResponse.error);
      return errorResponse;
    } finally {
  ...
  const moderate = useCallback(async (reviewId: string, action: 'approve' | 'reject' | 'flag', reason?: string): Promise<ProductResponse> => {
    ...
    try {
      ...
      const errorResponse = {
        success: false,
        error: err.message || 'Erro inesperado ao moderar review'
      };
      setError(errorResponse.error);
      return errorResponse;
    } finally {
  ...
  const bulkModerate = useCallback(async (reviewIds: string[], action: 'approve' | 'reject' | 'flag'): Promise<ProductResponse> => {
    ...
    try {
      ...
      const errorResponse = {
        success: false,
        error: err.message || 'Erro inesperado ao moderar reviews em lote'
      };
      setError(errorResponse.error);
      return errorResponse;
    } finally {
