...
class ProductsCacheService {
  ...
  setProducts(products: any[], filters?: any, ttl?: number): void {
    const key = this.generateProductsKey(filters);
    this.set(key, products, ttl);
  }

  getProducts(filters?: any): any[] | null {
    const key = this.generateProductsKey(filters);
    return this.get(key);
  }
  ...
  setProductVariations(productId: string, variations: any[], ttl?: number): void {
    const key = `product:${productId}:variations`;
    this.set(key, variations, ttl);
  }

  getProductVariations(productId: string): any[] | null {
    const key = `product:${productId}:variations`;
    return this.get(key);
  }
  ...
  setProductReviews(productId: string, reviews: any[], ttl?: number): void {
    const key = `product:${productId}:reviews`;
    this.set(key, reviews, ttl);
  }

  getProductReviews(productId: string): any[] | null {
    const key = `product:${productId}:reviews`;
    return this.get(key);
  }

  setProductAnalytics(productId: string, analytics: any, ttl?: number): void {
    const key = `product:${productId}:analytics`;
    this.set(key, analytics, ttl);
  }

  getProductAnalytics(productId: string): any | null {
    const key = `product:${productId}:analytics`;
    return this.get(key);
  }
  ...
  private generateProductsKey(filters?: any): string {
