...
export const validateImageData = (data: ProductImageFormData): ValidationResult => {
  ...
  if (data.alt && data.alt.length > 100) {
    errors.push('Texto alternativo deve ter no máximo 100 caracteres');
  }

  if (data.caption && data.caption.length > 200) {
...
export const validateBundleData = (data: ProductBundleFormData): ValidationResult => {
  ...
  if (data.discount) {
    ...
    if (data.discount.value > 100) {
      errors.push('Desconto não pode ser maior que 100%');
    }
  }
...
export const validateAll = (data: any, type: 'product' | 'variation' | 'image' | 'review' | 'bundle' | 'inventory' | 'alert'): ValidationResult => {
  switch (type) {
    case 'product':
    ...
    case 'inventory':
      return validateInventoryData(data);
    case 'alert':
      return validateInventoryAlert(data);
    default:
      return {
        isValid: false,
