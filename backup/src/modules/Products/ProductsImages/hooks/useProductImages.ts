...
import {
  ...
  getImageUrl,
  formatFileSize
} from '../services/productsImagesService';
import {
  ProductImage,
...
interface UseProductImagesReturn {
  ...
  reorder: (productId: string, imageOrder: { id: string; order: number }[]) => Promise<ProductResponse>;
...
export const useProductImages = (): UseProductImagesReturn => {
  ...
  const loadImages = useCallback(async (productId: string) => {
    ...
    try {
      ...
      setError(err.message || 'Erro inesperado ao carregar imagens');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const loadImage = useCallback(async (imageId: string) => {
    ...
    try {
      ...
      setError(err.message || 'Erro inesperado ao carregar imagem');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const uploadImage = useCallback(async (productId: string, file: File, metadata?: Partial<ProductImageFormData>): Promise<ImageUploadResponse> => {
    ...
    try {
      ...
      const errorResponse = {
        success: false,
        error: err.message || 'Erro inesperado ao fazer upload da imagem'
      };
  ...
  const updateImage = useCallback(async (imageId: string, metadata: Partial<ProductImageFormData>): Promise<ProductResponse> => {
    ...
    try {
      ...
      const errorResponse = {
        success: false,
        error: err.message || 'Erro inesperado ao atualizar imagem'
      };
      setError(errorResponse.error);
      return errorResponse;
    } finally {
  ...
  const deleteImage = useCallback(async (imageId: string): Promise<ProductResponse> => {
    ...
    try {
      ...
      const errorResponse = {
        success: false,
        error: err.message || 'Erro inesperado ao deletar imagem'
      };
      setError(errorResponse.error);
      return errorResponse;
    } finally {
  ...
  const bulkUpload = useCallback(async (productId: string, files: File[]): Promise<ImageUploadResponse> => {
    ...
    try {
      ...
      const errorResponse = {
        success: false,
        error: err.message || 'Erro inesperado ao fazer upload das imagens'
      };
      setError(errorResponse.error);
      return errorResponse;
    } finally {
  ...
  const bulkDelete = useCallback(async (imageIds: string[]): Promise<ProductResponse> => {
    ...
    try {
      ...
      const errorResponse = {
        success: false,
        error: err.message || 'Erro inesperado ao deletar imagens em lote'
      };
      setError(errorResponse.error);
      return errorResponse;
    } finally {
