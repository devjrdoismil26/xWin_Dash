          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center space-x-4 text-xs text-gray-500">
                <span>Plataforma: {post.platform}</span>
...
const SocialPostsManager: React.FC<SocialPostsManagerProps> = ({
  className = '',
  showHeader = true,
  showFilters = true,
  showActions = true,
  ...
  const handleDuplicatePost = useCallback((post: any) => {
    ui.setLoading(true);
  ...
  const handleSchedulePost = useCallback((post: any) => {
    console.log('Agendar post:', post);
    // Implementar lógica de agendamento
  }, []);

  const handleViewPost = useCallback((post: any) => {
    console.log('Ver post:', post);
    // Implementar lógica de visualização
  }, []);

  const handlePublishPost = useCallback((post: any) => {
    ui.setLoading(true);
  ...
  const handlePausePost = useCallback((post: any) => {
    ui.setLoading(true);
  ...
  if (ui.loading && !postsStore.posts.length) {
    return <SocialBufferLoadingSkeleton type="posts" className={className} />;
  }
