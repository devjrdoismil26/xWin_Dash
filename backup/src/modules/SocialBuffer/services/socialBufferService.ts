...
class SocialBufferService {
    ...
    for (let attempt = 1; attempt <= this.config.retry.max_attempts; attempt++) {
      try {
        ...
        if (attempt < this.config.retry.max_attempts) {
          console.warn(`${operationName} falhou (tentativa ${attempt}/${this.config.retry.max_attempts}), tentando novamente...`);
   ...
   */
  private validateData(data: any, rules: any[]): SocialBufferOperationResult {
   ...
   */
  async performRestore(backupData: any): Promise<SocialBufferOperationResult> {
    try {
