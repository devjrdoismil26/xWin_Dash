...
// Cache para hashtags
const hashtagsCache = new Map<string, { data: any; timestamp: number }>();
...
class HashtagsService {
  ...
  async getHashtagById(id: number): Promise<SocialHashtag> {
    try {
      ...
      // Cache do resultado
      hashtagsCache.set(cacheKey, { data: response.data, timestamp: Date.now() });
  ...
  async getRelatedHashtags(hashtag: string, platform: SocialPlatform, limit: number = 10): Promise<string[]> {
    try {
      ...
      return response.data.related_hashtags || [];
    } catch (error) {
