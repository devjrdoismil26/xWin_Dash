...
const engagementCache = new Map<string, { data: any; timestamp: number }>();
...
export interface Interaction {
  ...
  reaction_type?: string; // para reações específicas
  timestamp: string;
  platform: string;
  metadata?: {
    location?: string;
...
class EngagementService {
  ...
  async getPostEngagement(postId: string): Promise<EngagementData> {
    try {
      ...
      // Cache do resultado
      engagementCache.set(cacheKey, { data: response.data, timestamp: Date.now() });
  ...
  ): Promise<{ success: boolean; applied_optimizations: string[]; results: any }> {
    try {
