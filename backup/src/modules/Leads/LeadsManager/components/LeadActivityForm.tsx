...
const LeadActivityForm: React.FC<LeadActivityFormProps> = ({ 
  leadId, 
  onActivityAdded, 
  onCancel 
}) => {
  ...
  return (
    <div className="bg-white p-4 rounded-lg border">
      <div className="flex items-center gap-2 mb-4">
        <span className="text-2xl">{getActivityIcon(form.type)}</span>
        <h3 className="text-lg font-medium text-gray-900">Adicionar Atividade</h3>
      </div>
      <form className="space-y-4" onSubmit={handleSubmit}>
        <div>
          ...
          <Select
            ...
            value={form.type}
            onChange={(value) => setForm({ ...form, type: value as LeadActivity['type'] })}
          />
        </div>
        <div>
          <Textarea
