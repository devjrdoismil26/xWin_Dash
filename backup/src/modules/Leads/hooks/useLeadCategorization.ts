...
export interface CategorizationStats {
  ...
  most_used_tags: Array<{ tag: Tag; count: number }>;
...
export const useLeadCategorization = () => {
  ...
  const fetchTags = useCallback(async (): Promise<Tag[]> => {
    ...
    try {
      const response = await fetch('/api/v1/categorization/tags', {
        method: 'GET',
        headers: {
  ...
  const updateTag = useCallback(async (id: number, tagData: Partial<Tag>): Promise<Tag | null> => {
    try {
      const response = await fetch(`/api/v1/categorization/tags/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json',
  ...
  const updateCategory = useCallback(async (id: number, categoryData: Partial<Category>): Promise<Category | null> => {
    try {
      const response = await fetch(`/api/v1/categorization/categories/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json',
  ...
  const addTagToLead = useCallback(async (leadId: number, tagId: number): Promise<boolean> => {
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Erro ao adicionar tag ao lead';
      setError(errorMessage);
  ...
  const removeTagFromLead = useCallback(async (leadId: number, tagId: number): Promise<boolean> => {
    try {
      const response = await fetch(`/api/v1/leads/${leadId}/tags/${tagId}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json',
      ...
      const errorMessage = err instanceof Error ? err.message : 'Erro ao remover tag do lead';
      setError(errorMessage);
  ...
  const setLeadCategory = useCallback(async (leadId: number, categoryId: number): Promise<boolean> => {
    try {
      const response = await fetch(`/api/v1/leads/${leadId}/category`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json',
      ...
      const errorMessage = err instanceof Error ? err.message : 'Erro ao definir categoria do lead';
      setError(errorMessage);
