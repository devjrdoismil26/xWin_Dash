...
export const useLeadsCore = (): UseLeadsCoreReturn => {
  ...
  const createLead = useCallback(async (data: LeadFormData) => {
    ...
    try {
      ...
      const errorMessage = err.message || 'Erro ao criar lead';
      setError(errorMessage);
  ...
  const updateLead = useCallback(async (id: string, data: LeadFormData) => {
    ...
    try {
      ...
      const errorMessage = err.message || 'Erro ao atualizar lead';
      setError(errorMessage);
  ...
  const deleteLead = useCallback(async (id: string) => {
    ...
    try {
      ...
      const errorMessage = err.message || 'Erro ao excluir lead';
      setError(errorMessage);
