...
} from '../services/settingsOptimizationService';
...
export const useSettingsOptimization = (): UseSettingsOptimizationReturn => {
   ...
   */
  const createDebouncedFunctionWrapper = useCallback(<T extends (...args: any[]) => any>(
    key: string,
    func: T,
    delay?: number
  ): T => {
   ...
   */
  const createMemoizedFunctionWrapper = useCallback(<T extends (...args: any[]) => any>(
    key: string,
    func: T,
    ttl?: number
  ): T => {
