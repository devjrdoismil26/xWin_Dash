    ...
    while (this.preloadQueue.length > 0) {
      ...
      if (item && !this.preloadedItems.has(item.key)) {
        try {
          ...
          console.warn(`Erro ao preload ${item.key}:`, error);
        }
      }
    }
  }
...
class SettingsOptimizationService {
  ...
  getOptimizationStats(): {
    ...
    return {
      debounce: {
        activeTimers: (this.debounceManager as any).timers.size
      },
      preload: {
        preloadedItems: (this.preloadManager as any).preloadedItems.size,
        queueLength: (this.preloadManager as any).preloadQueue.length
      },
