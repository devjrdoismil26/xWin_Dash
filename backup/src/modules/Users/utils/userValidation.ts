 ...
 * Implementa validação robusta com regras configuráveis e validação em tempo real
 */
...
class UserValidator {
   ...
   * Validações cross-field
   */
  private async validateCrossFields(data: Record<string, any>, context?: any): Promise<ValidationResult> {
    const errors: ValidationError[] = [];
    const warnings: ValidationWarning[] = [];
  ...
  private initializeDefaultRules(): void {
    ...
    this.addRule('email', {
      ...
      validate: (value) => !value || value.length <= 255,
      priority: 8
    });
...
export const validationUtils = {
   ...
   */
  async validateUpdateUser(data: any, userId: string): Promise<ValidationResult> {
    return userValidator.validateObject(data, { 
      checkEmailAvailability: true,
      operation: 'update',
      userId 
   ...
   */
  async validateChangePassword(data: any): Promise<ValidationResult> {
   ...
   */
  async validateProfile(data: any): Promise<ValidationResult> {
