...
export interface NotificationStats {
  ...
  average_read_time: number; // em minutos
...
class UserNotificationsService {
  ...
  async getNotificationById(id: string): Promise<Notification> {
    try {
      ...
      // Cache do resultado
      notificationsCache.set(cacheKey, { data: response.data, timestamp: Date.now() });
  ...
  async archiveNotification(notificationId: string): Promise<Notification> {
    try {
      ...
      console.error(`Erro ao arquivar notificação ${notificationId}:`, error);
  ...
  async unarchiveNotification(notificationId: string): Promise<Notification> {
    try {
      ...
      console.error(`Erro ao desarquivar notificação ${notificationId}:`, error);
  ...
  async deleteNotification(notificationId: string): Promise<void> {
    try {
      ...
      console.error(`Erro ao remover notificação ${notificationId}:`, error);
  ...
  async getUnreadCount(userId: string): Promise<number> {
    try {
      ...
      // Cache do resultado
      notificationsCache.set(cacheKey, { data: response.data.count, timestamp: Date.now() });
      
      return response.data.count;
    } catch (error) {
      console.error(`Erro ao obter contagem de notificações não lidas do usuário ${userId}:`, error);
  ...
  async getNotificationSettings(userId: string): Promise<NotificationSettings> {
    try {
      ...
      console.error(`Erro ao obter configurações de notificação do usuário ${userId}:`, error);
  ...
  async updateNotificationSettings(userId: string, settings: Partial<NotificationSettings>): Promise<NotificationSettings> {
    try {
      ...
      console.error(`Erro ao atualizar configurações de notificação do usuário ${userId}:`, error);
  ...
  async getNotificationTemplates(): Promise<NotificationTemplate[]> {
    try {
      ...
      console.error('Erro ao obter templates de notificação:', error);
  ...
  async sendTemplateNotification(templateId: string, userId: string, variables: Record<string, any> = {}): Promise<Notification> {
    try {
      ...
      console.error(`Erro ao enviar notificação template ${templateId} para usuário ${userId}:`, error);
  ...
  async cancelScheduledNotification(notificationId: string): Promise<void> {
    try {
      ...
      console.error(`Erro ao cancelar notificação agendada ${notificationId}:`, error);
