...
const rolesCache = new Map<string, { data: any; timestamp: number }>();
...
class UserRolesService {
  ...
  async getRoles(): Promise<Role[]> {
    try {
      ...
      // Cache do resultado
      rolesCache.set(cacheKey, { data: response.data, timestamp: Date.now() });
  ...
  async getRoleById(id: string): Promise<Role> {
    try {
      ...
      console.error(`Erro ao obter role ${id}:`, error);
  ...
  async assignRole(userId: string, roleId: string, expiresAt?: string): Promise<RoleAssignment> {
    try {
      ...
      console.error(`Erro ao atribuir role ${roleId} ao usuário ${userId}:`, error);
  ...
  async removeRole(userId: string, roleId: string): Promise<void> {
    try {
      ...
      console.error(`Erro ao remover role ${roleId} do usuário ${userId}:`, error);
      throw new Error('Falha ao remover role');
    }
  }
  ...
  async getUserRoles(userId: string): Promise<Role[]> {
    try {
      ...
      console.error(`Erro ao obter roles do usuário ${userId}:`, error);
  ...
  async hasPermission(userId: string, permission: string): Promise<boolean> {
    try {
      ...
      console.error(`Erro ao verificar permissão ${permission} do usuário ${userId}:`, error);
      return false;
    }
  }
  ...
  async hasRole(userId: string, role: string): Promise<boolean> {
    try {
      ...
      console.error(`Erro ao verificar role ${role} do usuário ${userId}:`, error);
      return false;
    }
  }
  ...
  async checkPermissions(userId: string, permissions: string[]): Promise<Record<string, boolean>> {
    try {
      ...
      console.error(`Erro ao verificar permissões do usuário ${userId}:`, error);
  ...
  async checkRoles(userId: string, roles: string[]): Promise<Record<string, boolean>> {
    try {
      ...
      console.error(`Erro ao verificar roles do usuário ${userId}:`, error);
  ...
  async getPermissionCheck(userId: string, permission: string): Promise<PermissionCheck> {
    try {
      ...
      console.error(`Erro ao obter detalhes da permissão ${permission} do usuário ${userId}:`, error);
  ...
  async getUsersByRole(roleId: string): Promise<User[]> {
    try {
      ...
      console.error(`Erro ao obter usuários da role ${roleId}:`, error);
  ...
  async getUsersByPermission(permission: string): Promise<User[]> {
    try {
      ...
      console.error(`Erro ao obter usuários da permissão ${permission}:`, error);
  ...
  async duplicateRole(roleId: string, overrides?: Partial<CreateRoleData>): Promise<Role> {
    try {
      ...
      console.error(`Erro ao duplicar role ${roleId}:`, error);
  ...
  async assignMultipleRoles(userId: string, roleIds: string[]): Promise<RoleAssignment[]> {
    try {
      ...
      console.error(`Erro ao atribuir múltiplas roles ao usuário ${userId}:`, error);
  ...
  async removeMultipleRoles(userId: string, roleIds: string[]): Promise<void> {
    try {
      ...
      console.error(`Erro ao remover múltiplas roles do usuário ${userId}:`, error);
