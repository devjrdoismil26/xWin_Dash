...
class UserManagementService {
  ...
  async getUserById(id: string): Promise<User> {
    try {
      ...
      // Cache do resultado
      usersCache.set(cacheKey, { data: response.data, timestamp: Date.now() });
  ...
  async suspendUser(id: string, reason?: string): Promise<User> {
    try {
      ...
      console.error(`Erro ao suspender usuário ${id}:`, error);
  ...
  async unsuspendUser(id: string): Promise<User> {
    try {
      ...
      console.error(`Erro ao remover suspensão do usuário ${id}:`, error);
  ...
  async getUsersByStatus(status: string): Promise<User[]> {
    try {
      const result = await this.getUsers({ status: status as any, limit: 1000 });
      return result.data;
    } catch (error) {
      console.error(`Erro ao buscar usuários por status ${status}:`, error);
  ...
  async duplicateUser(id: string, overrides?: Partial<CreateUserData>): Promise<User> {
    try {
      ...
      console.error(`Erro ao duplicar usuário ${id}:`, error);
