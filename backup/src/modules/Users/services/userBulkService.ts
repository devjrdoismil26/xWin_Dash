...
class UserBulkService {
  ...
  async bulkDeleteUsers(data: BulkDeleteData): Promise<BulkOperationResult> {
    try {
      ...
      console.error('Erro ao remover usuários em lote:', error);
  ...
  async bulkAssignRole(data: BulkRoleAssignmentData): Promise<BulkOperationResult> {
    try {
      ...
      console.error('Erro ao atribuir role em lote:', error);
  ...
  async bulkRemoveRole(data: BulkRoleRemovalData): Promise<BulkOperationResult> {
    try {
      ...
      console.error('Erro ao remover role em lote:', error);
  ...
  async bulkActivateUsers(data: BulkActivationData): Promise<BulkOperationResult> {
    try {
      ...
      console.error('Erro ao ativar usuários em lote:', error);
  ...
  async bulkDeactivateUsers(data: BulkDeactivationData): Promise<BulkOperationResult> {
    try {
      ...
      console.error('Erro ao desativar usuários em lote:', error);
  ...
  async bulkSuspendUsers(data: BulkSuspensionData): Promise<BulkOperationResult> {
    try {
      ...
      console.error('Erro ao suspender usuários em lote:', error);
  ...
  async bulkUnsuspendUsers(userIds: string[]): Promise<BulkOperationResult> {
    try {
      ...
      console.error('Erro ao remover suspensão de usuários em lote:', error);
   ...
   */
  async startBulkOperation(operation: string, data: any): Promise<{ operation_id: string }> {
    try {
      ...
      console.error('Erro ao iniciar operação em lote:', error);
  ...
  async getBulkOperationProgress(operationId: string): Promise<BulkOperationProgress> {
    try {
      ...
      // Cache do resultado
      bulkCache.set(cacheKey, { data: response.data, timestamp: Date.now() });
      
      return response.data;
    } catch (error) {
      console.error(`Erro ao obter progresso da operação ${operationId}:`, error);
  ...
  async cancelBulkOperation(operationId: string): Promise<void> {
    try {
      ...
      console.error(`Erro ao cancelar operação ${operationId}:`, error);
  ...
  async getBulkOperationResult(operationId: string): Promise<BulkOperationResult> {
    try {
      ...
      console.error(`Erro ao obter resultado da operação ${operationId}:`, error);
  ...
  async getBulkOperationHistory(params: {
    ...
    try {
      ...
      console.error('Erro ao obter histórico de operações em lote:', error);
   ...
   */
  async validateBulkData(operation: string, data: any): Promise<{
    is_valid: boolean;
    errors: string[];
    warnings: string[];
    suggestions: string[];
    ...
    try {
      ...
      console.error('Erro ao validar dados para operação em lote:', error);
  ...
  async getImportTemplate(format: 'csv' | 'excel' = 'csv'): Promise<Blob> {
    try {
      ...
      console.error('Erro ao obter template de importação:', error);
  ...
  async getBulkOperationStats(): Promise<{
    ...
    try {
      ...
      console.error('Erro ao obter estatísticas de operações em lote:', error);
  ...
  async cleanupOldOperations(daysToKeep: number = 30): Promise<{ deleted_count: number }> {
    try {
      ...
      console.error('Erro ao limpar operações antigas:', error);
  ...
  async getBulkOperationSettings(): Promise<{
    ...
    try {
      ...
      console.error('Erro ao obter configurações de operações em lote:', error);
  ...
  async updateBulkOperationSettings(settings: Partial<{
    ...
    try {
      ...
      console.error('Erro ao atualizar configurações de operações em lote:', error);
