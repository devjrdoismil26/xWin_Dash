...
export const useMediaAnalytics = () => {
  ...
  const calculateBasicStats = useCallback((): MediaStats => {
    ...
    const storageUsed = media.reduce((sum, file) => sum + (file.size || 0), 0);
  ...
  const calculatePeriodStats = useCallback((startDate: Date, endDate: Date) => {
    ...
    return {
      ...
      storageAdded: periodMedia.reduce((sum, file) => sum + (file.size || 0), 0),
      typesBreakdown: periodMedia.reduce((acc, file) => {
  ...
  const calculateUsageByType = useCallback(() => {
    const typeStats = media.reduce((acc, file) => {
      if (!acc[file.type]) {
        acc[file.type] = {
          count: 0,
          totalSize: 0,
  ...
  const generateInsights = useCallback((
    ...
    const mostUsedType = Object.keys(usageByType).reduce((a, b) => 
