...
interface WorkflowsStoreActions {
  // ===== WORKFLOW CRUD =====
  fetchWorkflows: (params?: WorkflowFilters & { page?: number; limit?: number }) => Promise<void>;
  fetchWorkflow: (workflowId: string | number) => Promise<void>;
  createWorkflow: (workflowData: any) => Promise<{ success: boolean; data?: Workflow; error?: string }>;
  updateWorkflow: (workflowId: string | number, workflowData: any) => Promise<{ success: boolean; data?: Workflow; error?: string }>;
  ...
  clearFilters: () => void;
  updateFilter: (key: keyof WorkflowFilters, value: any) => void;
  ...
  // ===== SELECTION =====
  selectWorkflow: (workflowId: string | number) => void;
  deselectWorkflow: (workflowId: string | number) => void;
  selectAll: () => void;
  clearSelection: () => void;
  toggleSelection: (workflowId: string | number) => void;
    ...
    (set, get) => ({
      ...
      fetchWorkflows: async (params) => {
        ...
        try {
          ...
          set({
            error: error instanceof Error ? error.message : 'Erro ao carregar workflows',
            loading: false
          });
        }
      },
      ...
      fetchWorkflow: async (workflowId) => {
        ...
        try {
          ...
          set({
            error: error instanceof Error ? error.message : 'Erro ao carregar workflow',
            loading: false
          });
        }
      },
      ...
      createWorkflow: async (workflowData) => {
        ...
        try {
          ...
          const errorMessage = error instanceof Error ? error.message : 'Erro ao criar workflow';
      ...
      updateWorkflow: async (workflowId, workflowData) => {
        ...
        try {
          ...
          const errorMessage = error instanceof Error ? error.message : 'Erro ao atualizar workflow';
      ...
      deleteWorkflow: async (workflowId) => {
        ...
        try {
          ...
          const errorMessage = error instanceof Error ? error.message : 'Erro ao remover workflow';
      ...
      duplicateWorkflow: async (workflowId, newName) => {
        set({ isCreating: true, error: null });
        ...
        try {
          ...
          const errorMessage = error instanceof Error ? error.message : 'Erro ao duplicar workflow';
          set({ error: errorMessage, isCreating: false });
          return { success: false, error: errorMessage };
        }
      },

      toggleWorkflowStatus: async (workflowId) => {
        set({ isUpdating: true, error: null });
        ...
        try {
          ...
          const errorMessage = error instanceof Error ? error.message : 'Erro ao alterar status do workflow';
          set({ error: errorMessage, isUpdating: false });
          return { success: false, error: errorMessage };
        }
      },
      ...
      deleteSelected: async () => {
        ...
        for (const workflowId of selectedIds) {
          try {
            ...
            errors.push(`Erro ao remover workflow ${workflowId}: ${error instanceof Error ? error.message : 'Erro desconhecido'}`);
          }
        }
      ...
      toggleSelectedStatus: async () => {
        ...
        for (const workflowId of selectedIds) {
          try {
            ...
            errors.push(`Erro ao alterar status do workflow ${workflowId}: ${error instanceof Error ? error.message : 'Erro desconhecido'}`);
          }
        }
