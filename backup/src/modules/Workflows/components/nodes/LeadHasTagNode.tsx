...
const LeadHasTagNode = ({ 
  ...
  const handleRemoveTag = (tagToRemove) => {
    const updatedTags = config.tags.filter(tag => tag !== tagToRemove);
    handleConfigChange('tags', updatedTags);
  ...
  const handleTestCondition = async () => {
    ...
    try {
      ...
      setTestResult({
        success: result,
        leadData: sampleLead,
        matchedTags: config.tags.filter(tag => 
          leadTags.some(leadTag => 
            config.exactMatch 
              ? leadTag === tag 
              : leadTag.toLowerCase().includes(tag.toLowerCase())
  ...
  return (
    ...
    >
      ...
      <div className="node-content space-y-4">
        ...
        {/* Tags Configuration */}
        <div>
          <InputLabel>Tags to Check</InputLabel>
          {/* Add New Tag */}
        ...
        <div className="bg-gray-50 p-3 rounded-lg">
          <h4 className="font-semibold text-gray-900 mb-2">Matching Options</h4>
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <input
                type="checkbox"
          ...
          <div className={`p-3 rounded-lg ${testResult.success ? 'bg-green-50' : 'bg-red-50'}`}>
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className={`font-semibold ${testResult.success ? 'text-green-800' : 'text-red-800'}`}>
                {testResult.success ? 'Condition Met' : 'Condition Not Met'}
              </span>
            </div>
            <div className="text-sm space-y-1">
              <div>
                <strong>Operator:</strong> {testResult.operator}
              </div>
              <div>
                <strong>Lead Tags:</strong> {testResult.leadData.tags.join(', ')}
              </div>
              <div>
                <strong>Matched Tags:</strong> {testResult.matchedTags.join(', ') || 'None'}
              </div>
            </div>
          </div>
        )}
