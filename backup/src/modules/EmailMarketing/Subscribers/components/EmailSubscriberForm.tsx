...
const EmailSubscriberForm: React.FC<{subscriber?: any, onSave, onCancel}> = ({ subscriber = null, onSave, onCancel }) => {
    ...
    <Card>
      ...
      <Card.Content className="space-y-4">
        <div>
          ...
          <Input id="email" value={data.email} onChange={(e) => setData((p) => ({ ...p, email: e.target.value }))} placeholder="email@exemplo.com" className={errors.email ? 'border-red-500' : ''} />
          <InputError text={errors.email} />
        </div>
        <div>
          <InputLabel htmlFor="name">Nome</InputLabel>
          <Input id="name" value={data.name} onChange={(e) => setData((p) => ({ ...p, name: e.target.value }))} placeholder="Nome do inscrito" />
        </div>
      </Card.Content>
