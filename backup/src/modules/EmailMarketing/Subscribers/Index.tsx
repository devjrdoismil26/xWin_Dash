      ...
      <PageLayout>
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            ...
            <Button onClick={() => setIsFormModalOpen(true)}>Novo</Button>
          </div>
          <Card>
            <Card.Content>
              {loading ? (
                        ...
                        <tr key={item.id} className="border-b hover:bg-gray-50">
                          ...
                          <td className="py-2 px-3">
                            <div className="flex justify-end gap-2">
                              <Button variant="outline" size="sm" onClick={() => setSelectedSubscriber(item)}>Editar</Button>
                              <Button variant="destructive" size="sm" onClick={() => setIsConfirmDeleteModalOpen(true)}>Excluir</Button>
                            </div>
                          </td>
                        </tr>
                      ))}
          ...
          <Modal isOpen={isFormModalOpen} onClose={() => setIsFormModalOpen(false)} title={selectedSubscriber ? 'Editar Inscrito' : 'Adicionar Novo Inscrito'}>
            <div className="p-2 text-sm text-gray-600">Formulário em construção</div>
          </Modal>
          <ConfirmationModal
            ...
            text="Tem certeza que deseja excluir este inscrito?"
          />
        </div>
      </PageLayout>
    </AuthenticatedLayout>
