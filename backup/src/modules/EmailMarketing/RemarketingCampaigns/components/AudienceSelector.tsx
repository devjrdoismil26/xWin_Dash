...
const AudienceSelector: React.FC<{isOpen?: any, onClose, onSelect}> = ({ isOpen = true, onClose, onSelect }) => {
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-xs text-gray-600">{segment.description || 'Sem descrição'}</p>
                      </div>
                      <input
                        type="radio"
                        checked={selectedId === segment.id}
                        onChange={() => setSelectedId(segment.id)}
                      />
                    </div>
                  </Card>
                ))}
