/**
 * Sistema Avançado de Notificações - xWin Dash
 * Gerenciamento unificado de toasts, alertas e feedbacks
 */
...
const generateId = () => `notification-${Date.now()}-${++notificationCounter}`;
...
export const useAdvancedNotifications = (): UseNotificationsReturn => {
  ...
  const showNotification = useCallback((
    ...
    const newNotification: Notification = {
      id,
      timestamp: Date.now(),
      duration: notification.type === 'loading' ? undefined : (notification.duration ?? 5000),
      ...notification,
    };
